...
binary encodingは、domain直後から上記field順とする。全count/length/offset/size/dimension/layer indexは`u64` little-endian、stringは`u64` byte lengthの後にUTF-8、booleanは1 byteの`0/1`、optional layerは1 byteの`0/1`後にpresent時だけ`u64`、vectorは`u64` element count後に要素列とする。classification tagはrequired/config-conditional/known-unconsumedを`1/2/3`、consumer tagはnone/embedding+tied-output/final-norm/input-norm/post-norm/MLP gate/up/down/GDN 9 role/full-attention 6 roleを`0..23`、dtype tagはBF16/F16/F32/I32/I64/U8を`1..6`とする。chunk vectorはcount後に各`source_offset,destination_offset,byte_length`の3つの`u64`を置く。unknown tag、reserved tag、zero chunkを作らない。filesystem絶対path、payload bytes、HashMap順、JSON key順を含めない。digestは内部`[u8; 32]`、表示時だけ`sha256:<lower-hex>`とする。

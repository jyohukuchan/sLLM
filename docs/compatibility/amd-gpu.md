...
> 最終更新: 2026-09-25
>
> この文書はAMD向けの識別規則と初期候補を記録する。2026-09-09時点の初期targetはすべて`lifecycle=experimental`だった。計画targetのevidenceは`unverified`、canonical local実機のformal model-free G0/G1とPhase 6 A0 HIP VMM PoCは検証した限定範囲だけ`project-verified`とする。Phase 49のGQA P32とPhase 67のMXFP8 col8はexact `gfx1030`限定、Phase 50のResidual/GDN/MLP/P32、Phase 63のlarge-M MXFP8 WMMA、Phase 64／65のoperand direct-load、Phase 66のN128 ID37と低精度provider移植はexact `gfx1201`の狭いscope限定であり、target全体やSKU全体の昇格ではない。

2026-09-25のユーザー決定により、上の「初期target」の記述は過去の方針として残し、
Phase 87段階10以降の現行runtimeでは`gfx942`を`unsupported`とする。
Phase 12/36で得たMI300X実機evidenceは当時のsource/tupleに限定した履歴であり、
Paged KVの証拠へ継承しない。現行Rust sessionと公開C contextは
GPU allocation前に拒否し、旧residentコードとCI記述の撤去は段階10で続ける。
...
| CDNA 3 | `gfx942` | 初期FP8 pathでは使用しない | `unsupported`（2026-09-25現行runtime） | 過去binaryの`project-verified`（下記Hot Aisle tuple/scope限定。Paged未検証） |
...
### 2026-09-09 Phase83 exact `gfx1201` VMM alias診断と通常KV resident選択（旧runtime履歴）
...
当時のRust HIP adapterでは、sliding windowなしの通常KV stateをexact `gfx1201`で作る場合、logical capacityと

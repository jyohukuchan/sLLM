...
- `reference/llama.cpp`、`reference/vLLM`、`reference/SGLang`、`reference/AMD-ATOM`、`reference/TensorRT-LLM`は現時点では空であり、この配置からCI・テスト実装を調査することはできなかった。
- vLLMとTensorRT-LLMについては、workspace外に存在した読み取り専用mirrorを補助的に調査した。
...
- これらは設計上の参考であり、uLLMの対応実績または正しさの証拠にはしない。`reference/`へ固定commitのsourceを配置した後、一次sourceとして再調査する。
...
- 参照sourceを固定commitで`reference/`へ配置後、llama.cpp、vLLM、SGLang、ATOM、TensorRT-LLMのCI/testを再調査する。

...
| B3 tokenizer frontend | `crates/sllm-frontend/src/tokenizer.rs`、`crates/sllm-frontend/src/lib.rs`、新規`crates/sllm-frontend/tests/tokenizer_contract.rs`、`ci/fixtures/tokenizer-v1/**`、同fixtureをH1へ結合する`ci/matrix/path-to-suite-v1.json`の最小登録とrevision更新、対応する`ci/tools/validate_matrix.py`の期待revision同期、追加integration-test targetだけを反映する`ci/dependencies/rust-workspace-v1.json`のtarget inventory同期。B2 assetからだけ`tokenizers`を構築し、encode/decode、special-token identity、EOS集合との整合をtyped APIにする。 | 3〜5時間 / 6時間 | ASCII、Unicode、空、非整列長、未知/欠落/重複special token、malformed tokenizerをhost negative testで検証し、固定prompt token IDs/decodeがversioned fixtureと一致。H0〜H2、MSRV、fresh reviewがPASS。GPU/full modelなし。package、edge、lock、version、feature、licenseは変更せず、frontend module/fixture、H1 path登録、manifest/validator identity同期、target-only inventory同期だけでrollbackできる。 |
| B4 typed chat renderer | `crates/sllm-frontend/src/chat.rs`、`crates/sllm-frontend/src/lib.rs`、新規`crates/sllm-frontend/tests/chat_contract.rs`、`ci/fixtures/chat-template-v1/**`、`docs/references/qwen3.5-phase3-full-model-reader.md`、fixture専用H1 ownershipとrevisionを同期する`ci/matrix/path-to-suite-v1.json`・`ci/tools/validate_matrix.py`、新規integration-test targetだけを同期する`ci/dependencies/rust-workspace-v1.json`。locked template identityを要求し、Qwen3.5 text-only messageをtyped rendererで生成する。 | 3〜5時間 / 6時間 | fixed `hello`とUnicodeのrendered text/token IDsを固定し、image/video/tool/unknown role、不正content、template hash不一致を明示unsupportedにする。任意Jinjaを実行しない。package、edge、lock、version、feature、licenseを変えず、H0〜H2、fresh reviewがPASSし、renderer/module fixture、reader記録、H1 path/validator identity、target-only inventory同期だけでrollbackできる。実装前に固定templateのwhitespace、thinking branch、escapingのseparated reader記録を完了する。 |
...
B3 docs-only closeout commit `7904a2c196628adcc138eb6499a6a04bd5ebdb56`、tree `8217e4b698a390c31c10b6ed4460f63fa8988051`はpinned Python 3.12.10 strict H0 335/335、attempt 1、failed/skipped 0、clean exact SHA/tree、report SHA-256 `7f34247470172bb964992c28c7215fc1e17d91270e5279bd6ca40dcd5373defd`のsidecar一致とfresh独立review High/Medium/Low 0件を`PASS`し、B3全体を完了した。これをrollback baseとして2026-08-10 12:48:55 JSTにB4 typed chat rendererを開始する。予測3〜5時間、作業単位hard中断時刻18:48:55 JSTだが、先に到達する全体停止上限15:38:27 JSTをeffective hard stopとする。所有範囲は`crates/sllm-frontend/src/chat.rs`、`crates/sllm-frontend/src/lib.rs`、`crates/sllm-frontend/tests/chat_contract.rs`、`ci/fixtures/chat-template-v1/**`、固定Qwen3.5 templateのwhitespace・thinking branch・escaping調査を記録する`docs/references/qwen3.5-phase3-full-model-reader.md`、fixture専用H1 ownershipとrevisionを同期する`ci/matrix/path-to-suite-v1.json`・`ci/tools/validate_matrix.py`、新規integration-test targetだけを同期する`ci/dependencies/rust-workspace-v1.json`に限定する。実装前にreaderとimplementerを分離し、任意Jinjaを実行せず、text-only typed rendererと明示unsupported境界を固定する。readerはlock一致を確認した7,756-byteのfrontend template assetだけをbounded readし、weight payload、full model load、GPU、network、containerは使用しない。

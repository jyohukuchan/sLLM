...
| B4 typed chat renderer | `crates/sllm-frontend/src/chat.rs`、`crates/sllm-frontend/src/lib.rs`、`crates/sllm-frontend/Cargo.toml`、root `Cargo.lock`のlocal frontend依存一覧、新規`crates/sllm-frontend/tests/chat_contract.rs`、`ci/fixtures/chat-template-v1/**`、`docs/references/qwen3.5-phase3-full-model-reader.md`、fixture専用H1 ownershipとrevisionを同期する`ci/matrix/path-to-suite-v1.json`・`ci/tools/validate_matrix.py`、新規integration-test targetと既存workspace固定`sha2`へのfrontend edgeを同期する`ci/dependencies/rust-workspace-v1.json`・`ci/schema/rust-dependency-policy-v1.schema.json`・`ci/tools/validate_rust_dependencies.py`。locked template identityを要求し、Qwen3.5 text-only messageをtyped rendererで生成する。 | 3〜5時間 / 6時間 | fixed `hello`とUnicodeのrendered text/token IDsを固定し、image/video/tool/unknown role、不正content、template hash不一致を明示unsupportedにする。任意Jinjaを実行しない。既存`sha2`のversion/featureを再利用し、package集合は不変、frontend edgeは1件増、Cargo.lockはlocal `sllm-frontend`依存一覧だけを同期する。H0〜H2、MSRV、fresh reviewがPASSし、renderer/module fixture、reader記録、H1 path/validator identity、target/edge inventory同期だけでrollbackできる。実装前に固定templateのwhitespace、thinking branch、escapingのseparated reader記録を完了する。 |

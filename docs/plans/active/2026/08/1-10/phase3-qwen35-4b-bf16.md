...
各単位は開始時にrollback baseのcommit SHA/tree、開始時刻、予測、hard中断時刻を固定する。実装とreview修正後のcandidateを別のcommit SHA/treeへ固定し、その同一identityに受入evidenceを結合する。整理でtreeが変われば当該単位のevidenceを最初から取り直す。依存順は`B0 -> B1 -> B2 -> B3 -> B4 -> B5 -> B6 -> B7`とし、後続単位は直前単位のcandidate/evidence/reviewがPASSするまで開始しない。B0〜B6はhost-onlyで、full model download、weight payloadの一括materialize/decode/mmap/upload、GPU、containerを禁止する。固定cacheを使う適用確認は、全locked fileをbounded bufferでstreaming hashし、metadata/rangeを照合するだけのlocal model-bound evidenceとして行い、CPU CIへ持ち込まない。B7だけがHIP/runtime/backendへ影響し、canonical `0000:03:00.0`のV620とcanonical R9700を使う。spare V620は使用しない。

| ID | 所有範囲と成果物 | 予測 / hard上限 | 受入条件・evidence・rollback境界 |
| --- | --- | --- | --- |
| B0 dependency closure | `Cargo.toml`、`Cargo.lock`、`crates/sllm-frontend/Cargo.toml`、新規`ci/dependencies/rust-tokenizers-v1.json`、`ci/schema/rust-dependency-policy-v1.schema.json`、`ci/tools/validate_rust_dependencies.py`、`ci/tests/test_rust_dependencies.py`と必要な`ci/matrix/{suites-v1,host-v1,path-to-suite-v1}.json`登録。`tokenizers 0.21.4`のexact transitive version/checksum/license、許可feature `onig`、禁止feature、Rust 1.85 offline解決を機械検証する。 | 2〜4時間 / 5時間 | local crate cacheだけを使い、H0〜H2、`cargo +1.85.0 check --workspace --locked --offline`、fresh reviewがPASS。model/GPU/cache/networkなし。validator・manifest・Cargo/CI登録差分だけでrollbackできる。これを最初の実装単位とする。 |
...
| B7 exact-range GPU upload bridge | 新規`crates/sllm-hip/src/weights.rs`、`crates/sllm-hip/src/lib.rs`、`crates/sllm-hip/src/bin/sllm-weight-upload-g1-evidence.rs`、`ci/schema/weight-upload-g1-report-v1.schema.json`、`ci/matrix/weight-upload-semantic-g1-v1.json`、`ci/tools/validate_weight_upload_g1_contracts.py`、`ci/tests/test_weight_upload_g1_contracts.py`と既存suite/path登録。既存`Queue::copy_to_device()`とversioned transfer ABIだけを使い、B5 load planのverified chunkを順にopaque GPU bufferへuploadしてshardまたはtensor全体のhost複製を作らない。 | 6〜8時間 / 9時間 | 1、3、17、255、256、257 byte、複数chunk、tensor/destination境界前後でexact byte/readback、wrong range/target/dtype/identityをreject。H0〜H3、canonical G0/private G1、専用semantic upload G1、aggregate、pre/post health、cleanup、fresh reviewが同一SHAでPASS。canonical sLLM V620とR9700だけを使用し、失敗時はB6完了commitを維持する。native/C ABI変更が必要と判明した場合はscopeを拡張せず停止し、別のABI決定単位を計画する。 |

B0開始前にllama.cpp/vLLMの追加readerは不要である。ただしB0内では、local Cargo metadata/sourceだけを調べるread-only dependency auditorとvalidator implementerを分離する。B4だけは現行reader記録に正確なrender済みbytes、whitespace/escaping、thinking branchの全境界がないため、固定revisionのtemplate/tokenizer metadataを読むreaderとimplementerを分離する。B5はRust内部load-plan、B7は既存versioned transfer ABIの利用に限定するため、現時点のpublic C ABI未確定事項にはblockedされない。B7で既存ABI不足が判明した場合だけ中断し、runtime正本と互換性文書を同期する別単位を先に計画する。

2026-08-10時点でこのB0〜B7分割はhost H0 316/316、Markdown link、diff checkまでPASSしたplan candidateであり、実装開始済みではない。3回のfresh independent review transportがいずれも45分間local出力を生成せず停止したため、review判定は未完了である。同一immutable plan candidateのfresh reviewがHigh/Medium 0件でPASSするまでB0を開始しない。

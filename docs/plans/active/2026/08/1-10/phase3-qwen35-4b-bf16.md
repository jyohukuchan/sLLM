...
| B7 exact-range GPU upload bridge | `crates/sllm-core/src/weights.rs`、`crates/sllm-core/src/lib.rs`、`crates/sllm-hip/src/bin/sllm-weight-upload-g1-evidence.rs`、`ci/schema/weight-upload-g1-report-v1.schema.json`、`ci/matrix/weight-upload-semantic-g1-v1.json`、`ci/tools/validate_weight_upload_g1_contracts.py`、`ci/tests/test_weight_upload_g1_contracts.py`と既存suite/path登録。B5 load planのverified chunkを既存backend-neutral `ExecutionSession::upload()`へ順に接続し、既存HIP `ExecutionSessionAdapter`/versioned transfer ABIを通してopaque GPU bufferへuploadする。新しいHIP weight-upload wrapperや`Queue::copy_to_device()`直結経路を作らず、shardまたはtensor全体のhost複製も作らない。 | 6〜8時間 / 9時間 | 1、3、17、255、256、257 byte、複数chunk、tensor/destination境界前後でexact byte/readback、wrong range/target/dtype/identityをreject。H0〜H3、canonical G0/private G1、B5 load-plan接続専用semantic upload G1、aggregate、pre/post health、cleanup、fresh reviewが同一SHAでPASS。generic transferの重複実装・重複証明にはしない。canonical sLLM V620とR9700だけを使用し、失敗時はB6完了commitを維持する。native/C ABI変更が必要と判明した場合はscopeを拡張せず停止し、別のABI決定単位を計画する。 |

B0開始前にllama.cpp/vLLMの追加readerは不要である。ただしB0内では、local Cargo metadata/sourceだけを調べるread-only dependency auditorとvalidator implementerを分離する。B4だけは現行reader記録に正確なrender済みbytes、whitespace/escaping、thinking branchの全境界がないため、固定revisionのtemplate/tokenizer metadataを読むreaderとimplementerを分離する。B5はRust内部load-plan、B7は既存backend-neutral upload APIとversioned transfer ABIの利用に限定するため、現時点のpublic C ABI未確定事項にはblockedされない。B7で既存ABI不足が判明した場合だけ中断し、runtime正本と互換性文書を同期する別単位を先に計画する。

2026-08-10時点でこのB0〜B7分割はhost H0 316/316、Markdown link、diff checkまでPASSしたplan candidateであり、実装開始済みではない。3回のfresh independent review transportがいずれも45分間local出力を生成せず停止した後、commit専用reviewでB0が`tokenizers`閉包外のworkspace依存を網羅しないMedium 1件を検出した。全workspace targetの依存graphへ対象を拡張し、B7も既存`ExecutionSession::upload()`を再利用するよう修正した。同一immutable plan candidateのfresh reviewがHigh/Medium 0件でPASSするまでB0を開始しない。

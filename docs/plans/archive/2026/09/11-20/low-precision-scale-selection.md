...
- 完了（2026-09-19）。フェーズ番号は割り当てていない。段階0〜4はCodex、段階5はオーケストレーターが実施した。
- 2026-09-19のユーザー決定で、MXFP8重み・活性値とMXFP6重みの飽和しないscaleを既定にした。NVFP4活性値の
  2候補選択は、wave8カーネルの不具合を修正して測り直すと悪化したため既定にしなかった（履歴の段階5を参照）。
- 段階0〜4は2026-09-19に完了（測定値・hash・失敗試行・形式ごとの採否判断材料は
  [`docs/history/2026/09/11-20/low-precision-scale-selection.md`](../../../../../history/2026/09/11-20/low-precision-scale-selection.md)
  に記録）。段階5も同日に完了した。
...
[実装・検証履歴](../../../../../history/2026/09/11-20/low-precision-scale-selection.md)

...
- text frontendは`tokenizers =0.21.4`をdefault featureなし・`onig` featureだけで固定し、HTTP、progressbar、`esaxx_fast`を無効にする。chat templateは任意Jinjaを実行せず、locked template hashを検証したtyped Qwen3.5 text-only rendererとする。停止policyをversioned lock/schema/APIへ反映し、全依存version、checksum、license、MSRVをroot lockへoffline固定してからB1を実装する。
...
- Phase 3全体の作業単位、受入条件、evidence、rollback境界は[Phase 3 Qwen3.5-4B BF16 text生成計画](active/2026/08/1-10/phase3-qwen35-4b-bf16.md)を正とし、最初のmodel-bound数値経路は[Stage A model lock・RMSNorm・G2計画](active/2026/08/1-10/phase3-model-lock-rmsnorm-g2.md)を正とする。

## 現在の状態と次の作業

- 現在: Phase 2前半のH3、G0、model-free G1を完了した。commit `f393d688a051d2b73c8773d8a930a711592609bc`（tree `2ccda6e7c0614d585f26babc6b7c68ca51220bbe`）に対し、H0〜H3とcanonical `gfx1030`/`gfx1201`のG0/G1・fail-closed集約がPASSした。Phase 3では`Qwen/Qwen3.5-4B`を完全SHA `851bf6e806efd8d0a36b00ddf55e13ccb7b8cd0a`へ固定し、完全model lock、reader記録、hash済みFDへsemantic parseを結び付けるhost validator、RMSNorm semantic contractと独立NumPy oracleを受入済みとした。実cache 13 file・9,342,905,899 bytesのcontent-only再検証はPASSし、物理的trusted read-only条件は未成立として分離した。public HIP runtime、baseline RMSNorm、専用H3 compile-only、controller-owned semantic G1のhost実装とmain-side authority監査まで進み、現行dirty worktreeではsemantic 38件とH0 131件・H1 151件・H2 35件、local fail-closed集約がPASSした。semantic G1のfresh独立再review、同一immutable candidateのH0〜H3とcanonical GPU G0/G1/G2/P0、後続のfull text生成/G3は未完了である。H3 required昇格の20回・7日観測は並行follow-upであり、Phase 3開始を待たせない。
...
- 2026-08-08にPhase 3 Stage Aを再開する。各工程は開始前に予測時間を固定し、予測上端を1時間超えた時点を中断上限とする。上限到達時は安全なrollback可能点で一旦停止し、経過時間、完了範囲、未完了理由、次の分割案を報告する。Stage Aの初期見積りと上限は[Stage A計画](active/2026/08/1-10/phase3-model-lock-rmsnorm-g2.md)を正とし、後続Stageも開始前に8時間以下の作業単位へ分割する。
...
- 次:
  1. [Stage A計画](active/2026/08/1-10/phase3-model-lock-rmsnorm-g2.md)に従い、controller-owned semantic G1のfresh独立再reviewと、direct test・標準containerによるhost再検証を進める。中断されたcustom capsule hardeningは使用しない。
  2. G2/P0を実装し、immutable host evidence取得前に最小のtrusted-development baselineを新規作成・reviewしたうえで、同一immutable candidateのpublic HIP runtime、baseline RMSNorm、semantic G1、real-weight G2、P0をH0〜H3およびcanonical GPU G0とともにfail-closed集約する。

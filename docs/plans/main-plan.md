...
- Phase 3全体の作業単位、受入条件、evidence、rollback境界は[Phase 3 Qwen3.5-4B BF16 text生成計画](archive/2026/08/1-10/phase3-qwen35-4b-bf16.md)を正とし、完了した最初のmodel-bound数値経路は[Stage A model lock・RMSNorm・G2計画](archive/2026/08/1-10/phase3-model-lock-rmsnorm-g2.md)に記録する。

## 現在の状態と次の作業

- 最新状態: Phase 3は2026-08-11に完了し、詳細なcloseout evidenceは[archive済みPhase 3計画](archive/2026/08/1-10/phase3-qwen35-4b-bf16.md)を正とする。H3 required promotionはnonblocking follow-upであり、Phase 3をblockしない。
...
- Stage D D0 host-only structural graph draftを2026-08-11に実装した。固定lock/canonical load planからembedding、明示32 layer、final norm、tied LM head、argmaxを484 node、required 426 binding、known-unconsumed 312 metadata、linear/full state合計64 logical rowとして構築する。Q/K scale materialization、checked alias backing、linear stateの実weight接続、KV descriptor owner、component countとsource-range非overlapを型付きで固定した。独立reviewのHigh 3/Medium 4/Low 2は修正済みで、core unit 70件・integration 18件、all-target check、format、clippy `-D warnings`、diff checkがPASSした。D0はGPU、model payload、runtime prefill/decodeを含まず、次はD1 request-local orchestrationである。
- Stage D D1 host-only orchestrationとD2 dirty full provisioningを実装した。D1はrequired upload、owned/alias buffer、8 KV/24 linear state、Q/K scale展開、prefill/decode、publication/poisonを既存ownerへ接続し、host検証をPASSした。D2は固定cache fingerprint `sha256:f143d7b504170d071c77818105f7a07dc0297c6bea0c61a5404b071fed0c1fae`とplan digest `sha256:0474ed893fbc043c3ace0197515f8d99e27fe3d28a4844fbdd9781bb9d30c7fa`でrequired 426を全uploadし、canonical V620/R9700のVRAMをそれぞれ16→8,478→16 MiB、257→8,735→257 MiBと実測した。fallback/forward/kernel dispatchなし、cleanup 0である。immutable candidate、real-weight G2、最終review前のためD2は未完了である。
- Stage D integration candidate `992e190326259fe55b210f684b1f482905ed63d3`、tree `5f7937cc670528fccbc119796e940aa977ba479b`でhost gate、exact dual-target build、canonical V620/R9700 full provisioning、現行lockのreal-weight RMSNorm G2各6 case、pre/post healthとcleanupをPASSした。D2 binary SHA-256は`gfx1030=1366602c2844d39a389fed00492d355516fec76c4cb18b1102e013b0f1f83871`、`gfx1201=cc603af979449db8fd89dc4adbd5815e6710459b1d2ae7564a767f6061055f1b`、G2 aggregate SHA-256は`29f8c829eead02c1ca47871041ccd75f094911918d086ce170c690dcce870760`で、fallbackと残留processはない。最終reviewは指摘0件でD2/Stage Dを完了した。
- Stage EをE0 CLI/stop orchestration（4〜6時間）、E1 V620 first full-model smoke（8〜14時間）、E2 G3 schema/golden/dual-GPU evidence（6〜10時間）へ分割した。合計予測は18〜30時間で、2回reject、1時間の機能進捗停止、各上端超過、新HIP ABI/kernel必要判明をreplan triggerとする。E0はhost focused contract、E1はV620 PASS後だけR9700、E2は同一immutable identityの両GPU一致とaggregateを受入条件とする。
- Stage E E0 host draftを完了した。`sllm generate`へverified prompt/chat frontend、exact target/device、明示greedy、bounded max tokens、D1 prefill/decode、versioned stop判定、stop token非表示、cleanupを接続した。同一生成loopのfake executorで両stop IDとbudget 1/3/17/255/256/257を検証し、workspace check、CLI 11件、frontend 48件、clippy、exact target別buildをPASSした。GPU forwardは未実行で、次はE1 V620 first smokeである。
- E1 dirty full-model smokeで、tokenizer base 248,044、added後ID span 248,070、model/LM-head capacity 248,320を既存frontendが等値比較していたB3負債を検出し、全tokenizer IDがmodel capacity内にある契約へ修正した。V620の`Hello` max 1/2とR9700 max 2は生成`[220]`/`[220,220]`、max budget stop、fallbackなし、cleanup 0でPASSし、両targetが一致した。修正commitのclean再実行前のためE1は未完了である。
- E1 candidate `a7abc5a3eeef895ec49a98123337a617bd74f554`、tree `a766951e784520b22180ab017643140d0f7f727d`でhost gateとexact dual-target CLI buildをPASSした。clean `Hello` max 2は両GPUでinput `[9419]`、generated/visible `[220,220]`、decode input `[220]`、max budget stopが一致し、V620 7.394秒、R9700 6.366秒、fallback/cleanup残件/全GPU process残留なしだった。E1を完了した。
- E2数値診断でE1の`[220,220]`を不正な旧結果として退けた。GDNとelementwiseのBF16境界・query scale・decay順序を修正し、独立NumPy oracleとV620の全491 output tensorをbit exactにした。全compute submissionの実metadataを集約するHIP/no-fallback dispatch auditもproduction CLIへ追加した。dirty exact両targetでASCII 8-token、Unicode stop `248046`、257-token非整列prefillのtoken列と監査数が一致した。次はこのsemantic/build unitをrollback可能なcheckpointへ固定し、tracked G3 controller/schema/aggregate、immutable dual-GPU evidence、最終reviewを取得する。`248044`はhost stop-controller coverageを維持し、安定したfull-model promptなしにGPU goldenを作らない。
- E2 semantic/build unitをcommit `64a9f9752712fc7cff4f482ea7888b0aba03b719`、tree `cea24196f45881d4d8a5e0d23aa686b7594a440e`へ固定した。これを推論binaryの検証対象とし、後続commitでtracked化するG3 controller/schema/matrixはevidence tooling identityとして分離する。exact binary SHA-256は`gfx1030=62c02a93960cbd462ee8bc16aa9031db98d6d16bb25cea95fd9bc8a64267d867`、`gfx1201=20235ac7761721be62586313ed4976a503cd210899e976de49948148bdf0988c`である。canonical V620 `GPU-76a08c022586fed6`とR9700 `GPU-a8e9ddefa2d60f55`でprompt長`1/7/255/256/257`とUnicode stop `248046`の計12 runを取得し、12/12 PASS、全48 golden audit比較一致、全dispatch HIP、fallbackなし、timeout・実行前後health・process cleanup PASSとなった。aggregate SHA-256は`c94259b1ec4ac173c8b70d7bfffc3625017c77c4e40b1c46f26e5f0055cc0708`である。`amd-smi process -g`が選択外GPUのprocessも列挙するhost挙動により並列試行は保守的なfalse FAILとなったため、採用evidenceは両GPUを直列実行した。host contract 13/13、両schema、canonical matrix、全manifestからのaggregate bit-exact再計算をPASSした。integration reviewはHigh/Medium 0、Low 1で、schema counter/PIDの有限上限不足を修正してnegative testを追加した。同一単位の3回目reviewはreplan ruleに従い行わず、指摘限定testで閉じた。E2/G3 integrationは完了したが、Phase 3全体のrelease判定とfinal relevant matrixは未完了である。
- release整形をsemantic/build commit `2c859a579007c388d956e46905ef71fb5a9d5881`、tree `1fb63550fe2d4405a97a00ba28e2bcb1dfc75f14`へ固定し、local-development H0 340/340、H1 312/344、H2 35/42をPASSした。exact CLI SHA-256は`gfx1030=360154b9f43c7be7db26676cd6635b3a33da450275752076779f3cba7adf11a1`、`gfx1201=faafe84bc320b413661214393abea41a107a8893ddb1cf6dcdcd34247410868a`で、各artifactはhostと指定targetだけのbundleを含む。evidence tooling commit `b74bf2d10330d9c0c5ceae46b84a9716b40df5ad`へmatrix bindingを分離し、同semantic/build candidateのG3を両GPU各6 case、attempt 2で12/12 PASS、48比較0 mismatchとして再取得した。final aggregate SHA-256は`00321368b363f75f9140cf6eb2c7c9e94f6fe47a14688ad86097f229d93e5a2f`である。attempt 1はruntime loader path未指定によりGPU実行前にexit 127で失敗し、採用しない。累積release reviewはcorrectness/security blocker 0、identity分離とG3 evidenceに指摘なしだったが、最初のhost evidenceがPython 3.12.3である点をrelease-evidence High 1件とした。固定Python 3.12.10のdetached clean candidateでstrict H0を2回実行したところ、いずれも340件中同じ3 commandだけが2 GiB RSS上限を超えて337/340 FAILした。attempt 2 peakはclippy 3.67 GiB、MSRV 2.31 GiB、dependency validator 3.30 GiBである。2回rejectのreplan ruleに従い追加verification/reviewを停止し、Phase 3の機能実装とG3 integrationは完了、release/pushはH0 resource contractのreplanまで保留とする。
...
- 2026-08-11にPhase 3を完了した。host evidence/tooling candidateはcommit `e3def2dffd018c0b8df41e5e1d8940270060182a`、tree `f284b329e9252167182769311d1a64c2c7254625`で、先行release candidateとの差分はH0 clippy、MSRV check、dependency cargo checkの各Cargo commandへの明示`--jobs 1`とexact contract testsだけであり、推論source/build inputs、model lock、GPU artifact、G3 controller/matrix意味論、2 GiB gateは不変である。focused independent reviewは`PASS/no findings`（Python static、focused 7/7・18/18、diff check、Cargo option placement、H0 limit）だった。run-id `phase3-release-e3def2df` attempt 1のclean detached pinned Python 3.12.10 strict tupleはH0 340/340、H1 312/312、H2 35/35、strict aggregate `PASS`（errors/warnings 0）となり、report SHA-256はH0 `8902d167cceb17eb38c5b5c59d39276b459678f6b0bf5584f4e26a6ab3f7efad`、H1 `327d7e03c141671a4c209ba478ceb6da3612a2baffcddedfeefb034cea54aa96`、H2 `2150963dd5a53a4948cabf316ebd9994729f524ede2fbb4b03becbb533a51ef9`、matrix `c528256c36a02d31a0770756d54d2d3b4c84dac36100e47f6c2200ad4a498ee4`、aggregate `609800e9cf432ece8ce8096c5fa6409013fbfc5c5aa0ba2ea38cc7110fa111b4`である。既存release evidenceはsemantic/build commit `2c859a579007c388d956e46905ef71fb5a9d5881`、tree `1fb63550fe2d4405a97a00ba28e2bcb1dfc75f14`、exact CLI hash、G3 tooling commit `b74bf2d10330d9c0c5ceae46b84a9716b40df5ad`、matrix `b0e3bc7d31cf8084bb8e3e5c66767353eb5e75992b72a4edd977e97656184f39`、aggregate `00321368b363f75f9140cf6eb2c7c9e94f6fe47a14688ad86097f229d93e5a2f`へmappingし、canonical V620/R9700のattempt 2 12/12 PASS、48 comparisons、0 mismatchを再利用した。旧host Python/resource findingはこのstrict tupleとfocused reviewで解決し、e3def2dfではGPUを再実行しておらず、H3 required promotionはnonblocking follow-upとして継続する。
...
- 次:
  1. Stage DはD0 graph、D1 request-local orchestration、D2 dual-GPU full provisioning/real-weight G2まで完了した。
  2. Phase 3のE2/G3 integration、format済みcandidateのcanonical両GPU12/12 evidence、host strict tuple、最終resolutionを完了した。release reviewの実装指摘は0件で、旧host Python/resource findingはe3def2dfのstrict H0とfocused reviewで解決済みである。詳細なidentity・report・hashはarchive済みPhase 3計画を正とする。
...
- Phase 3で完了したG3 golden token sequence以外の、追加op・shape・入力範囲に対するGPU数値toleranceと性能回帰閾値は将来の拡張課題とする。固定metadataから決めた停止policyをgoldenから逆算しない。

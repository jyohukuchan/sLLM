...
are not benchmark evidence. `ullm-openai.service` became active at 20:19:35
JST while the direct run's tail and the entire candidate run were executing.
The journal does not attribute that start to a particular concurrent task.
They also record different `runner_git_head` values because the

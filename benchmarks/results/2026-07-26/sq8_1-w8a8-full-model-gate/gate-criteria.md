# SQ8_1 W8A8 full-model quality gate — qualifying-run criteria v0.2

Status: **frozen before the qualifying measurement run** (2026-07-26)

This file is the decision contract for the qualifying run. Its numerical
thresholds are not to be loosened after observing a result. A missing metric,
an incomplete corpus, a non-finite value, or a failed fake-quant harness
control is a No-Go.

## Protocol revision record

The preserved v0.1 run in `attempt-1-coverage-incomplete/` evaluated the
specified 20 evenly spaced records at a 256-token cap. It produced 3,568 valid
scored positions, below the already-frozen 4,000-position coverage requirement.
It is therefore explicitly non-qualifying and is retained as raw provenance,
not used as a pass result.

Before this qualifying run, v0.2 changes only the per-record truncation cap
from 256 to 384 tokens for the **same** 20 record IDs and five-domain x4
selection. A tokenizer-only coverage probe showed 4,243 positions at that cap.
This strictly adds scored positions; it does not relax an acceptance threshold,
replace a record, alter a quantizer, or change any numerical/top-k/top-1
criterion below. All decision thresholds in v0.1 are reproduced unchanged.

A one-prompt / 16-token instrument smoke was excluded before either 20-record
run after its strict control exposed a CPU BF16-versus-FP32 accumulation
difference. The qualifying gate uses the model converted to FP32 and a matching
FP32 `F.linear` operand boundary, avoiding that mixed-precision ambiguity.
...
- Primary candidate scope: every one of the 248 transformer `torch.nn.Linear`
  ...
  FP32 because they are not selected by the present SQ8_1 projection path.
...
- Corpus: 20 deterministically evenly spaced records from frozen
  `D_stats-shard-00.jsonl`, truncated at 384 tokens. The selector must cover
...
relative L2 and cosine, top-k preservation, reference top-1 retention, hidden
maximum error, and token agreement. Its published AQ4 bounds are loose because
AQ4 is a 4-bit production path (`logits_relative_l2 <= 0.1468`,
...
int8 quantizer. They are therefore applied to the FP32 re-expression harness
...
strictly defined near-margin swap to the FP32 runner-up. The rule is fixed
...
| W8A8 logits | Absolute fidelity | aggregate logits relative L2 <= `0.060`; worst per-prompt logits relative L2 <= `0.080`; aggregate logits max abs <= `1.0`; mean token KL(FP32||W8A8) <= `0.005`; worst per-prompt mean KL <= `0.010` |
...
| Top-k | FP32/W8A8 ranking preservation | aggregate top-10 overlap >= `0.950`; FP32 top-1 is in W8A8 top-10 for every scored position |
| Greedy token agreement | Teacher-forced logits at all scored positions | exact agreement >= `99.0%` and 95% Wilson lower bound >= `98.5%`; every mismatch must have W8A8 top-1 equal FP32 top-2 **and** FP32 top1-minus-top2 margin <= `0.050` |
...
the raw FP32 logit difference at that position. Token agreement is
...
`max(abs(x))/RMS >= 4` bypass activation quantization. It is an upper bound for
an outlier side route, not a deployable format and not a pass-path.
...
W8A8 gate or removes at least 50% of the W8A8-to-W8A16 aggregate-logit-L2 gap.
Otherwise an outlier side route is not supported by this run. A per-channel
scale or SmoothQuant design remains unconfirmed until separately implemented
and re-gated; this run may only identify it as a next experiment.

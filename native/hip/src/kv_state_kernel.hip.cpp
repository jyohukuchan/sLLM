...
extern "C" __global__ __launch_bounds__(
  ...
  const uint64_t total = static_cast<uint64_t>(token_count) *
                         SLLM_HIP_KV_HEAD_COUNT * SLLM_HIP_KV_HEAD_DIM;
  if (element >= total) {
    return;
  }
  const uint64_t row =
      element / (SLLM_HIP_KV_HEAD_COUNT * SLLM_HIP_KV_HEAD_DIM);
  const uint64_t within_row =
      element % (SLLM_HIP_KV_HEAD_COUNT * SLLM_HIP_KV_HEAD_DIM);
  const uint64_t head = within_row / SLLM_HIP_KV_HEAD_DIM;
  const uint64_t dimension = within_row % SLLM_HIP_KV_HEAD_DIM;
  const uint64_t output_offset = head * capacity_tokens * SLLM_HIP_KV_HEAD_DIM +
                                 (start_position + row) * SLLM_HIP_KV_HEAD_DIM +
                                 dimension;
...
hipError_t launch(const uint16_t *const key_input,
  ...
  hipLaunchKernelGGL(sllm_kv_state_bf16_to_f16_transpose_v1, grid, block, 0U,

#include "minimax_m3_moe_route_kernel_internal.hpp"

#include <cmath>

namespace {

__device__ __forceinline__ float stable_sigmoid(const float value) noexcept {
  if (value >= 0.0F) {
    const float negative_exp = expf(-value);
    return 1.0F / (1.0F + negative_exp);
  }
  const float positive_exp = expf(value);
...
fail_row(int32_t *const expert_ids, float *const expert_weights,
  ...
  (void)atomicCAS(status, SLLM_MINIMAX_M3_MOE_ROUTE_STATUS_OK, failure_status);
  for (uint32_t slot = 0U;
       slot < SLLM_HIP_MINIMAX_M3_MOE_ROUTE_SELECTED_EXPERT_COUNT; ++slot) {
...
extern "C" __global__ __launch_bounds__(
  ...
  constexpr uint64_t expert_count = SLLM_HIP_MINIMAX_M3_MOE_ROUTE_EXPERT_COUNT;
  ...
  for (uint64_t expert = 0U; expert < expert_count; ++expert) {
    if (!isfinite(logits[row + expert]) || !isfinite(selection_bias[expert])) {
  ...
  for (uint32_t slot = 0U; slot < selected_count; ++slot) {
    ...
    for (uint32_t expert = 0U; expert < expert_count; ++expert) {
      ...
      const float score = stable_sigmoid(logits[row + expert]);
  ...
  for (uint32_t slot = 0U; slot < selected_count; ++slot) {
    const float weight = (selected_scores[slot] / normalizer) * 2.0F;
    if (!isfinite(weight)) {
      fail_row(expert_ids, expert_weights, status, pair_base,
               SLLM_MINIMAX_M3_MOE_ROUTE_STATUS_NONFINITE);
      return;
...
extern "C" __global__ __launch_bounds__(
  ...
  constexpr uint32_t expert_count =
      static_cast<uint32_t>(SLLM_HIP_MINIMAX_M3_MOE_ROUTE_EXPERT_COUNT);
  constexpr uint32_t selected_count =
      SLLM_HIP_MINIMAX_M3_MOE_ROUTE_SELECTED_EXPERT_COUNT;
  ...
  if (expert < expert_count) {
    int32_t count = 0;
    if (*status == SLLM_MINIMAX_M3_MOE_ROUTE_STATUS_OK) {
  ...
  __syncthreads();
  if (expert < expert_count && *status == SLLM_MINIMAX_M3_MOE_ROUTE_STATUS_OK) {
  ...
  } else if (expert == 0U && *status != SLLM_MINIMAX_M3_MOE_ROUTE_STATUS_OK) {
    for (uint64_t pair = 0U; pair < pair_count; ++pair) {
      expert_ids[pair] = -1;
...
hipError_t launch(const float *const logits, const float *const selection_bias,
  ...
  hipLaunchKernelGGL(sllm_minimax_m3_moe_route_sigmoid_top4_v1,
                     dim3(static_cast<uint32_t>(token_count)),
                     dim3(SLLM_HIP_MINIMAX_M3_MOE_ROUTE_WORKGROUP_SIZE), 0U,
                     stream, logits, selection_bias, expert_ids, expert_weights,
                     status);
  result = hipGetLastError();
  if (result != hipSuccess) {
    return result;
  }
  hipLaunchKernelGGL(sllm_minimax_m3_moe_route_stable_group_v1, dim3(1U),
                     dim3(SLLM_HIP_MINIMAX_M3_MOE_ROUTE_WORKGROUP_SIZE), 0U,

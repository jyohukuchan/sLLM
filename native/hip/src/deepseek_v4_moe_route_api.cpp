...
validate_tensor(const sllm_tensor_binding_t &binding, const uint32_t rank,
  ...
  if (binding.buffer == nullptr || binding.rank != rank ||
    ...
    return sllm_public_runtime::write_error(
        ...
        "DeepSeek V4 route tensor buffer, rank, dtype, or encoding differs");
  }
  if (binding.byte_offset % element_bytes != 0U) {
    return sllm_public_runtime::write_error(
        sink, SLLM_STATUS_MISALIGNED_OFFSET,
        "DeepSeek V4 route tensor offset is misaligned");
  }
  ...
  if ((rank == 2U && multiply_overflows(elements, second_extent, &elements)) ||
    ...
    return sllm_public_runtime::write_error(
        ...
        "DeepSeek V4 route tensor interval overflowed u64");
  }
...
sllm_status_t validate_inactive_tensor(const sllm_tensor_binding_t &binding,
                                       sllm_error_sink_t *const sink) noexcept {
  if (!all_zero(&binding, sizeof(binding))) {
    return sllm_public_runtime::write_error(
        sink, SLLM_STATUS_INVALID_TENSOR_BINDING,
        "DeepSeek V4 route inactive tensor binding must be all-zero");
  }
  return SLLM_STATUS_OK;
}

sllm_status_t
validate_query_info(const sllm_deepseek_v4_moe_route_query_info_t *const info,
                    sllm_error_sink_t *const sink) noexcept {
  if (info == nullptr) {
    return sllm_public_runtime::write_error(
        sink, SLLM_STATUS_INVALID_ARGUMENT,
        "DeepSeek V4 route query info output is null");
  }
  uint32_t prefix[3] = {};
  ...
  if (prefix[0] != sizeof(*info)) {
    return sllm_public_runtime::write_error(
        sink, SLLM_STATUS_INVALID_ARGUMENT,
        "DeepSeek V4 route query info struct size is unsupported");
  }
  if (prefix[1] != SLLM_HIP_ABI_VERSION) {
    return sllm_public_runtime::write_error(
        sink, SLLM_STATUS_INVALID_ABI_VERSION,
        "DeepSeek V4 route query info ABI is unsupported");
  }
  if (prefix[2] != SLLM_HIP_DEEPSEEK_V4_MOE_ROUTE_QUERY_INFO_VERSION) {
    return sllm_public_runtime::write_error(
        sink, SLLM_STATUS_INVALID_ARGUMENT,
        "DeepSeek V4 route query info version is unsupported");
  }
  if (info->reserved0 != 0U ||
      !all_zero(info->reserved, sizeof(info->reserved))) {
    return sllm_public_runtime::write_error(
        sink, SLLM_STATUS_RESERVED_NONZERO,
        "DeepSeek V4 route query info reserved fields must be zero");
  }
  return SLLM_STATUS_OK;
}
...
sllm_status_t validate_descriptor_prefix(
  ...
  if (descriptor == nullptr) {
    return sllm_public_runtime::write_error(
        sink, SLLM_STATUS_INVALID_ARGUMENT,
        "DeepSeek V4 route descriptor is null");
  }
  uint32_t prefix[2] = {};
  ...
  if (prefix[0] != sizeof(*descriptor)) {
    return sllm_public_runtime::write_error(
        sink, SLLM_STATUS_INVALID_ARGUMENT,
        "DeepSeek V4 route descriptor struct size is unsupported");
  }
  if (prefix[1] != SLLM_HIP_ABI_VERSION) {
    return sllm_public_runtime::write_error(
        sink, SLLM_STATUS_INVALID_ABI_VERSION,
        "DeepSeek V4 route descriptor ABI is unsupported");
  }
  return SLLM_STATUS_OK;
}
...
sllm_status_t validate_and_copy_descriptor(
  ...
  if (metadata == nullptr) {
    return sllm_public_runtime::write_error(
        sink, SLLM_STATUS_INVALID_ARGUMENT,
        "DeepSeek V4 route descriptor metadata output is null");
  }
  const sllm_status_t prefix = validate_descriptor_prefix(descriptor, sink);
  if (prefix != SLLM_STATUS_OK) {
    return prefix;
  }
  if (descriptor->op_version != SLLM_HIP_DEEPSEEK_V4_MOE_ROUTE_VERSION) {
    return sllm_public_runtime::write_error(
        sink, SLLM_STATUS_INVALID_ARGUMENT,
        "DeepSeek V4 route operation version is unsupported");
  }
  if (descriptor->reserved0 != 0U ||
      !all_zero(descriptor->reserved, sizeof(descriptor->reserved))) {
    return sllm_public_runtime::write_error(
        sink, SLLM_STATUS_RESERVED_NONZERO,
        "DeepSeek V4 route descriptor reserved fields must be zero");
  }
  if (descriptor->mode != SLLM_DEEPSEEK_V4_MOE_ROUTE_MODE_SCORE &&
      descriptor->mode != SLLM_DEEPSEEK_V4_MOE_ROUTE_MODE_HASH) {
    return sllm_public_runtime::write_error(
        sink, SLLM_STATUS_INVALID_ARGUMENT,
        "DeepSeek V4 route mode is unsupported");
  }
  if (descriptor->selected_expert_count !=
      SLLM_HIP_DEEPSEEK_V4_MOE_ROUTE_SELECTED_EXPERT_COUNT) {
    return sllm_public_runtime::write_error(
        sink, SLLM_STATUS_UNSUPPORTED,
        "DeepSeek V4 route requires the reviewed top-6 shape");
  }
  if (descriptor->renormalize > 1U ||
      !std::isfinite(descriptor->routed_scale) ||
      descriptor->routed_scale <= 0.0F) {
    return sllm_public_runtime::write_error(
        sink, SLLM_STATUS_INVALID_ARGUMENT,
        "DeepSeek V4 route renormalize flag or positive routed scale is "
        "invalid");
  }

  if (descriptor->logits.shape[0] == 0U ||
      descriptor->logits.shape[0] > SLLM_HIP_DEEPSEEK_V4_MOE_ROUTE_MAX_TOKENS) {
    return sllm_public_runtime::write_error(
        sink, SLLM_STATUS_UNSUPPORTED,
        "DeepSeek V4 route token count exceeds its launch contract");
  }
  metadata->token_count = descriptor->logits.shape[0];
  metadata->expert_count = SLLM_HIP_DEEPSEEK_V4_MOE_ROUTE_EXPERT_COUNT;
  metadata->selected_expert_count = descriptor->selected_expert_count;
  metadata->mode = descriptor->mode;
  metadata->renormalize = descriptor->renormalize;
  metadata->routed_scale = descriptor->routed_scale;

  sllm_status_t status = validate_tensor(
      descriptor->logits, 2U, SLLM_TENSOR_DTYPE_BF16, UINT64_C(2),
  ...
  if (metadata->mode == SLLM_DEEPSEEK_V4_MOE_ROUTE_MODE_SCORE) {
    ...
    status = validate_inactive_tensor(descriptor->hash_expert_ids, sink);
  } else {
    status = validate_inactive_tensor(descriptor->selection_bias, sink);
    if (status == SLLM_STATUS_OK) {
      status = validate_tensor(
          descriptor->hash_expert_ids, 2U, SLLM_TENSOR_DTYPE_I32, UINT64_C(4),
          metadata->token_count, metadata->selected_expert_count,
          &metadata->hash_expert_ids, sink);
    }
  }
  if (status != SLLM_STATUS_OK) {
    return status;
  ...
  if (multiply_overflows(metadata->token_count, metadata->selected_expert_count,
    ...
    return sllm_public_runtime::write_error(
        sink, SLLM_STATUS_METADATA_OVERFLOW,
        "DeepSeek V4 route pair count overflowed u64");
  }
  uint64_t cursor = 0U;
  ...
  if (multiply_overflows(metadata->pair_count, UINT64_C(4), &cursor)) {
    return sllm_public_runtime::write_error(
        sink, SLLM_STATUS_METADATA_OVERFLOW,
        "DeepSeek V4 route metadata layout overflowed");
  }
  ...
  if (descriptor->metadata.byte_offset % alignof(int32_t) != 0U) {
    return sllm_public_runtime::write_error(
        sink, SLLM_STATUS_MISALIGNED_OFFSET,
        "DeepSeek V4 route metadata output is not i32 aligned");
  }
  return SLLM_STATUS_OK;
}

sllm_status_t validate_dispatch_info(
    const sllm_deepseek_v4_moe_route_dispatch_info_t *const info,
    sllm_error_sink_t *const sink) noexcept {
  if (info == nullptr) {
    return sllm_public_runtime::write_error(
        sink, SLLM_STATUS_INVALID_ARGUMENT,
        "DeepSeek V4 route dispatch info output is null");
  }
  uint32_t prefix[3] = {};
  std::memcpy(prefix, info, sizeof(prefix));
  if (prefix[0] != sizeof(*info)) {
    return sllm_public_runtime::write_error(
        sink, SLLM_STATUS_INVALID_ARGUMENT,
        "DeepSeek V4 route dispatch info struct size is unsupported");
  }
  if (prefix[1] != SLLM_HIP_ABI_VERSION) {
    return sllm_public_runtime::write_error(
        sink, SLLM_STATUS_INVALID_ABI_VERSION,
        "DeepSeek V4 route dispatch info ABI is unsupported");
  }
  if (prefix[2] != SLLM_HIP_DEEPSEEK_V4_MOE_ROUTE_DISPATCH_INFO_VERSION) {
    return sllm_public_runtime::write_error(
        sink, SLLM_STATUS_INVALID_ARGUMENT,
        "DeepSeek V4 route dispatch info version is unsupported");
  }
  if (info->reserved0 != 0U ||
      !all_zero(info->reserved, sizeof(info->reserved))) {
    return sllm_public_runtime::write_error(
        sink, SLLM_STATUS_RESERVED_NONZERO,
        "DeepSeek V4 route dispatch info reserved fields must be zero");
  }
  return SLLM_STATUS_OK;
}
...
extern "C" sllm_status_t sllm_deepseek_v4_moe_route_query(
    const sllm_deepseek_v4_moe_route_desc_t *const descriptor,
    sllm_deepseek_v4_moe_route_query_info_t *const info,
  ...
  try {
    ...
    const sllm_status_t info_status =
        sllm_deepseek_v4_moe_route::validate_query_info(info, error_sink);
    ...
    info->routed_scale = metadata.routed_scale;
    return SLLM_STATUS_OK;
  } catch (...) {
    return sllm_public_runtime::write_error(
        error_sink, SLLM_STATUS_INTERNAL_ERROR,
        "unexpected exception in DeepSeek V4 route query");
  }
}

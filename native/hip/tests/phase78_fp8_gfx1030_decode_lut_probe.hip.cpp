// Phase 78 standalone gfx1030 FP8 decode LUT probe.
//
// ID68 is the branchy E4M3FN -> FP16 dword8, wave4/col32 control.  The two
// candidates replace both activation and weight ingress with a 256-entry
// uint16 FP16-bit LUT: one reads a device constant table and the other loads
// a padded copy into LDS once per workgroup.  The accumulation, outer scales,
// and BF16-RNE epilogue are intentionally unchanged.
//
...
__device__ __constant__ uint16_t kDeviceFp16Lut[kLutEntries];
...
__device__ __forceinline__ uint8_t device_weight_code(const uint32_t copy,
  ...
  // Keep the code pattern independent of the conversion mechanism.
  constexpr uint8_t finite[16] = {0x00U, 0x01U, 0x08U, 0x10U, 0x18U, 0x20U,
                                  0x28U, 0x30U, 0x80U, 0x81U, 0x88U, 0x90U,
                                  0x98U, 0xa0U, 0xa8U, 0xb0U};
  return finite[(copy * 11U + column * 3U + inner * 7U + 9U) & 15U];
}

// One empty slot per 32-entry group keeps group boundaries from aliasing the
// same bank.  The table occupies 263 halfwords (526 bytes), below 1 KiB.
__device__ __forceinline__ uint32_t padded_lut_slot(const uint32_t code) {
  return code + (code >> 5U);
}

enum class KernelMode : uint32_t { Branchy, ConstantLut, LdsLut };

template <KernelMode Mode>
__device__ __forceinline__ uint16_t lut_value(const uint8_t code,
                                              const uint16_t *const lds) {
  if constexpr (Mode == KernelMode::ConstantLut)
    return kDeviceFp16Lut[code];
  return lds[padded_lut_slot(code)];
}

template <KernelMode Mode>
__device__ __forceinline__ uint32_t pair_fp16_bits(const uint32_t dword,
                                                   const uint16_t *const lds) {
  const uint8_t first = static_cast<uint8_t>(dword & 0xffU);
  const uint8_t second = static_cast<uint8_t>((dword >> 8U) & 0xffU);
  return static_cast<uint32_t>(lut_value<Mode>(first, lds)) |
         (static_cast<uint32_t>(lut_value<Mode>(second, lds)) << 16U);
}

template <KernelMode Mode>
__device__ __forceinline__ void
decode_body(const uint8_t *const activation,
            const float *const activation_scales, const uint8_t *const weight,
            const float *const weight_scales, uint16_t *const output,
...
#pragma unroll
    ...
    __half2 activation_first_low;
    __half2 activation_first_high;
    __half2 activation_second_low;
    __half2 activation_second_high;
    if constexpr (Mode == KernelMode::Branchy) {
      const sllm_lowp::E4M3FnFp16x4Bits first =
          sllm_lowp::e4m3fnx4_to_fp16x2_bits(activation_first);
      const sllm_lowp::E4M3FnFp16x4Bits second =
          sllm_lowp::e4m3fnx4_to_fp16x2_bits(activation_second);
      activation_first_low = half2_from_bits(first.low);
      activation_first_high = half2_from_bits(first.high);
      activation_second_low = half2_from_bits(second.low);
      activation_second_high = half2_from_bits(second.high);
    } else {
      activation_first_low =
          half2_from_bits(pair_fp16_bits<Mode>(activation_first, lds));
      activation_first_high =
          half2_from_bits(pair_fp16_bits<Mode>(activation_first >> 16U, lds));
      activation_second_low =
          half2_from_bits(pair_fp16_bits<Mode>(activation_second, lds));
      activation_second_high =
          half2_from_bits(pair_fp16_bits<Mode>(activation_second >> 16U, lds));
    }
#pragma unroll
    for (uint32_t local_column = 0U; local_column < columns_per_wave;
         ++local_column) {
      ...
      if (column >= n)
        continue;
      __half2 weight_first_low;
      __half2 weight_first_high;
      __half2 weight_second_low;
      __half2 weight_second_high;
      if constexpr (Mode == KernelMode::Branchy) {
        const sllm_lowp::E4M3FnFp16x4Bits first =
            sllm_lowp::e4m3fnx4_to_fp16x2_bits(weight_first[local_column]);
        const sllm_lowp::E4M3FnFp16x4Bits second =
            sllm_lowp::e4m3fnx4_to_fp16x2_bits(weight_second[local_column]);
        weight_first_low = half2_from_bits(first.low);
        weight_first_high = half2_from_bits(first.high);
        weight_second_low = half2_from_bits(second.low);
        weight_second_high = half2_from_bits(second.high);
      } else {
        weight_first_low = half2_from_bits(
            pair_fp16_bits<Mode>(weight_first[local_column], lds));
        weight_first_high = half2_from_bits(
            pair_fp16_bits<Mode>(weight_first[local_column] >> 16U, lds));
        weight_second_low = half2_from_bits(
            pair_fp16_bits<Mode>(weight_second[local_column], lds));
        weight_second_high = half2_from_bits(
            pair_fp16_bits<Mode>(weight_second[local_column] >> 16U, lds));
      }
...
template <KernelMode Mode>
__global__ __launch_bounds__(kThreads, 1) void branchy_or_constant_kernel(
    ...
    const uint64_t n) {
  decode_body<Mode>(activation, activation_scales, weight, weight_scales,
                    output, m, k, n, nullptr);
}

__global__ __launch_bounds__(kThreads, 1) void lds_lut_kernel(
    const uint8_t *const activation, const float *const activation_scales,
    const uint8_t *const weight, const float *const weight_scales,
    uint16_t *const output, const uint64_t m, const uint64_t k,
    const uint64_t n) {
  __shared__ uint16_t lut[272U];
  if (threadIdx.x < kLutEntries)
    lut[padded_lut_slot(threadIdx.x)] = kDeviceFp16Lut[threadIdx.x];
  __syncthreads();
  decode_body<KernelMode::LdsLut>(activation, activation_scales, weight,
                                  weight_scales, output, m, k, n, lut);
}

enum class CandidateId : uint32_t { Control68, ConstantLut, LdsLut };
  ...
  case CandidateId::Control68:
    return {id, "id68-dword8-wave4col32-control",
            reinterpret_cast<const void *>(
                branchy_or_constant_kernel<KernelMode::Branchy>)};
  case CandidateId::ConstantLut:
    return {id, "lut-constant-256x-u16",
            reinterpret_cast<const void *>(
                branchy_or_constant_kernel<KernelMode::ConstantLut>)};
  case CandidateId::LdsLut:
    return {id, "lut-lds-padded-256x-u16",
            reinterpret_cast<const void *>(lds_lut_kernel)};
  }
  return {CandidateId::Control68, "invalid", nullptr};
}
...
__global__ void fill_weight(uint8_t *const weight, const uint64_t k,
  ...
  if (index >= k * n)
    return;
  weight[index] = device_weight_code(copy, index / k, index % k);
}

__global__ void code_lut_oracle(const uint8_t *const input,
                                uint16_t *const output) {
  const uint32_t index = blockIdx.x * blockDim.x + threadIdx.x;
  if (index < kLutEntries)
    output[index] = kDeviceFp16Lut[input[index]];
}
...
bool launch(const Candidate &current, const ShapeCase &shape,
  ...
  switch (current.id) {
  case CandidateId::Control68:
    hipLaunchKernelGGL((branchy_or_constant_kernel<KernelMode::Branchy>), grid,
    ...
    break;
  case CandidateId::ConstantLut:
    hipLaunchKernelGGL((branchy_or_constant_kernel<KernelMode::ConstantLut>),
                       grid, block, 0U, buffers->stream, buffers->activation,
                       buffers->activation_scale, plane.weight, plane.scales,
                       buffers->output, 1U, shape.k, shape.n);
    break;
  case CandidateId::LdsLut:
    hipLaunchKernelGGL(lds_lut_kernel, grid, block, 0U, buffers->stream,
  ...
  for (int iteration = 0; iteration < kMeasured; ++iteration) {
    ...
    for (std::size_t position = 0U; position < ids.size(); ++position) {
      ...
      if (!hip_ok(hipEventRecord(buffers->start, buffers->stream),
                  "event start") ||
          !launch(candidate(ids[index]), shape, copy, buffers) ||
...
bool initialize_lut() {
  std::array<uint16_t, kLutEntries> host_lut{};
  for (uint32_t code = 0U; code < kLutEntries; ++code)
    host_lut[code] = sllm_lowp::e4m3fn_to_fp16_bits(static_cast<uint8_t>(code));
  return hip_ok(hipMemcpyToSymbol(HIP_SYMBOL(kDeviceFp16Lut), host_lut.data(),
                                  sizeof(host_lut)),
                "copy constant LUT");
}

bool all_codes_oracle() {
  std::array<uint8_t, kLutEntries> input{};
  std::array<uint16_t, kLutEntries> actual{};
  for (uint32_t i = 0U; i < kLutEntries; ++i)
  ...
  bool ok =
      ...
      hip_ok(hipMemcpy(device_input, input.data(), input.size(),
             ...
             "copy code input");
  if (ok) {
    hipLaunchKernelGGL(code_lut_oracle, dim3(1U), dim3(kLutEntries), 0U,
                       nullptr, device_input, device_output);
    ok = hip_ok(hipGetLastError(), "launch LUT oracle") &&
         hip_ok(hipDeviceSynchronize(), "sync LUT oracle") &&
  ...
  std::printf("oracle lut_codes=256 mismatches=%zu status=%s\n", mismatches,
...
void resources(const Candidate &current) {
  ...
  std::printf(
      "resources candidate=%s vgpr=%d sgpr=unavailable static_lds=%zu "
      "dynamic_lds=0 scratch=%zu active_blocks=%d attr=%s occupancy=%s\n",
      current.name, attributes.numRegs, attributes.sharedSizeBytes,
      attributes.localSizeBytes, active, hipGetErrorString(attr),
...
int main() {
  ...
  const std::vector<CandidateId> ids = {
      CandidateId::Control68, CandidateId::ConstantLut, CandidateId::LdsLut};

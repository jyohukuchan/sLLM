// Phase 87 Stage 10: Qwen3.5 MoE public Paged FP16 KV correctness.
//
// This opt-in test uses the additive public ABI with Qwen3.5 MoE full
// attention geometry [q_heads=16, kv_heads=2, head_dim=256]. It covers the
// 127/128/129 boundaries, M=1..5 decode, fork/COW/cancel, and an independent
// FP16 attention oracle.
...
int main() {
  ...
  for (const uint32_t query_heads : {16U}) {
  ...
  if (passed) {
    std::cout
        << "phase87_stage10_qwen35_moe_paged_fp16_public_gpu_test: PASS target="
        << SLLM_TEST_EXPECTED_TARGET
        << " cases=7 geometry=qwen35_moe[16,2,256] "
           "boundaries=127,128,129 decode_m=1..5 prefill=1 "
           "fork_cow=1 cancel=1 oracle=independent-fp16-attention "

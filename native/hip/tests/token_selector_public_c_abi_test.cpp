...
bool verify_rng_contract() {
  ...
  if (sllm_token_selector_kernel::launch(logits, additive, mask, 3U, 1.0F, 7U,
                                         0U, &first, nullptr) != hipSuccess) {
    return false;
  }
  if (sllm_token_selector_kernel::launch(logits, additive, mask, 3U, 1.0F, 7U,
                                         1U, &second, nullptr) != hipSuccess) {
    return false;
  }
...
bool prepare_invalid(const sllm_context_t *const context,
  ...
  Error error;
  const bool ok = expect(sllm_token_selector_prepare(context, &descriptor,
                                                     &plan, &error.sink),
...
bool run_selector(const sllm_context_t *const context,
  ...
  sllm_buffer_create_info_t info{
      sizeof(info), SLLM_HIP_ABI_VERSION, sizeof(logits_host), 0U,
      0U,           {0U, 0U, 0U, 0U, 0U}};
  ...
  ok = ok && record.status == SLLM_STATUS_OK && record.token_id >= 0 &&
       ...
       dispatch.dispatch_id != 0U && dispatch.dispatch_count == 1U &&
       dispatch.fallback_allowed == 0U && dispatch.fallback_used == 0U &&
       std::strcmp(dispatch.kernel_symbol, "token_selector.bf16_f32_mask.v1") ==
           0 &&
       std::strcmp(dispatch.device_symbol,
                   "sllm_token_selector_bf16_f32_mask_v1") == 0;
  if (plan != nullptr) {

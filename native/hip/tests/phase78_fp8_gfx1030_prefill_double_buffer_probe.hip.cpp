// Phase 78 standalone gfx1030 outer-vector FP8 prefill double-buffer probe.
//
// The control is the production ID71 64x64/K32 single-buffer schedule.  The
// candidates apply the Phase 75 ID55 load-next/compute-current schedule to
// 128x64 and 64x64 output tiles.  Every path keeps the numerical contract
// unchanged: OCP E4M3FN bytes are expanded exactly to FP16, half2 products are
// accumulated in FP32 in increasing-K order, F32 row/column outer scales are
// applied once in the epilogue, and output is rounded to BF16 RNE.
//
      ...
      for (uint32_t lane = 0U; lane < values_per_load; ++lane) {
        destination[lane] =
            source_row < m && source_inner + lane < k
                ? sllm_lowp::e4m3fn_to_fp16_bits_no_table(
...
enum class CandidateId : uint32_t { Id71, Double128x64, Double64x64 };
...
Candidate candidate(const CandidateId id) {
  switch (id) {
  case CandidateId::Id71:
    return {id, "id71-control-64x64-k32-single", 64U, 64U,
            reinterpret_cast<const void *>(fp8_outer_single_kernel<64U, 64U>)};
  case CandidateId::Double128x64:
    return {id, "candidate-128x64-k32-double", 128U, 64U,
            reinterpret_cast<const void *>(fp8_outer_double_kernel<128U, 64U>)};
  case CandidateId::Double64x64:
    return {id, "candidate-64x64-k32-double", 64U, 64U,
            reinterpret_cast<const void *>(fp8_outer_double_kernel<64U, 64U>)};
  }
  return {id, "invalid", 0U, 0U, nullptr};
}

constexpr std::array<CandidateId, 3> kCandidateIds = {
    CandidateId::Id71, CandidateId::Double128x64, CandidateId::Double64x64};
...
bool launch(const Candidate current, const Shape shape,
  ...
  switch (current.id) {
  case CandidateId::Id71:
    hipLaunchKernelGGL((fp8_outer_single_kernel<64U, 64U>),
    ...
    break;
  case CandidateId::Double128x64:
    hipLaunchKernelGGL((fp8_outer_double_kernel<128U, 64U>),
                       dim3(static_cast<uint32_t>(block_count)), dim3(kThreads),
                       0U, buffers->stream, buffers->activation,
                       buffers->activation_scales, buffers->weight,
                       buffers->weight_scales, buffers->output, shape.m,
    ...
    break;
  case CandidateId::Double64x64:
    hipLaunchKernelGGL((fp8_outer_double_kernel<64U, 64U>),
                       dim3(static_cast<uint32_t>(block_count)), dim3(kThreads),
                       0U, buffers->stream, buffers->activation,
                       buffers->activation_scales, buffers->weight,
                       buffers->weight_scales, buffers->output, shape.m,
...
bool measure(const Candidate current, const Shape shape,
             DeviceBuffers *const buffers, float *const median_us,
             float *const min_us, float *const max_us) {
...
int main(int argc, char **argv) {
  ...
  if (argc > 2 || (argc == 2 && !parse_device(argv[1], &device))) {
    std::fprintf(stderr,
                 "usage: phase78_fp8_gfx1030_prefill_double_buffer_probe "
                 "[DEVICE]\n");
    return EXIT_FAILURE;
  }
  ...
  for (const CandidateId id : kCandidateIds) {
    print_resources(candidate(id));
  }

  constexpr std::array<Shape, 6> production_shapes = {
      Shape{128U, 5120U, 17408U, "m128-wide"},
      Shape{128U, 17408U, 5120U, "m128-down"},
      Shape{512U, 5120U, 17408U, "m512-wide"},
      Shape{512U, 17408U, 5120U, "m512-down"},
      Shape{1024U, 5120U, 17408U, "m1024-wide"},
      Shape{1024U, 17408U, 5120U, "m1024-down"}};

  for (const Shape shape : production_shapes) {
    ...
    std::array<float, kCandidateIds.size()> medians{};
    for (size_t candidate_index = 0U; candidate_index < kCandidateIds.size();
         ++candidate_index) {
      const Candidate current = candidate(kCandidateIds[candidate_index]);
      float minimum = 0.0F;
      float median = 0.0F;
      float maximum = 0.0F;
      if (!measure(current, shape, &buffers, &median, &minimum, &maximum)) {
        free_buffers(&buffers);
        return EXIT_FAILURE;
      }
      medians[candidate_index] = median;
      const double tflops =
          (2.0 * static_cast<double>(shape.m) * static_cast<double>(shape.k) *
           static_cast<double>(shape.n)) /
          (static_cast<double>(median) * 1.0e6);
      std::printf(
          "result candidate=%s shape=%s m=%llu k=%llu n=%llu min_us=%.3f "
          "median_us=%.3f max_us=%.3f tflops=%.6f\n",
          current.name, shape.name, static_cast<unsigned long long>(shape.m),
          static_cast<unsigned long long>(shape.k),
          static_cast<unsigned long long>(shape.n), minimum, median, maximum,
          tflops);

      std::vector<uint16_t> first;
      std::vector<uint16_t> second;
      if (!capture(current, shape, &buffers, &first) ||
          !capture(current, shape, &buffers, &second)) {
        free_buffers(&buffers);
        return EXIT_FAILURE;
      }
      all_ok = compare_outputs("determinism", shape, first, second) && all_ok;
      all_ok = check_oracle(current, shape, first, false) && all_ok;
      if (current.id == CandidateId::Id71) {
        control_output = std::move(first);
      } else {
        all_ok =
            compare_outputs("bitwise-vs-id71", shape, control_output, first) &&
            all_ok;
      }
    }
    std::printf(
        "speedup shape=%s double128_vs_id71=%.6f double64_vs_id71=%.6f\n",
        shape.name, medians[0] / medians[1], medians[0] / medians[2]);
    free_buffers(&buffers);
  }

  // Exercise both sides of the K32, M64/M128, and N64 tile boundaries.
  constexpr std::array<Shape, 4> boundary_shapes = {
      Shape{63U, 31U, 63U, "below-m64-k32-n64"},
      Shape{65U, 33U, 65U, "above-m64-k32-n64"},
      Shape{127U, 31U, 63U, "below-m128-k32"},
      Shape{129U, 33U, 65U, "above-m128-k32"}};
  for (const Shape shape : boundary_shapes) {
    ...
    for (const CandidateId id : kCandidateIds) {
      ...
      all_ok = compare_outputs("boundary-determinism", shape, first, second) &&
               all_ok;
      all_ok = check_oracle(current, shape, first, true) && all_ok;
      if (id == CandidateId::Id71) {
        control_output = std::move(first);
      } else {
        all_ok = compare_outputs("boundary-bitwise-vs-id71", shape,
                                 control_output, first) &&
                 all_ok;
      }
    }
    free_buffers(&buffers);
  ...
  std::printf("cleanup free_bytes=%zu total_bytes=%zu status=%s\n"
              "summary status=%s candidates=%zu production_shapes=%zu "
              "boundary_shapes=%zu warmups=%d measured=%d\n",
              free_bytes, total_bytes, memory_ok ? "PASS" : "FAIL",
              all_ok ? "PASS" : "FAIL", kCandidateIds.size(),
              production_shapes.size(), boundary_shapes.size(), kWarmups,
              kMeasured);

// Phase 87 Stage 10: Ministral 3 public Paged FP16 KV request smoke.
//
// The official lock fixes Ministral 3 at [query heads=32, KV heads=8,
// head_dim=128]. This test exercises that exact geometry through the public
// additive Paged ABI, including the 128-token boundary, decode M=1..5,
// append-to-attention chaining, fork/COW, cancel, and cleanup.
...
constexpr uint32_t kQueryHeads = 32U;
constexpr uint32_t kHeadDim = 128U;
constexpr uint32_t kTokenBlock = 128U;
constexpr uint64_t kCapacity = 768U;
constexpr uint64_t kLogicalBlocks = 6U;
constexpr uint64_t kMaxPhysicalBlocks = 12U;
...
double oracle_value(const std::vector<uint16_t> &keys,
  ...
  for (uint64_t token = 0U; token != length; ++token) {
    ...
    scores[static_cast<std::size_t>(token)] =
        score / std::sqrt(static_cast<double>(kHeadDim));
...
bool run_attention(const sllm_context_t *const context,
  ...
  const uint32_t expected_kernel =
      paged ? SLLM_HIP_CAUSAL_ATTENTION_KERNEL_ID_PAGED_GENERIC_FORMATS_V1 : 0U;
  if (!expect(status, SLLM_STATUS_OK,
    ...
    std::cerr << "attention did not report exact HIP/no-fallback provider"
              << " status=" << status << " backend=" << dispatch.backend
              << " kernel=" << dispatch.kernel_id
              << " query_count=" << dispatch.query_count
              << " q_heads=" << dispatch.q_heads
              << " kv_heads=" << dispatch.kv_heads
...
bool run_case(const sllm_context_t *const context,
  ...
  const std::array<uint32_t, 4> oracle_dimensions{0U, 31U, 64U, 127U};
  ...
  const uint64_t expected_shared_blocks = prefix_length / kTokenBlock;
...
uint16_t entropy_bf16(const bool key, const uint64_t token, const uint32_t head,
                      const uint32_t dimension) {
  const uint32_t exponent =
      122U + static_cast<uint32_t>((token * (key ? 7U : 11U) +
                                    head * (key ? 13U : 5U) +
                                    dimension * (key ? 3U : 17U)) %
                                   (key ? 9U : 8U));
  const uint32_t mantissa =
      static_cast<uint32_t>((token * 29U + head * 47U +
                             dimension * (key ? 19U : 23U) + (key ? 3U : 11U)) &
                            0x7fU);
  const bool negative =
      ((token + head * (key ? 5U : 7U) + dimension * (key ? 3U : 11U)) & 1U) !=
      0U;
  return static_cast<uint16_t>((negative ? 0x8000U : 0U) | (exponent << 7U) |
                               mantissa);
}

std::vector<uint16_t> make_entropy_kv(const bool key) {
  std::vector<uint16_t> result(static_cast<std::size_t>(kCapacity) * kKvHeads *
                               kHeadDim);
  for (uint64_t token = 0U; token != kCapacity; ++token) {
    for (uint32_t head = 0U; head != kKvHeads; ++head) {
      for (uint32_t dimension = 0U; dimension != kHeadDim; ++dimension) {
        result[(static_cast<std::size_t>(token) * kKvHeads + head) * kHeadDim +
               dimension] = entropy_bf16(key, token, head, dimension);
      }
    }
  }
  return result;
...
std::vector<uint16_t> make_entropy_queries() {
  std::vector<uint16_t> result(static_cast<std::size_t>(kCapacity) *
                               kQueryHeads * kHeadDim);
  for (uint64_t row = 0U; row != kCapacity; ++row) {
    for (uint32_t head = 0U; head != kQueryHeads; ++head) {
      for (uint32_t dimension = 0U; dimension != kHeadDim; ++dimension) {
        result[(static_cast<std::size_t>(row) * kQueryHeads + head) * kHeadDim +
               dimension] = entropy_bf16(false, row + 17U, head, dimension);
      }
    }
  }
  return result;
...
int main() {
  ...
  for (const uint64_t prefix : {UINT64_C(127), UINT64_C(128), UINT64_C(129)}) {
    if (passed &&
        !run_case(context, queue, prefix, 1U, keys, values, queries)) {
      std::cerr << "Ministral public paged FP16 request failed prefix="
                << prefix << '\n';
      passed = false;
    }
  }
  ...
  if (passed)
    passed = run_case(context, queue, 543U, 1U, keys, values, queries);
  if (passed) {
    const std::vector<uint16_t> entropy_keys = make_entropy_kv(true);
    const std::vector<uint16_t> entropy_values = make_entropy_kv(false);
    const std::vector<uint16_t> entropy_queries = make_entropy_queries();
    passed = run_case(context, queue, 543U, 1U, entropy_keys, entropy_values,
                      entropy_queries);
  }
  ...
  if (passed) {
    std::cout
        << "phase87_stage10_ministral3_paged_fp16_public_gpu_test: PASS target="
        << SLLM_TEST_EXPECTED_TARGET
        << " cases=9 geometry=ministral3[32,8,128] "

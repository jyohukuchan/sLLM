// Phase 87 stage 7 probe: residual RMSNorm producer fusion for FP8 / NVFP4
// activation quantization.
//
// The control is the decomposed chain (production RMSNorm followed by the
// production activation quantizer). The candidate is the fused producer. The
// probe fails closed on any byte difference between the two, then times both
// with an AB/BA protocol.
//
...
//     -I native/hip/src \
//     -c native/hip/tests/phase87_s7_rmsnorm_probe.hip.cpp \
//     -o phase87-s7-rmsnorm-gfx1030.o
//   amdclang++ -O3 -ffp-contract=off -DNDEBUG --offload-arch=gfx1030 \
//     -mcode-object-version=6 -mno-wavefrontsize64 --hip-link \
//     phase87-s7-rmsnorm-gfx1030.o /path/to/libsllm_lowp.a \
//     -L"${ROCM_PATH}/lib" -lamdhip64 -o phase87-s7-rmsnorm-gfx1030
//
// Usage: phase87-s7-rmsnorm-gfx1030 --target=gfx1030 \
//   [--warmup-ms=300] [--samples=9] [--reps=100]
//
// Scope notes:
...
//   * The fused candidates group the row by 32 lanes, matching
//     `residual_rmsnorm_body<32,8>`. A `SLLM_HIP_COMPILE_WAVE64=1` control
//     groups by 64, so `inverse_rms` may differ by an ULP; production wiring
//     must keep these variants off under that build.
...
// squared sums stay finite; "specials" also injects zeros, signed zeros,
// infinities, NaNs and subnormals; "zeros" exercises the amax==0 branch.
...
constexpr Shape kShapes[] = {
    {1U, UINT64_C(5120), "m1k5120"},   {3U, UINT64_C(5120), "m3k5120"},
    {17U, UINT64_C(5120), "m17k5120"}, {4U, UINT64_C(5120), "m4k5120"},
    {1U, UINT64_C(5119), "m1k5119"},   {1U, UINT64_C(5121), "m1k5121"},
    {1U, UINT64_C(31), "m1k31"},       {1U, UINT64_C(32), "m1k32"},
    {1U, UINT64_C(33), "m1k33"},       {1U, UINT64_C(17), "m1k17"},
    {2U, UINT64_C(17408), "m2k17408"}, {1U, UINT64_C(6144), "m1k6144"},
};

constexpr Shape kTimedShapes[] = {
    {1U, UINT64_C(5120), "m1k5120"},
    {3U, UINT64_C(5120), "m3k5120"},
    {1U, UINT64_C(17408), "m1k17408"},
    {2U, UINT64_C(17408), "m2k17408"},
};
...
int main(const int argc, char **const argv) {
  ...
  const uint32_t scale_mode = SLLM_RMSNORM_SCALE_MODE_OFFSET_ONE;
  ...
  for (const Shape &shape : shapes) {
    ...
    for (const std::string &pattern : patterns) {
      DeviceBuffer<uint16_t> residual(row_elements);
      DeviceBuffer<uint16_t> addend(row_elements);
      DeviceBuffer<uint16_t> raw_scale(static_cast<size_t>(k));
      DeviceBuffer<uint16_t> control_residual_output(row_elements);
      DeviceBuffer<uint16_t> fused_residual_output(row_elements);
      ...
      fill_bf16(host.data(), host.size(), pattern, UINT32_C(0x1234), 119U,
                UINT32_C(16));
      upload(residual.get(), host);
      fill_bf16(host.data(), host.size(), pattern, UINT32_C(0x5678), 119U,
                UINT32_C(16));
      upload(addend.get(), host);
      host.resize(static_cast<size_t>(k));
      fill_bf16(host.data(), host.size(), pattern, UINT32_C(0x9abc), 104U,
                UINT32_C(32));
      upload(raw_scale.get(), host);

      std::vector<uint16_t> host_control(row_elements);
      std::vector<uint16_t> host_fused(row_elements);
      ...
      S7_HIP_CHECK(sllm_rmsnorm_kernel::launch_residual_fused(
          residual.get(), addend.get(), raw_scale.get(),
          control_residual_output.get(), control_bf16_output.get(), k, m,
          epsilon, scale_mode, nullptr));
      S7_HIP_CHECK(sllm_matmul_kernel::launch_fp8_quantize(
          control_bf16_output.get(), control_codes.get(), control_scales.get(),
          m, k, false, nullptr));
      S7_HIP_CHECK(sllm_rmsnorm_kernel::launch_residual_prequant_fp8(
          residual.get(), addend.get(), raw_scale.get(),
          fused_residual_output.get(), fused_codes.get(), fused_scales.get(), k,
          m, epsilon, scale_mode, 0U, nullptr));
      S7_HIP_CHECK(hipDeviceSynchronize());

      S7_HIP_CHECK(hipMemcpy(host_control.data(), control_residual_output.get(),
                             row_elements * sizeof(uint16_t),
                             hipMemcpyDeviceToHost));
      S7_HIP_CHECK(hipMemcpy(host_fused.data(), fused_residual_output.get(),
                             row_elements * sizeof(uint16_t),
                             hipMemcpyDeviceToHost));
      const bool residual_match =
          std::memcmp(host_control.data(), host_fused.data(),
                      row_elements * sizeof(uint16_t)) == 0;
      ...
      S7_HIP_CHECK(hipMemcpy(host_control_codes.data(), control_codes.get(),
                             host_control_codes.size(), hipMemcpyDeviceToHost));
      S7_HIP_CHECK(hipMemcpy(host_fused_codes.data(), fused_codes.get(),
                             host_fused_codes.size(), hipMemcpyDeviceToHost));
      const size_t code_diff = byte_diff(host_control_codes, host_fused_codes);
      ...
      for (size_t index = 0; index < m; ++index) {
        if (std::memcmp(&host_control_scales[index], &host_fused_scales[index],
                        sizeof(float)) != 0) {
          ++scale_diff;
        }
      }

      const bool fp8_ok = residual_match && code_diff == 0U && scale_diff == 0U;
      if (!fp8_ok) {
        ++failures;
      }
      ...
      for (const float global : global_candidates) {
        ...
        S7_HIP_CHECK(sllm_rmsnorm_kernel::launch_residual_prequant_nvfp4(
            residual.get(), addend.get(), raw_scale.get(),
            fused_residual_output.get(), fused_packed.get(), fused_blocks.get(),
            global_scale, k, m, epsilon, scale_mode, nullptr));
        S7_HIP_CHECK(hipDeviceSynchronize());

        S7_HIP_CHECK(
            hipMemcpy(host_control.data(), control_residual_output.get(),
                      row_elements * sizeof(uint16_t), hipMemcpyDeviceToHost));
        S7_HIP_CHECK(hipMemcpy(host_fused.data(), fused_residual_output.get(),
                               row_elements * sizeof(uint16_t),
                               hipMemcpyDeviceToHost));
        const bool nv_residual_match =
            std::memcmp(host_control.data(), host_fused.data(),
                        row_elements * sizeof(uint16_t)) == 0;
        ...
        std::vector<uint8_t> host_fused_packed(
            static_cast<size_t>(nvfp4_packed_bytes(m, k)));
        S7_HIP_CHECK(hipMemcpy(host_control_packed.data(), control_packed.get(),
                               host_control_packed.size(),
                               hipMemcpyDeviceToHost));
        ...
        std::vector<uint8_t> host_fused_blocks(
            static_cast<size_t>(nvfp4_block_bytes(m, k)));
        S7_HIP_CHECK(hipMemcpy(host_control_blocks.data(), control_blocks.get(),
                               host_control_blocks.size(),
                               hipMemcpyDeviceToHost));
        ...
        const bool nvfp4_ok =
            nv_residual_match && packed_diff == 0U && block_diff == 0U;
        if (!nvfp4_ok) {
          ++failures;
        }
        emit("{\"kind\":\"correctness\",\"format\":\"nvfp4\",\"shape\":\"" +
             ...
             ",\"residual_match\":" + (nv_residual_match ? "true" : "false") +
  ...
  for (const Shape &shape : timed_shapes) {
    ...
    DeviceBuffer<uint16_t> addend(row_elements);
    DeviceBuffer<uint16_t> raw_scale(static_cast<size_t>(k));
    DeviceBuffer<uint16_t> residual_output(row_elements);
    ...
    upload(addend.get(), host);
    host.resize(static_cast<size_t>(k));
    fill_bf16(host.data(), host.size(), "normal", UINT32_C(0x1357), 104U,
              UINT32_C(32));
    upload(raw_scale.get(), host);
    ...
    // producer time plus quantizer time.
    {
      auto producer = [&]() {
        S7_HIP_CHECK(sllm_rmsnorm_kernel::launch_residual_fused(
            residual.get(), addend.get(), raw_scale.get(),
            residual_output.get(), bf16_output.get(), k, m, epsilon, scale_mode,
            stream));
      };
    ...
    for (int variant = 0; variant < 2; ++variant) {
      ...
      auto fused = [&]() {
        if (is_fp8) {
          S7_HIP_CHECK(sllm_rmsnorm_kernel::launch_residual_prequant_fp8(
              residual.get(), addend.get(), raw_scale.get(),
              residual_output.get(), codes.get(), scales.get(), k, m, epsilon,
              scale_mode, 0U, stream));
        } else {
          S7_HIP_CHECK(sllm_rmsnorm_kernel::launch_residual_prequant_nvfp4(
              residual.get(), addend.get(), raw_scale.get(),
              residual_output.get(), packed.get(), blocks.get(), global_scale,
              k, m, epsilon, scale_mode, stream));
        }
      };
    ...
    // The production shape runs inside one HIP graph. Only a graph launch
    // exposes the inter-kernel gap that removing a node saves, so the eager
    // numbers above understate the real shortening. Capture `reps` copies of
    // each chain so a single launch measures the whole batch and host launch
    // overhead cancels between the two.
    for (int variant = 0; variant < 2; ++variant) {
      const bool is_fp8 = variant == 0;
      const std::string format = is_fp8 ? "fp8" : "nvfp4";
      auto instantiate = [&](auto &&body) {
        hipGraph_t graph = nullptr;
        S7_HIP_CHECK(
            hipStreamBeginCapture(stream, hipStreamCaptureModeThreadLocal));
        for (int copy = 0; copy < reps; ++copy) {
          body();
        }
        S7_HIP_CHECK(hipStreamEndCapture(stream, &graph));
        hipGraphExec_t exec = nullptr;
        S7_HIP_CHECK(hipGraphInstantiate(&exec, graph, nullptr, nullptr, 0));
        S7_HIP_CHECK(hipGraphDestroy(graph));
        return exec;
      };
      const hipGraphExec_t control_graph = instantiate([&]() {
        S7_HIP_CHECK(sllm_rmsnorm_kernel::launch_residual_fused(
            residual.get(), addend.get(), raw_scale.get(),
            residual_output.get(), bf16_output.get(), k, m, epsilon, scale_mode,
            stream));
      ...
      const hipGraphExec_t fused_graph = instantiate([&]() {
        if (is_fp8) {
          S7_HIP_CHECK(sllm_rmsnorm_kernel::launch_residual_prequant_fp8(
              residual.get(), addend.get(), raw_scale.get(),
              residual_output.get(), codes.get(), scales.get(), k, m, epsilon,
      ...
      auto measure = [&](const hipGraphExec_t exec) {
        const auto warm_start = std::chrono::steady_clock::now();
        while (std::chrono::duration_cast<std::chrono::milliseconds>(
                   std::chrono::steady_clock::now() - warm_start)
                   .count() < warmup_ms) {
          S7_HIP_CHECK(hipGraphLaunch(exec, stream));
        }
        S7_HIP_CHECK(hipStreamSynchronize(stream));
        ...
        for (int sample = 0; sample < samples; ++sample) {
          ...
          float elapsed = 0.0F;
          S7_HIP_CHECK(hipEventElapsedTime(&elapsed, start, stop));
          values.push_back(static_cast<double>(elapsed) / reps);
          S7_HIP_CHECK(hipEventDestroy(start));
          S7_HIP_CHECK(hipEventDestroy(stop));
        }
      ...
      double graph_control_ms = 0.0;
      double graph_fused_ms = 0.0;
      for (int order = 0; order < 2; ++order) {
        const double first = measure(order == 0 ? control_graph : fused_graph);
        const double second = measure(order == 0 ? fused_graph : control_graph);
        graph_control_ms += order == 0 ? first : second;
        graph_fused_ms += order == 0 ? second : first;
      }
      graph_control_ms *= 0.5;
      graph_fused_ms *= 0.5;
      S7_HIP_CHECK(hipGraphExecDestroy(control_graph));
      S7_HIP_CHECK(hipGraphExecDestroy(fused_graph));
      emit("{\"kind\":\"timing\",\"space\":\"graph\",\"format\":\"" + format +
           "\",\"shape\":\"" + std::string(shape.name) +
           "\",\"control_ms\":" + std::to_string(graph_control_ms) +
           ",\"fused_ms\":" + std::to_string(graph_fused_ms) +
           ",\"delta_ms\":" +
           std::to_string(graph_control_ms - graph_fused_ms) + ",\"speedup\":" +
           std::to_string(graph_control_ms /
                          (graph_fused_ms > 1.0e-9 ? graph_fused_ms : 1.0e-9)) +
           "}");
    }
  }

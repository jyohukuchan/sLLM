...
// Phase 78 ID84 K-loop prefetch probe for the production gfx1201 path.
//
// The control is the current production ID84 wrapper itself, included from
// matmul_kernel.hip.cpp.  The two private candidates prefetch P=2 or P=4
// ID67 block iterations before doing the same block16 DP4A and FP32 update.
...
//
// The run is intended for --target gfx1201 (R9700).  The tiny K48/N37 case and
// both artifact K/N orientations exercise the production ID84/ID67 wrapper.
...
// Compile the modified include's ID67 body under a probe-only symbol.  The
// archive contributes the production symbol used by launch_control; this
// wrapper lets the same executable compare that old archive body with the
// post-change inc body without colliding device symbols.
extern "C" __global__ __launch_bounds__(256, 1) void sllm_phase78_id84_inc_p4(
  ...
  sllm_id84_nvfp4_scale_lut_detail::sllm_id84_nvfp4_id67_body(
      packed_activation, activation_scales, packed_weight, weight_scales,
      weight_tensor_scale, input_tensor_scale, output, m, k, n, scale_lut);
}
...
// launch_nvfp4_w4a4 in the handoff archive uses (n + 31) / 32 for ID84,
// even though each ID67 wave group covers 128 columns.  Match that launcher
// grid for both private paths so the A/B timing includes identical idle
// workgroups and differs only in the body.
constexpr uint32_t kArchiveColumnsPerWorkgroup = 32U;
...
// Keep only raw source words in the lookahead window.  Decoding them after
// the window is loaded limits the extra state to 15 scalar words per block
// (two activation words, eight weight words, and five scale bytes), instead
// of retaining decoded packs.  P=2/P=4 are the only variants measured here.
struct sllm_phase78_id84_prefetch_raw_block final {
...
__device__ __forceinline__ void sllm_phase78_id84_prefetch_load(
    const uint8_t *const packed_activation,
    const uint8_t *const activation_scales, const uint8_t *const packed_weight,
    const uint8_t *const weight_scales, const uint64_t blocks_per_row,
    const uint64_t column_base, const uint64_t n, const uint64_t block,
    sllm_phase78_id84_prefetch_raw_block *const result) {
  (void)PrefetchIterations;
  *result = {};
...
template <uint32_t PrefetchIterations>
__device__ __forceinline__ void sllm_phase78_id84_prefetch_body(
    const uint8_t *const packed_activation,
    const uint8_t *const activation_scales, const uint8_t *const packed_weight,
    const uint8_t *const weight_scales, const float *const weight_tensor_scale,
    const float *const input_tensor_scale, uint16_t *const output,
  ...
  // Each lane owns the same block sequence as production ID84.  Grouping P
  // consecutive lane iterations moves their global loads above the compute
  // for the group without changing the per-lane FP32 accumulation order.
  for (uint64_t group = lane; group < blocks_per_row;
       group += kWave * PrefetchIterations) {
    sllm_phase78_id84_prefetch_raw_block prefetched[PrefetchIterations];
#pragma unroll
    for (uint32_t lookahead = 0U; lookahead < PrefetchIterations; ++lookahead) {
      sllm_phase78_id84_prefetch_load<PrefetchIterations>(
          packed_activation, activation_scales, packed_weight, weight_scales,
          blocks_per_row, column_base, n,
    ...
    for (uint32_t lookahead = 0U; lookahead < PrefetchIterations; ++lookahead) {
      ...
      const sllm_id84_nvfp4_scale_lut_detail::ScaledPacks activation_pack0 =
          sllm_id84_nvfp4_scale_lut_detail::sllm_id84_nvfp4_scaled_packs(
              prefetched[lookahead].activation_word0);
      const sllm_id84_nvfp4_scale_lut_detail::ScaledPacks activation_pack1 =
          sllm_id84_nvfp4_scale_lut_detail::sllm_id84_nvfp4_scaled_packs(
              prefetched[lookahead].activation_word1);
...
extern "C" __global__
__launch_bounds__(256, 1) void sllm_phase78_id84_prefetch_p2(
    const uint8_t *const packed_activation,
    const uint8_t *const activation_scales, const uint8_t *const packed_weight,
    const uint8_t *const weight_scales, const float *const weight_tensor_scale,
    const float *const input_tensor_scale, uint16_t *const output,
  ...
  sllm_id84_nvfp4_scale_lut_detail::sllm_id84_nvfp4_populate_constant_lut(
      scale_lut);
  sllm_phase78_id84_prefetch_body<2U>(
      packed_activation, activation_scales, packed_weight, weight_scales,
      weight_tensor_scale, input_tensor_scale, output, m, k, n, scale_lut);
}

extern "C" __global__
__launch_bounds__(256, 1) void sllm_phase78_id84_prefetch_p4(
    const uint8_t *const packed_activation,
    const uint8_t *const activation_scales, const uint8_t *const packed_weight,
    const uint8_t *const weight_scales, const float *const weight_tensor_scale,
    const float *const input_tensor_scale, uint16_t *const output,
  ...
  sllm_id84_nvfp4_scale_lut_detail::sllm_id84_nvfp4_populate_constant_lut(
      scale_lut);
  sllm_phase78_id84_prefetch_body<4U>(
      packed_activation, activation_scales, packed_weight, weight_scales,
      weight_tensor_scale, input_tensor_scale, output, m, k, n, scale_lut);
}
...
bool launch_control(const Shape &shape, const bool activation_shared,
  ...
  // Its gfx1201 selector launches the production ID84/ID67 body and performs
...
bool launch_candidate(const Shape &shape, const bool activation_shared,
                      Buffers *const buffers) {
  (void)activation_shared;
  const uint64_t grid_u64 = (shape.n + kArchiveColumnsPerWorkgroup - 1U) /
                            kArchiveColumnsPerWorkgroup;
  if (grid_u64 == 0U || grid_u64 > UINT32_MAX)
    return false;
  const dim3 grid(static_cast<uint32_t>(grid_u64));
  const dim3 block(kThreads);
  hipLaunchKernelGGL(
      sllm_phase78_id84_prefetch_p2, grid, block, 0U, buffers->stream,
...
bool launch_constant_candidate(const Shape &shape, const bool activation_shared,
  ...
  const dim3 grid(static_cast<uint32_t>(grid_u64));
  const dim3 block(kThreads);
  hipLaunchKernelGGL(sllm_phase78_id84_inc_p4, grid, block, 0U, buffers->stream,
                     buffers->activation, buffers->activation_scales,
                     buffers->weight, buffers->weight_scales,
                     buffers->weight_tensor_scale, buffers->input_tensor_scale,
                     buffers->output, 1U, shape.k, shape.n);

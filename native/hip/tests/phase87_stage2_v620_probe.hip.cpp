// Phase 87 WU-2V/C1 bounded probe for exact gfx1030, M=1, K=6144, N=5120.
//
// The control is the production ID82 public launcher from the linked lowp
// archive.  The candidate is test-only and differs only by explicitly
// scalarizing the wave-uniform first weight-column address with readfirstlane.
...
// Candidate copy of the current production ID82 Rows=1 body.  The only
// intentional change is the scalarized first address of the four weight
// columns belonging to each wave.
...
candidate_body(const uint8_t *const activation,
  ...
  // column_base is wave-uniform.  Read both address halves from lane zero so
  // the four subsequent column pointers can be formed from SGPR state.
  const uint64_t weight_address =
      reinterpret_cast<uintptr_t>(weight) + column_base * TupleK;
  const uint32_t weight_low =
      __builtin_amdgcn_readfirstlane(static_cast<uint32_t>(weight_address));
  const uint32_t weight_high = __builtin_amdgcn_readfirstlane(
      static_cast<uint32_t>(weight_address >> 32U));
  const uint64_t weight_base_address =
      (static_cast<uint64_t>(weight_high) << 32U) | weight_low;
  const auto *const weight_base =
      reinterpret_cast<const uint8_t *>(weight_base_address);
...
#pragma unroll 1
  for (uint32_t group = 0U; group < TupleGroups; ++group) {
    ...
    uint32_t activation_first[kPrefetchChunks];
    uint32_t activation_second[kPrefetchChunks];
...
#pragma unroll
      for (uint32_t local_column = 0U; local_column < kColumnsPerWave;
           ++local_column) {
        const auto *const column_dwords = reinterpret_cast<const uint32_t *>(
            weight_base + static_cast<uint64_t>(local_column) * TupleK);
...
#pragma unroll
    for (uint32_t chunk = 0U; chunk < kPrefetchChunks; ++chunk) {
      const __half2 activation_first_low =
          packed_half2(lut_pair(activation_first[chunk], lut));
      const __half2 activation_first_high =
          packed_half2(lut_pair(activation_first[chunk] >> 16U, lut));
      const __half2 activation_second_low =
          packed_half2(lut_pair(activation_second[chunk], lut));
      const __half2 activation_second_high =
          packed_half2(lut_pair(activation_second[chunk] >> 16U, lut));
#pragma unroll
      for (uint32_t local_column = 0U; local_column < kColumnsPerWave;
           ++local_column) {
        const __half2 weight_first_low =
            packed_half2(lut_pair(weight_first[chunk][local_column], lut));
        const __half2 weight_first_high = packed_half2(
        ...
        const __half2 weight_second_high = packed_half2(
...
int main(int argc, char **argv) {
  try {
    ...
    std::cout << "{\"kind\":\"identity\",\"state\":\""
              ...
              << "\",\"m\":1,\"k\":" << kK << ",\"n\":" << kN
              << ",\"pattern\":" << pattern
              << ",\"candidate\":\"wave_uniform_weight_base_readfirstlane\""

// Phase 78 standalone GQA6 rocBLAS attention feasibility probe (gfx1201).
//
// Q is resident BF16 [M,24,256], while K/V are resident FP16 in the
// interleaved [token,4,256] layout.  This probe compares the existing
// qtile4/K8 causal control with explicit four-way strided-batched rocBLAS:
// FP16-score (FP32 accumulation, FP16 probabilities), and F32-score (SGEMM).
// It is deliberately independent of production sources.
...
// Existing single-request control, with an 8-key LDS tile (Qtile4/K8).
__global__ __launch_bounds__(256, 1) void qtile4_k8_control(
  ...
  __shared__ uint16_t kt[8][kD];
  __shared__ uint16_t vt[8][kD];
...
#pragma unroll
  ...
  for (uint64_t begin = 0; begin <= last_key; begin += 8U) {
    const uint32_t count =
        static_cast<uint32_t>(std::min<uint64_t>(8U, last_key - begin + 1U));
    for (uint32_t i = t; i < 8U * kD; i += kThreads) {
...
bool stage_kv(Base *b, Pipe *p, uint64_t context, float *elapsed_us) {
...
bool measure_control(Base *b, uint64_t rows, uint64_t start, float *us) {
  const uint32_t blocks = static_cast<uint32_t>((rows + 3U) / 4U * kKvHeads);
  for (uint32_t i = 0; i < kWarmups; ++i) {
    hipLaunchKernelGGL(qtile4_k8_control, dim3(blocks), dim3(kThreads), 0U,
...
void compare(const char *name, const std::vector<uint16_t> &expected,
  ...
  std::printf("oracle candidate=%s values=%zu max_bf16_ulp=%u over1=%llu "
              "max_abs=%g max_rel=%g max_index=%zu status=%s\n",
              name, expected.size(), max, static_cast<unsigned long long>(over),
              max_abs, max_rel, max_index, max <= 16U ? "INFO" : "N2");
}
void resources() {
  const std::array<const void *, 5> f = {
      reinterpret_cast<const void *>(qtile4_k8_control),
  ...
  const std::array<const char *, 5> n = {"qtile4-k8-control", "pack-f16",
  ...
  for (size_t i = 0; i < f.size(); ++i) {
    ...
    std::printf("resources kernel=%s vgpr=%d lds=%zu scratch=%zu "
                "active_blocks=%d attrs=%s occupancy=%s\n",
...
int main(int argc, char **argv) {
  uint32_t device = 0U;
  uint64_t parsed = 0U;
  bool chunk_probe = false;
  if (argc > 2 || (argc == 2 && std::strcmp(argv[1], "--chunks") != 0 &&
                   (!parse_u64(argv[1], &parsed) || parsed > UINT32_MAX))) {
    std::fprintf(stderr, "usage: phase78_gqa6_gfx1201_rocblas_attention_probe "
                         "[device|--chunks]\n");
    return EXIT_FAILURE;
  }
  if (argc == 2) {
    chunk_probe = std::strcmp(argv[1], "--chunks") == 0;
    if (!chunk_probe)
  ...
  std::printf("target=%s device=%u pci=%04x:%02x:%02x name=%s "
              "sharedMemPerBlock=%zu regsPerBlock=%d maxThreads=%d\n",
              prop.gcnArchName, device, prop.pciDomainID, prop.pciBusID,
              prop.pciDeviceID, prop.name, prop.sharedMemPerBlock,
              prop.regsPerBlock, prop.maxThreadsPerBlock);
  if (std::string(prop.gcnArchName).find("gfx1201") == std::string::npos) {
    std::fprintf(stderr, "target_error expected gfx1201\n");
    return EXIT_FAILURE;
  }
  resources();

  // Only the deliberately small oracle case is evaluated on CPU.  Full
  // context cases compare against the GPU control to avoid a 59-billion-FMA
  // CPU oracle; all production-sized allocations remain device-resident.
  {
    constexpr uint64_t rows = 7U, context = 13U, start = 5U;
    std::vector<uint16_t> q, k, v, exp, got, control;
    ...
    compare("qtile4-k8-control-oracle", exp, control);
    for (Pipe::Kind kind : {Pipe::F16, Pipe::F32}) {
      ...
      float stage_us = 0.0F;
      if (kind == Pipe::F32 && !stage_kv(&b, &p, context, &stage_us)) {
        free_pipe(&p);
        free_base(&b);
        return EXIT_FAILURE;
      }
      ...
      compare(kind == Pipe::F16 ? "rocblas-fp16-score-oracle"
                                : "sgemm-f32-score-oracle",
              exp, got);
      std::vector<uint16_t> again;
      bool det = launch_pipe(&b, &p, rows, context, start) &&
                 hip_ok(hipStreamSynchronize(b.stream), "determinism sync") &&
                 copy_out(&b, rows, &again) && again == got;
      std::printf("determinism candidate=%s bitwise=%s stage_kv_us=%.3f\n",
                  kind == Pipe::F16 ? "rocblas-fp16-score" : "sgemm-f32-score",
                  det ? "PASS" : "FAIL", stage_us);
      free_pipe(&p);
    }
    free_base(&b);
  }
  ...
  if (chunk_probe) {
    // Qwen3.8 chunked prefill shape: all but the final chunk have M=1024;
    // context is the committed length and start is the bottom-right offset.
    for (uint64_t start :
         {0U, 1024U, 2048U, 3072U, 4096U, 5120U, 6144U, 7168U, 8192U, 9216U}) {
      const uint64_t rows = start == 9216U ? 219U : 1024U;
      const uint64_t context = start + rows;
      ...
      Pipe p;
      if (!make_pipe(rows, context, Pipe::F16, b.stream, &p)) {
        free_base(&b);
        return EXIT_FAILURE;
      }
      float candidate_us = 0.0F;
      if (!measure_pipe(&b, &p, rows, context, start, &candidate_us) ||
          !copy_out(&b, rows, &got)) {
        free_pipe(&p);
        free_base(&b);
        return EXIT_FAILURE;
      }
      compare("rocblas-fp16-score-chunk-vs-control", control, got);
      std::printf("chunk_result candidate=rocblas-fp16-score rows=%llu "
                  "context=%llu start=%llu control_us=%.3f candidate_us=%.3f "
                  "speedup_vs_qtile=%.4f\n",
                  static_cast<unsigned long long>(rows),
                  static_cast<unsigned long long>(context),
                  static_cast<unsigned long long>(start), control_us,
                  candidate_us, static_cast<double>(control_us) / candidate_us);
      free_pipe(&p);
      free_base(&b);
    }
    return EXIT_SUCCESS;
  ...
  for (uint64_t rows : {128U, 512U, 1024U}) {
    for (int long_tail = 0; long_tail < 2; ++long_tail) {
      ...
      for (Pipe::Kind kind : {Pipe::F16, Pipe::F32}) {
        ...
        compare(kind == Pipe::F16 ? "rocblas-fp16-score-vs-control"
                                  : "sgemm-f32-score-vs-control",
                control, got);
        std::vector<uint16_t> again;
        const bool det =
            launch_pipe(&b, &p, rows, context, start) &&
        ...
        const double projected =
            static_cast<double>(us) + (kind == Pipe::F32 ? stage_us : 0.0F);
        const double tflops = flops / (projected * 1.0e9);
        const double speedup = static_cast<double>(control_us) / projected;
        std::printf(
            "pipeline_result candidate=%s rows=%llu context=%llu start=%llu "
            "compute_us=%.3f stage_kv_us=%.3f projected_us=%.3f "
            "workspace_bytes=%llu qpack_bytes=%llu score_bytes=%llu "
            "pv_bytes=%llu qk_pv_tflops=%.4f speedup_vs_qtile=%.4f "
            "determinism=%s precision=%s classification=%s\n",
            kind == Pipe::F16 ? "rocblas-fp16-score" : "sgemm-f32-score",
            static_cast<unsigned long long>(rows),
            static_cast<unsigned long long>(context),
            static_cast<unsigned long long>(start), us, stage_us, projected,
            ...
            det ? "PASS" : "FAIL",
            kind == Pipe::F16 ? "fp16-score" : "f32-score",
            kind == Pipe::F16 ? "N2" : "N1");
        free_pipe(&p);
      }
      free_base(&b);
  ...
  std::printf(
      "summary status=%s target=gfx1201 control=qtile4-k8 "
      "pipeline=4-way-strided-batched-qk-softmax-pv warmups=%u measured=%u "
      "oracle=7x13-start5 contexts=M,9435 candidates=fp16-score,f32-score\n",

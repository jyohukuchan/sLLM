// Phase 87 Stage 10: public Paged KV request correctness.
//
// This test deliberately uses the additive public ABI.  It keeps the case
// count small while covering the three sides of the 128-token boundary,
// append->attention dependency ordering, paged fork/COW, and a small host
// MXFP8 attention oracle.
#include "sllm/hip.h"
...
bool wait_and_release(sllm_completion_t **const completion,
  ...
  return waited && timed && released && *completion == nullptr;
}
...
bool run_case(const sllm_context_t *const context,
  ...
  passed &= parent_view.observed_length == prefix_length &&
  ...
  const uint64_t bytes_per_token =
      static_cast<uint64_t>(kKvHeads) * (kHeadDim + kHeadDim / 32U) * 2U;
  passed &=
  ...
  std::cout << "phase87 request prefix=" << prefix_length
...
int main() {
  ...
  for (const uint64_t prefix : {UINT64_C(127), UINT64_C(128), UINT64_C(129)}) {
    if (passed &&
      ...
      std::cerr << "public paged request case failed at prefix=" << prefix
                << '\n';
      passed = false;
    }
  }
  ...
  if (passed) {
    std::cout << "phase87_stage10_paged_request_public_gpu_test: PASS target="
              << SLLM_TEST_EXPECTED_TARGET
              << " cases=7 boundaries=127,128,129 decode_m=1..5 fork_cow=1 "
                 "cancel_rollback=1"
                 " oracle=independent-mxfp8-attention fallback=0 cleanup=0\n";
  }
  return passed ? 0 : 1;
}

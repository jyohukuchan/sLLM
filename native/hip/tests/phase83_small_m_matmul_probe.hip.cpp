...
// Phase 83 bounded M=1..4 production-provider probe.  This deliberately uses
// the public matmul ABI rather than private launchers: each M gets its own
// prepared plan, activation quantization, provider selection, dispatch audit,
// device timing, and sampled independent oracle.  It is a probe, not a
// production selector or a correctness gate.

namespace {

constexpr std::array<uint64_t, 4> kRows = {1U, 2U, 3U, 4U};
...
std::vector<uint8_t> make_fp8_weight(const Shape &shape) {
  const uint64_t values = shape.k * shape.n;
  std::vector<uint8_t> result(static_cast<std::size_t>(values + shape.n * 4U),
                              UINT8_C(0x38)); // E4M3FN 1.0.
  const float one = 1.0F;
  for (uint64_t row = 0U; row != shape.n; ++row) {
    std::memcpy(result.data() + values + row * sizeof(float), &one,
                sizeof(one));
  }
  return result;
}
...
bool check_samples(const Shape &shape, const uint64_t rows,
  ...
  const uint64_t sample_mid = shape.n / 2U;
  const uint64_t sample_last = shape.n - 1U;
  const std::array<uint64_t, 3> columns = {0U, sample_mid, sample_last};
...
bool run_shape(const Shape &shape, const sllm_context_t *const context,
  ...
  const bool is_nvfp4 = std::strcmp(shape.family, "NVFP4") == 0;
  const uint64_t weight_bytes =
      is_nvfp4 ? nvfp4_weight_bytes(shape) : shape.k * shape.n + shape.n * 4U;
  std::vector<uint8_t> host_weight =
      is_nvfp4 ? make_nvfp4_weight(shape) : make_fp8_weight(shape);
  ...
  for (const uint64_t rows : kRows) {
    ...
    if (valid) {
      ...
      const double sequential_ratio =
          m1_ms > 0.0 ? run.median_ms / (static_cast<double>(rows) * m1_ms)
                      : 0.0;
      std::cout << std::fixed << std::setprecision(6)
                << "phase83_small_m family=" << shape.family
                ...
                << " sequential_m1_ratio=" << sequential_ratio
                << " max_bf16_ulp=" << run.max_bf16_ulp
...
void print_environment() {
  constexpr std::array<const char *, 7> names = {
      ...
      "SLLM_NVFP4_W4A4_PREFILL_FORCE_ROW8_COL8",
  ...
  std::cout << "phase83_small_m target=" << SLLM_TEST_EXPECTED_TARGET
            << " timing=completion_device_event warmups=" << kWarmups
            << " measured=" << kMeasured << '\n';
  for (const char *const name : names) {
    const char *const value = std::getenv(name);
    std::cout << "  " << name << "=" << (value == nullptr ? "<unset>" : value)
              << '\n';
  }
}
  ...
  try {
    ...
    const std::array<Shape, 4> shapes = {{
        ...
        {"NVFP4", "mlp_down", 17408U, 5120U, SLLM_HIP_MATMUL_NVFP4_W4A4_VERSION,
         ...
         SLLM_TENSOR_ENCODING_NVFP4_W4A4_BLOCK16_E4M3FN_F32},
        {"FP8", "gdn_qkv", 5120U, 10240U, SLLM_HIP_MATMUL_FP8_VERSION,
         SLLM_TENSOR_DTYPE_F8_E4M3_FN, SLLM_TENSOR_ENCODING_FP8_OUTER_F32},
        {"FP8", "lm_head", 5120U, 248320U, SLLM_HIP_MATMUL_FP8_VERSION,
         SLLM_TENSOR_DTYPE_F8_E4M3_FN, SLLM_TENSOR_ENCODING_FP8_OUTER_F32},
    }};
    ...
    std::cout
        << "phase83_small_m status=" << (valid ? "PASS" : "FAIL")
        << " gpu_run_required=1 independent_sample_oracle=1 resources_released="
        << ((queue == nullptr && context == nullptr) ? 1 : 0) << '\n';
    return valid ? 0 : 1;
  } catch (const std::exception &error) {
    std::cerr << "phase83_small_m exception: " << error.what() << '\n';
    return 2;
  }
}

// WU1.1 operator benchmark. Existing project Phase83 fixture/FP32 oracle
// reused.
...
namespace {
#include "phase87_wu1_1_candidates.hpp"

// C3 merge reads each partition directly; larger splits do not exceed LDS.
template <unsigned Splits>
__global__ void phase87_wu1_1_merge(const float *w, uint16_t *out, unsigned M) {
...
namespace {
bool use_gqa_control = false;
unsigned split_count(int variant) {
  return variant == 0 ? 32U : 32U << variant;
}
...
struct WU1Buffers {
  ...
  WU1Buffers(const Fixture &f, uint64_t l, unsigned count)
      : q(f.query.size()), out(f.query.size() + 32), k(f.key.size()),
        ...
        work(size_t(count) * 24 * 256 * 258 + 64), length(l), m(count) {
    q.put(f.query);
    k.put(f.key);
    v.put(f.value);
    ks.put(f.key_scales);
...
template <unsigned Splits> void candidate_stage1(WU1Buffers &b) {
  if (use_gqa_control) {
    hipLaunchKernelGGL(
        HIP_KERNEL_NAME(
            sllm_causal_attention_kernel::phase87_wu1_1_gqa_stage1<Splits>),
        dim3(b.m * 4 * Splits), dim3(192), 0, nullptr, b.q.ptr, b.k.ptr,
        b.v.ptr, b.ks.ptr, b.vs.ptr, b.work.ptr, b.m, b.length - b.m);
  } else {
    hipLaunchKernelGGL(
        HIP_KERNEL_NAME(
            sllm_causal_attention_kernel::phase87_wu1_1_wave_stage1<Splits>),
        dim3(b.m * 24 * Splits), dim3(32), 0, nullptr, b.q.ptr, b.k.ptr,
        b.v.ptr, b.ks.ptr, b.vs.ptr, nullptr, nullptr, b.work.ptr, b.m,
        b.length - b.m, 24U, 4U, 256U, 1.0F, 1.0F);
  }
}
  ...
  case 0:
    if (use_gqa_control) {
      hipLaunchKernelGGL(
          ...
          dim3(b.m * 4 * 32), dim3(192), 0, nullptr, b.q.ptr, b.k.ptr, b.v.ptr,
          b.ks.ptr, b.vs.ptr, b.work.ptr, b.m, b.length - b.m);
    } else
      baseline_stage1<32>(b);
    break;
  case 1:
    candidate_stage1<64>(b);
    break;
  case 2:
    candidate_stage1<128>(b);
    break;
  case 3:
    require(!use_gqa_control, "V620 supports only split64/128 candidates");
    candidate_stage1<256>(b);
    break;
  default:
...
template <unsigned Splits> void candidate_stage2(WU1Buffers &b) {
  hipLaunchKernelGGL(
      HIP_KERNEL_NAME(
          sllm_causal_attention_kernel::phase87_wu1_1_merge<Splits>),
...
void stage2(WU1Buffers &b, int variant) {
  ...
  case 0:
    ...
    break;
  case 1:
    candidate_stage2<64>(b);
    break;
  case 2:
    candidate_stage2<128>(b);
    break;
  case 3:
    candidate_stage2<256>(b);
    break;
  default:
    throw std::runtime_error("variant");
  }
...
Errors validate(WU1Buffers &b, int variant,
  ...
  for (size_t i = size_t(b.m) * 24 * split_count(variant) * 258;
       i < workspace.size(); ++i) {
    uint32_t raw;
...
int main(int argc, char **argv) {
  try {
    ...
    use_gqa_control = expected_target == "gfx1030";
    const int variant_count = use_gqa_control ? 3 : 4;
    ...
    bool pass = true;
    {
      ...
      if (pass && bench)
        for (int candidate = 1; candidate < variant_count; ++candidate) {
          std::vector<float> base1, cand1, base2, cand2, base_total, cand_total;
          for (int round = 0; round < 3; ++round) {
            for (int pos = 0; pos < 2; ++pos) {
              ...
              for (int sample = 0; sample < 9; ++sample) {
                ...
                (v == 0 ? base2 : cand2).push_back(t.stage2);
          ...
          std::cout << "{\"kind\":\"performance\",\"target\":\""
                    ...
                    << ",\"control_stage2_ms\":" << med(base2)
                    << ",\"candidate_stage2_ms\":" << med(cand2)
          ...
          samples("control_stage2_samples_ms", base2);
          samples("candidate_stage2_samples_ms", cand2);

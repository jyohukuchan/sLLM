#ifndef SLLM_LOWP_KERNEL_INTERNAL_HPP
#define SLLM_LOWP_KERNEL_INTERNAL_HPP
...
// Phase 83 ID92: gfx1030 FP8 outer-scale M=2..4 fusion. One CTA keeps each
// output-column weight dword/scale live while accumulating independent rows;
// M=1 remains on the existing ID82/ID68 providers.
constexpr const char *kFp8OuterDecodeGfx1030FusedM2_4LogicalKernelId =
...
static_assert(sizeof("sllm_matmul_fp8_outer_prefill_gfx1030_half2_64x64_v1") <=
              64U);
static_assert(sizeof("matmul.fp8.outer.decode.gfx1030.fused.m2_4.v1") <= 64U);
static_assert(
...
constexpr const char *kNvfp4W4A4PrefillGfx1201WmmaKahanLogicalKernelId =
...
constexpr const char *kNvfp4W4A4SmallMGfx1201RowGridLogicalKernelId =
    "matmul.nvfp4.w4a4.small_m.gfx1201.rowgrid.v1";
constexpr const char *kNvfp4W4A4SmallMGfx1201RowGridDeviceSymbol =
...
enum class KernelVariant : uint32_t {
  ...
  Unspecialized = 1U,
  Fp8Emulation = 6U,
  // Retired W4A16 audit IDs. Keep the numbers reserved without mapping them
  // to a kernel, provider, or launch path.
  RetiredNvfp4W4A16DecodePackedDequant = 8U,
  RetiredNvfp4W4A16PrefillRow8Tiled256 = 9U,
  RetiredNvfp4W4A16BaselinePackedDequant = 10U,
  Nvfp4W4A4Packed = 11U,
  Mxfp4W4A4Decode = 14U,
  ...
  Nvfp4W4A4PrefillCompensated64x64 = 87U,
  Nvfp4W4A4SmallMRowGrid = 88U,
  Nvfp4W4A4PrefillGfx1201WmmaKahan = 89U,
  ...
  // Retired W8A16/W6A16 kernel IDs. Keep numeric tombstones for audit data;
  // no symbols or launchers are compiled for these variants.
...
struct SelectorDecision final {
  ...
  constexpr bool runnable() const noexcept { return supported && enabled; }
};
...
make_selector_decision(const KernelVariant variant, const bool supported,
                       const bool enabled, const bool adopted,
...
constexpr bool
phase83_gfx1201_nvfp4_w4a4_wmma_kahan_shape(const uint64_t m, const uint64_t k,
                                            const uint64_t n) noexcept {
  return m > 1U && k >= 2048U && (k % 16U) == 0U &&
...
phase78_nvfp4_w4a4_decode_wave4col32_shape(const uint64_t m, const uint64_t k,
                                           const uint64_t n) noexcept {
  return m == 1U && k > 0U && (k % 16U) == 0U && k <= kNvfp4W4A4DecodeMaxK &&
         n > 0U;
}
...
constexpr bool phase83_gfx1201_nvfp4_w4a4_small_m_rowgrid_shape(
    const uint64_t m, const uint64_t k, const uint64_t n) noexcept {
  return m >= 2U && m <= 4U &&
         phase78_nvfp4_w4a4_decode_scale_lut_gfx1201_activation_shared_shape(
             1U, k, n);
...
static_assert(phase83_gfx1201_nvfp4_w4a4_small_m_rowgrid_shape(2U, 5120U,
                                                               17408U));
static_assert(phase83_gfx1201_nvfp4_w4a4_small_m_rowgrid_shape(4U, 17408U,
                                                               5120U));
static_assert(!phase83_gfx1201_nvfp4_w4a4_small_m_rowgrid_shape(1U, 5120U,
                                                                17408U));
static_assert(!phase83_gfx1201_nvfp4_w4a4_small_m_rowgrid_shape(5U, 5120U,
                                                                17408U));
static_assert(!phase83_gfx1201_nvfp4_w4a4_small_m_rowgrid_shape(2U, 5120U,
                                                                5120U));
...
constexpr bool
nvfp4_w4a4_decode_wave4_adopted_shape(const uint64_t m, const uint64_t k,
                                      const uint64_t n) noexcept {
  return phase78_nvfp4_w4a4_decode_wave4col32_shape(m, k, n) && k >= 1024U &&
         n >= 1024U;
}
...
select_nvfp4_w4a4_variant(const uint64_t m, const uint64_t k, const uint64_t n,
  ...
  if (m == 1U) {
    ...
    // Explicit provider controls retain precedence over Phase82 defaults.
    // LUT adoption is restricted to the two measured projection shapes.
    if (force_wave4 == nullptr && force_activation_shared == nullptr &&
        force_scale_lut == nullptr && supported_target &&
        phase78_nvfp4_w4a4_decode_activation_shared_shape(m, k, n)) {
      return KernelVariant::Nvfp4W4A4DecodeScaleLut;
    }
    if (force_wave4 == nullptr && force_activation_shared == nullptr &&
    ...
    if (force_wave4 == nullptr && supported_target &&
        nvfp4_w4a4_decode_wave4_adopted_shape(m, k, n)) {
      return KernelVariant::Nvfp4W4A4DecodeWave4Column32;
    }
    return KernelVariant::Nvfp4W4A4Decode;
  }
  if (target_is(target, "gfx1030") && m >= 2U && m <= 4U &&
      phase78_nvfp4_w4a4_decode_activation_shared_shape(1U, k, n) &&
      selector_env_is_one("SLLM_NVFP4_W4A4_SMALL_M_ROWGRID")) {
    return KernelVariant::Nvfp4W4A4SmallMRowGrid;
  }
  if (target_is(target, "gfx1201") &&
  ...
  if (target_is(target, "gfx1201") &&
      phase83_gfx1201_nvfp4_w4a4_wmma_kahan_shape(m, k, n) &&
...
inline SelectorDecision
select_nvfp4_w4a4_decision(const uint64_t m, const uint64_t k, const uint64_t n,
                           const char *const target) noexcept {
  const KernelVariant variant = select_nvfp4_w4a4_variant(m, k, n, target);
  const bool exact_gfx1030 = target_is(target, "gfx1030");
  const bool exact_gfx1201 = target_is(target, "gfx1201");
  const bool known_target =
      exact_gfx1030 || exact_gfx1201 || target_is(target, "gfx942");
  const bool supported_decode_target = exact_gfx1030 || exact_gfx1201;
  switch (variant) {
  case KernelVariant::Nvfp4W4A4DecodeWave4Column32: {
    const bool supported = supported_decode_target &&
                           phase78_nvfp4_w4a4_decode_wave4col32_shape(m, k, n);
    const bool adopted =
  ...
  case KernelVariant::Nvfp4W4A4DecodeScaleLut: {
    ...
    return make_selector_decision(
        variant, supported,
        adopted || selector_env_is_one(kNvfp4W4A4DecodeScaleLutEnvironment),
        adopted,
        !supported ? kSelectorReasonUnsupported
        : adopted  ? kSelectorReasonAdopted
                   : kSelectorReasonForcedShape);
  ...
  case KernelVariant::Nvfp4W4A4PrefillGfx1201F16Staging: {
    const bool supported =
        exact_gfx1201 && phase78_gfx1201_nvfp4_w4a4_f16_staging_shape(m, k, n);
    return make_selector_decision(
        variant, supported,
        selector_env_is_one(kNvfp4W4A4PrefillGfx1201F16StagingEnvironment),
        false,
...
constexpr bool
fp8_outer_decode_dword8_adopted_shape(const uint64_t m, const uint64_t k,
                                      const uint64_t n) noexcept {
  return fp8_outer_decode_gfx1030_half2_shape(m, k, n) && k >= 128U && n >= 64U;
}
...
inline KernelVariant
select_fp8_software_variant(const uint64_t m, const uint64_t k,
                            const uint64_t n, const char *const target,
  ...
  if (m == 1U) {
    ...
    if (force_lds_lut == nullptr && force_activation_shared == nullptr &&
        force_dword8 == nullptr && force_half2 == nullptr &&
  ...
  if (!fnuz && fp8_outer_decode_gfx1030_fused_m2_4_shape(m, k, n)) {
    return KernelVariant::Fp8OuterDecodeGfx1030FusedM2_4;
  }
  const char *const force_half2 =
...
inline SelectorDecision
select_fp8_software_decision(const uint64_t m, const uint64_t k,
                             const uint64_t n, const char *const target,
                             const bool fnuz = false) noexcept {
  const KernelVariant variant =
      select_fp8_software_variant(m, k, n, target, fnuz);
  const bool exact_gfx1030 = target_is(target, "gfx1030");
  switch (variant) {
  ...
  case KernelVariant::Fp8OuterDecodeGfx1030Dword8Wave4Col32: {
    ...
    const bool adopted =
        supported && !direct_force && !shared_force &&
  ...
  case KernelVariant::Fp8OuterDecodeGfx1030LdsLutWave4Col32: {
    ...
    return make_selector_decision(
        variant, supported,
        adopted || selector_env_is_one(kFp8OuterDecodeGfx1030LdsLutEnvironment),
        adopted,
        !supported ? kSelectorReasonUnsupported
        : adopted  ? kSelectorReasonAdopted
                   : kSelectorReasonForcedShape);
  ...
  case KernelVariant::Fp8OuterDecodeGfx1030FusedM2_4: {
    const bool supported = exact_gfx1030 && !fnuz &&
                           fp8_outer_decode_gfx1030_fused_m2_4_shape(m, k, n);
    return make_selector_decision(variant, supported, supported, supported,
                                  supported ? kSelectorReasonAdopted
                                            : kSelectorReasonUnsupported);
  }
  case KernelVariant::Fp8OuterPrefillGfx1030Half2_64x64: {
    const bool supported =
        exact_gfx1030 && !fnuz && m > 1U && k != 0U && n != 0U;
    const bool forced =
  ...
  case KernelVariant::Fp8Emulation: {
    ...
    if (exact_gfx1030) {
      reason = baseline_selector_reason(forced_decode || forced_prefill, fnuz,
                                        decode_candidate_requested,
                                        decode_candidate_configured);
    }
    return make_selector_decision(variant, supported, true,
                                  !forced_decode && !forced_prefill, reason);
  }
  default:
    return make_selector_decision(variant, false, false, false,
                                  kSelectorReasonUnsupported);
...
// Returns nullptr for IDs that are not low-precision providers so the
// integrating runtime can resolve its own reference/provider IDs.
constexpr const char *
lowp_logical_kernel_id(const KernelVariant variant) noexcept {
  ...
  if (variant == KernelVariant::Fp8OuterDecodeGfx1030FusedM2_4) {
    return kFp8OuterDecodeGfx1030FusedM2_4LogicalKernelId;
  }
  ...
  return variant == KernelVariant::Fp8Emulation ? kFp8EmulationLogicalKernelId
         ...
         : variant == KernelVariant::Nvfp4W4A4PrefillGfx1201WmmaKahan
             ? kNvfp4W4A4PrefillGfx1201WmmaKahanLogicalKernelId
         : variant == KernelVariant::Nvfp4W4A4SmallMGfx1201RowGrid
             ? kNvfp4W4A4SmallMGfx1201RowGridLogicalKernelId
         : variant == KernelVariant::Nvfp4W4A4PrefillCompensated64x64
             ? "matmul.nvfp4.w4a4.block16.prefill.compensated64x64.v1"
         ...
         : variant == KernelVariant::Nvfp4W4A4SmallMRowGrid
             ? "matmul.nvfp4.w4a4.small_m.rowgrid.v1"
...
// Returns nullptr for IDs that are not low-precision providers.
constexpr const char *lowp_device_symbol(const KernelVariant variant) noexcept {
  if (variant == KernelVariant::Mxfp8W8A8PrefillPhase85SmallM) {
  ...
  if (variant == KernelVariant::Fp8OuterDecodeGfx1030FusedM2_4) {
    return kFp8OuterDecodeGfx1030FusedM2_4K5120N10240DeviceSymbol;
  }
  if (variant == KernelVariant::Nvfp4W4A4DecodeScaleLut) {
  ...
  return variant == KernelVariant::Fp8Emulation ? kFp8EmulationDeviceSymbol
         : variant == KernelVariant::Fp8OuterPrefillTiled16
         ...
         : variant == KernelVariant::Nvfp4W4A4SmallMGfx1201RowGrid
             ? kNvfp4W4A4SmallMGfx1201RowGridDeviceSymbol
         : variant == KernelVariant::Nvfp4W4A4PrefillCompensated64x64
...
inline const char *
lowp_logical_kernel_id_for_target(const KernelVariant variant,
                                  const char *const target) noexcept {
  (void)target;
  return lowp_logical_kernel_id(variant);
}

inline const char *
lowp_device_symbol_for_target(const KernelVariant variant,
                              const char *const target) noexcept {
  if (variant == KernelVariant::Nvfp4W4A4DecodeScaleLut) {
    (void)target;
    return kNvfp4W4A4DecodeScaleLutDeviceSymbol;
  }
  return lowp_device_symbol(variant);
}

inline const char *lowp_device_symbol_for_target(const KernelVariant variant,
  ...
  if (variant == KernelVariant::Fp8OuterDecodeGfx1030FusedM2_4 &&
    ...
    if (n == UINT64_C(248320)) {
      return kFp8OuterDecodeGfx1030FusedM2_4K5120N248320DeviceSymbol;
    }
    return kFp8OuterDecodeGfx1030FusedM2_4K5120N10240DeviceSymbol;
  }
  return lowp_device_symbol_for_target(variant, target);
}

// IDs that are not low-precision providers use the generic row-major launch
// geometry; the integrating runtime resolves its own providers first.
constexpr uint32_t lowp_grid_size_x(const KernelVariant variant,
  ...
  return variant == KernelVariant::Fp8OuterPrefillTiled16
         ...
         : variant == KernelVariant::Fp8OuterDecodeGfx1030LdsLutWave4Col32
             ? static_cast<uint32_t>(
                   (n + kFp8OuterDecodeGfx1030Half2ColumnsPerWorkgroup - 1U) /
                   kFp8OuterDecodeGfx1030Half2ColumnsPerWorkgroup)
         : variant == KernelVariant::Fp8OuterDecodeGfx1030FusedM2_4
             ? static_cast<uint32_t>(
                   (n + kFp8OuterDecodeGfx1030Half2ColumnsPerWorkgroup - 1U) /
                   kFp8OuterDecodeGfx1030Half2ColumnsPerWorkgroup)
         : variant ==
...
static_assert(
    ...
    kFp8OuterDecodeGfx1030LdsLutWorkgroupSize);
static_assert(workgroup_size_x(KernelVariant::Fp8OuterDecodeGfx1030FusedM2_4) ==
              kFp8OuterDecodeGfx1030LdsLutWorkgroupSize);
static_assert(lowp_grid_size_x(KernelVariant::Fp8OuterDecodeGfx1030FusedM2_4,
                               2U, 10240U, 5120U) == 320U);
static_assert(lowp_grid_size_x(KernelVariant::Fp8OuterDecodeGfx1030FusedM2_4,
                               2U, 12288U, 5120U) == 384U);
static_assert(lowp_grid_size_x(KernelVariant::Fp8OuterDecodeGfx1030FusedM2_4,
                               2U, 1024U, 5120U) == 32U);
static_assert(lowp_grid_size_x(KernelVariant::Fp8OuterDecodeGfx1030FusedM2_4,
                               2U, 17408U, 5120U) == 544U);
static_assert(lowp_grid_size_x(KernelVariant::Fp8OuterDecodeGfx1030FusedM2_4,
                               2U, 5120U, 17408U) == 160U);
static_assert(lowp_grid_size_x(KernelVariant::Fp8OuterDecodeGfx1030FusedM2_4,
                               2U, 5120U, 6144U) == 160U);
static_assert(
    workgroup_size_x(
...
static_assert(
    ...
    kFp8OuterDecodeGfx1030Half2WorkgroupSize);
static_assert(
    lowp_grid_size_x(KernelVariant::Mxfp8W8A8PrefillWmmaN128DirectBoth, 128U,
                     128U) == 1U);
static_assert(
    lowp_grid_size_x(KernelVariant::Mxfp8W8A8PrefillWmmaN128DirectBoth, 128U,
                     256U) == 2U);
static_assert(lowp_grid_size_x(KernelVariant::Mxfp8W8A8PrefillPhase85SmallM, 2U,
                               1024U) == 128U);
static_assert(lowp_grid_size_x(KernelVariant::Mxfp6W6A6PrefillPhase85SmallM, 4U,
                               1025U) == 129U);
static_assert(lowp_grid_size_x(KernelVariant::Mxfp8W8A8M1Col2, 1U, 1024U) ==
              512U);
static_assert(lowp_grid_size_x(KernelVariant::Mxfp8W8A8M1Col2, 1U, 1025U) ==
              513U);
static_assert(lowp_grid_size_x(KernelVariant::Mxfp6W6A6M1Col2, 1U, 1024U) ==
              512U);
static_assert(lowp_grid_size_x(KernelVariant::Nvfp4W4A4Decode, 1U, 7U) == 7U);
static_assert(lowp_grid_size_x(KernelVariant::Nvfp4W4A4DecodeWave4Column32, 1U,
                               31U) == 1U);
static_assert(lowp_grid_size_x(KernelVariant::Nvfp4W4A4DecodeWave4Column32, 1U,
                               32U) == 1U);
static_assert(lowp_grid_size_x(KernelVariant::Nvfp4W4A4DecodeWave4Column32, 1U,
                               33U) == 2U);
static_assert(lowp_grid_size_x(KernelVariant::Nvfp4W4A4DecodeActivationShared,
                               1U, 5120U) == 160U);
static_assert(lowp_grid_size_x(KernelVariant::Nvfp4W4A4DecodeActivationShared,
                               1U, 17408U) == 544U);
static_assert(
...
static_assert(
    lowp_grid_size_x(
        ...
        12288U) == 192U);
static_assert(lowp_grid_size_x(KernelVariant::Nvfp4W4A4PrefillRow8Tiled256, 17U,
                               7U) == 21U);
static_assert(lowp_grid_size_x(KernelVariant::Nvfp4W4A4PrefillRow8Col8Tiled256,
                               17U, 7U) == 3U);
static_assert(lowp_grid_size_x(KernelVariant::Nvfp4W4A4PrefillDp4a64x64, 65U,
                               65U) == 4U);
static_assert(lowp_grid_size_x(KernelVariant::Nvfp4W4A4PrefillGfx1201Wmma128x64,
                               127U, 63U) == 1U);
static_assert(lowp_grid_size_x(KernelVariant::Nvfp4W4A4PrefillGfx1201Wmma128x64,
                               129U, 65U) == 2U);
static_assert(lowp_grid_size_x(KernelVariant::Nvfp4W4A4PrefillGfx1201WmmaKahan,
                               1024U, 17408U) == 272U);
static_assert(lowp_grid_size_x(KernelVariant::Nvfp4W4A4Packed, 2U, 7U) == 14U);
static_assert(lowp_grid_size_x(KernelVariant::Fp8OuterPrefillTiled16, 17U,
                               17U) == 4U);
static_assert(
    lowp_grid_size_x(KernelVariant::Fp8OuterPrefillGfx1030Half2_128x64, 129U,
                     65U) == 4U);
static_assert(lowp_grid_size_x(KernelVariant::Fp8OuterPrefillGfx1030Half2_64x64,
                               65U, 65U) == 4U);
static_assert(lowp_grid_size_x(KernelVariant::Fp8OuterPrefillGfx1030Half2_64x64,
                               17U, 5120U) == 80U);
static_assert(lowp_grid_size_x(KernelVariant::Fp8OuterPrefillGfx1030Half2_64x64,
                               17U, 5120U, 6144U) == 160U);
static_assert(lowp_grid_size_x(KernelVariant::Fp8OuterPrefillGfx1030Half2_64x64,
                               17U, 10240U, 5120U) == 160U);
static_assert(lowp_grid_size_x(KernelVariant::Fp8OuterPrefillGfx1030Half2_64x64,
                               17U, 12288U, 5120U) == 192U);
static_assert(lowp_grid_size_x(KernelVariant::Fp8OuterPrefillGfx1030Half2_64x64,
                               17U, 17408U, 5120U) == 272U);
static_assert(lowp_grid_size_x(KernelVariant::Fp8OuterPrefillGfx1030Half2_64x64,
                               33U, 5120U, 6144U) == 80U);
static_assert(
    lowp_grid_size_x(KernelVariant::Fp8OuterDecodeGfx1030Half2Wave4Col32, 1U,
                     32U) == 1U);
static_assert(
    lowp_grid_size_x(KernelVariant::Fp8OuterDecodeGfx1030Half2Wave4Col32, 1U,
                     33U) == 2U);
static_assert(
    lowp_grid_size_x(KernelVariant::Fp8OuterDecodeGfx1030Dword8Wave4Col32, 1U,
                     32U) == 1U);
static_assert(
    lowp_grid_size_x(KernelVariant::Fp8OuterDecodeGfx1030Dword8Wave4Col32, 1U,
                     33U) == 2U);
static_assert(
    lowp_grid_size_x(KernelVariant::Fp8OuterDecodeGfx1030LdsLutWave4Col32, 1U,
                     32U) == 1U);
static_assert(
    lowp_grid_size_x(KernelVariant::Fp8OuterDecodeGfx1030LdsLutWave4Col32, 1U,
                     33U) == 2U);
static_assert(lowp_grid_size_x(KernelVariant::Nvfp4W4A4DecodeScaleLut, 1U,
                               32U) == 1U);
static_assert(lowp_grid_size_x(KernelVariant::Nvfp4W4A4DecodeScaleLut, 1U,
                               33U) == 2U);
static_assert(lowp_grid_size_x(KernelVariant::Nvfp4W4A4SmallMGfx1201RowGrid, 2U,
                               32U) == 1U);
static_assert(lowp_grid_size_x(KernelVariant::Nvfp4W4A4SmallMGfx1201RowGrid, 4U,
                               33U) == 2U);
static_assert(lowp_grid_size_x(KernelVariant::Nvfp4W4A4SmallMVgprReuse, 2U,
                               32U) == 1U);
static_assert(lowp_grid_size_x(KernelVariant::Nvfp4W4A4SmallMVgprReuse, 4U,
                               33U) == 2U);
static_assert(lowp_grid_size_x(KernelVariant::Mxfp8W8A8PrefillWmmaN64, 128U,
                               15U) == 1U);
static_assert(lowp_grid_size_x(KernelVariant::Mxfp8W8A8PrefillWmmaN64, 128U,
                               16U) == 1U);
static_assert(lowp_grid_size_x(KernelVariant::Mxfp8W8A8PrefillWmmaN64, 128U,
...
static_assert(lowp_grid_size_x(KernelVariant::Mxfp8W8A8PrefillWmmaN64, 128U,
                               65U) == 2U);
static_assert(lowp_grid_size_x(KernelVariant::Mxfp8W8A8PrefillWmmaN16, 128U,
                               33U) == 3U);
static_assert(
    lowp_grid_size_x(KernelVariant::Mxfp8W8A8PrefillMmqGfx1030Vector32, 9U,
                     9U) == 4U);
...
hipError_t launch_fp8_outer_decode_gfx1030_fused_m2_4(
    const uint8_t *activation, const float *activation_scales,
    const uint8_t *weight, const float *weight_scales, uint16_t *output,
    uint64_t m, uint64_t k, uint64_t n, hipStream_t stream) noexcept;
...
#endif // SLLM_LOWP_KERNEL_INTERNAL_HPP

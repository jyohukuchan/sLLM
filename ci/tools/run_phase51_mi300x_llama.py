#!/usr/bin/env python3
"""Produce the fixed llama.cpp Phase 51 MI300X comparison rows.
...
ROW_SCHEMA_VERSION = "phase51-mi300x-llama-row-v1"
WRAPPER_SCHEMA_VERSION = "llama-phase51-mi300x-v1"
...
MAX_TOKEN_ID = 248319
DEFAULT_AMD_SMI = f"{ROCM_ROOT}/bin/amd-smi"
...
def matrix() -> list[dict[str, Any]]:
    ...
    for case_id, input_count, output_count in CASE_SPECS:
        ...
        rows.append({
            "row_id": f"phase51-mi300x-llama-{case_id}",
            "case_id": case_id,
            "input_token_ids": ids,
...
class _SysfsMonitor:
    ...
    def start(self) -> None:
        self.thread = threading.Thread(target=self._run, name="phase51-mi300x-llama-sysfs", daemon=True)
        self.thread.start()
...
def _failure_class(
    ...
    if capture.get("timed_out") is True:
        return "timeout", "wrapper exceeded the configured timeout"
...
def execution_environment(row_id: str, library_dir: Path, base: Mapping[str, str] | None = None) -> dict[str, str]:
    ...
    env["LD_LIBRARY_PATH"] = f"{library_dir}:{ROCM_ROOT}/lib"
    env["SLLM_PHASE51_MI300X_LLAMA_ROW"] = row_id
    return env
...
def _resume_row(row_dir: Path, row: Mapping[str, Any], model_sha256: str, binary_sha256: str) -> dict[str, Any] | None:
    ...
    if isinstance(report, dict) and report.get("state") == "FAIL":
        if report.get("schema_version") != ROW_SCHEMA_VERSION or report.get("row") != dict(row) or report.get("model", {}).get("sha256") != model_sha256 or report.get("binary", {}).get("sha256") != binary_sha256 or report.get("target") != TARGET or report.get("gpu_uuid") != GPU_UUID or report.get("gpu_bdf") != GPU_BDF:
            _fail(f"{row['row_id']}: existing failed row identity differs")
        ...
        if not isinstance(failure, dict) or not isinstance(failure.get("kind"), str) or not isinstance(failure.get("reason"), str) or not failure["kind"] or not failure["reason"] or len(failure["reason"]) > MAX_FAILURE_REASON_CHARS:
            _fail(f"{row['row_id']}: existing failed row failure evidence is malformed")
        if not isinstance(report.get("command"), list) or not report["command"] or not isinstance(report.get("process"), dict) or not isinstance(report.get("memory"), dict) or not isinstance(report.get("monitor"), dict):
            _fail(f"{row['row_id']}: existing failed row execution evidence is absent")
        raw = report.get("raw")
        if not isinstance(raw, dict):
            _fail(f"{row['row_id']}: existing failed row raw manifest is absent")
        for key in ("stdout", "stderr", "monitor_tsv"):
            item = raw.get(key)
            if not isinstance(item, dict):
                _fail(f"{row['row_id']}: existing failed row raw {key} manifest is absent")
...
def _failure_report(
    ...
    report: dict[str, Any] = {
        ...
        "environment": {"ROCR_VISIBLE_DEVICES": environment.get("ROCR_VISIBLE_DEVICES"), "LD_LIBRARY_PATH": environment.get("LD_LIBRARY_PATH")},
        ...
        "raw": {
            "stdout": {"path": str(raw_paths["stdout"].resolve()), "sha256": sha256_bytes(raw_paths["stdout"].read_bytes())},
            "stderr": {"path": str(raw_paths["stderr"].resolve()), "sha256": sha256_bytes(raw_paths["stderr"].read_bytes())},
            "monitor_tsv": {"path": str(raw_paths["monitor_tsv"].resolve()), "sha256": sha256_bytes(raw_paths["monitor_tsv"].read_bytes())},
        },
    ...
    atomic_write(row_dir / "row.json", canonical_bytes(report), "failed row report")
    return report


def run_row(binary: Path, binary_identity: Mapping[str, Any], model: Path, model_identity: Mapping[str, Any], row: Mapping[str, Any], output_dir: Path, sysfs_root: Path, timeout_seconds: float, base_env: Mapping[str, str] | None = None, amd_smi: Path | None = None, gpu_bdf: str | None = None) -> dict[str, Any]:
    ...
    owned_pids = [capture["pid"]] if isinstance(capture.get("pid"), int) else []
    after_process = _safe_process_snapshot(binary, owned_pids, amd_smi=amd_smi, gpu_bdf=gpu_bdf)
    ...
    raw_paths: dict[str, Path] = {"stdout": stdout_path, "stderr": stderr_path, "monitor_tsv": monitor_path}
    if input_token_path is not None:
    ...
    if result is None:
        ...
        return _failure_report(row_dir, row, binary_identity, model_identity, gpu_bdf, command, env, before_process, after_process, capture, baseline, settled, monitor, raw_paths, kind, reason)
    report = {
        "schema_version": ROW_SCHEMA_VERSION, "state": "PASS", "row": row,
        "binary": dict(binary_identity), "model": dict(model_identity), "target": TARGET, "gpu_uuid": GPU_UUID, "gpu_bdf": GPU_BDF,
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    if args.target != TARGET or args.gpu_uuid != GPU_UUID or args.gpu_bdf != GPU_BDF:
        _fail(f"Phase 51 llama producer is restricted to exact {TARGET}/{GPU_UUID}/{GPU_BDF}")
    ...
    reports = [run_row(binary, binary_identity, model, model_identity, row, output_dir, sysfs_root, args.timeout_seconds, amd_smi=amd_smi, gpu_bdf=args.gpu_bdf) for row in matrix()]
    summary_state = "FAIL" if any(report.get("state") == "FAIL" for report in reports) else "PASS"
    summary = {"schema_version": SCHEMA_VERSION, "state": summary_state, "target": TARGET, "gpu_uuid": GPU_UUID, "amd_smi_uuid": GPU_AMD_SMI_UUID, "gpu_bdf": GPU_BDF, "actual_arch": ACTUAL_ARCH, "wavefront_size": WAVEFRONT_SIZE, "rocm_root": ROCM_ROOT, "rocm_source_root": ROCM_SOURCE_ROOT, "rocm_version": ROCM_VERSION, "llama": {"commit": LLAMA_COMMIT, "tag": LLAMA_TAG}, "protocol": {"normal": {"warmups": 3, "measured": 10}, "extended": {"warmups": 1, "measured": 3, "context_length": 131072}, "batch_size": 1, "n_batch": 2048, "n_ubatch": 512, "weights": "BF16", "kv": "F16"}, "matrix": {"cases": [item[0] for item in CASE_SPECS], "row_count": len(reports)}, "binary": binary_identity, "model": model_identity, "rows": reports}
    failures = [
    ...
    summary_path = output_dir / "phase51-mi300x-llama-v1.json"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--gpu-bdf", default=GPU_BDF)

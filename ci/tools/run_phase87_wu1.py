#!/usr/bin/env python3
"""Run the Phase 87 WU1 attention operator matrix on one exact GPU.
...
SOURCE_PATHS = (
    Path("native/hip/tests/phase87_wu1_probe.hip.cpp"),
    Path("native/hip/tests/phase87_wu1_c1.hpp"),
    Path("native/hip/tests/phase87_wu1_c2.hpp"),
    Path("native/hip/tests/phase87_wu1_c3.hpp"),
...
def matrix() -> list[dict[str, int]]:
    cases = []
    for length in (1023, 1024, 1025, 8192, 8193):
    ...
    cases.append({"length": 8256, "m": 1, "pattern": 0, "bench": 1})
...
def validate_rows(path: Path, case: dict[str, int], target: str) -> dict:
    ...
    expected_performance = 4 if case["bench"] else 0
    if len(rows) != 5 + expected_performance + 1:
        raise RuntimeError(f"unexpected row count for {path}: {len(rows)}")
    if len(oracle_rows) != 5 or sorted(row.get("variant") for row in oracle_rows) != list(range(5)):
    ...
    if expected_performance and sorted(row.get("variant") for row in performance_rows) != [1, 2, 3, 4]:
    ...
    for row in performance_rows:
        ...
        if row.get("variant") not in (1, 2, 3, 4):
        ...
        for key in ("control_stage1_ms", "candidate_stage1_ms", "control_total_ms", "candidate_total_ms"):
            value = row.get(key)
            if not isinstance(value, (int, float)) or not math.isfinite(float(value)) or value <= 0:
...
def main() -> int:
    ...
    cases = matrix()
    report = {
        "schema": "phase87-wu1-execution-v1",
        "state": "running",
        "target": args.target,
        "uuid": uuid,
        "binary": str(binary),

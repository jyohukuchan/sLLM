#!/usr/bin/env python3
"""Run the bounded Phase 49 V620 direct-performance matrix.

This producer is intentionally self contained so that it can be copied to the
V620 host together with the release binary and run without model-cache helpers.
...
SCHEMA_VERSION = "phase49-v620-sllm-v1"
ROW_SCHEMA_VERSION = "phase49-v620-sllm-row-v1"
...
def matrix() -> list[dict[str, Any]]:
    ...
    for case_id, input_count, output_count in CASE_SPECS:
        ...
        rows.append(
            {
                    "row_id": f"phase49-v620-sllm-{case_id}",
                    "weight": "bf16",
...
class _SysfsMonitor:
    ...
    def start(self) -> None:
        self.thread = threading.Thread(target=self._run, name="phase49-v620-sllm-sysfs", daemon=True)
        self.thread.start()
...
def _execution_environment(
    ...
    # exact canonical V620 UUID rather than relying on ordinal discovery.
    env["ROCR_VISIBLE_DEVICES"] = gpu_uuid
    env["SLLM_PHASE49_V620_ROW"] = row_id
...
def validate_result(document: Any, row: Mapping[str, Any], weight: str) -> dict[str, Any]:
    ...
    if not isinstance(identities, dict) or identities.get("target") != TARGET or not isinstance(audit, dict) or audit.get("selected_backend") != "hip" or audit.get("target") != TARGET or audit.get("all_dispatches_hip") is not True or audit.get("fallback_used") is not False:
        _fail(f"{row['row_id']}: exact gfx1030/HIP identity is invalid")
...
def run_row(
    ...
    report = {
        ...
        "command": command,
        "environment": {"ROCR_VISIBLE_DEVICES": env.get("ROCR_VISIBLE_DEVICES"), "row": env.get("SLLM_PHASE49_V620_ROW")},
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    if args.target != TARGET:
        _fail("Phase 49 is restricted to exact target gfx1030")
    if args.gpu_uuid != GPU_UUID:
        _fail(f"Phase 49 is restricted to canonical V620 UUID {GPU_UUID}")
    ...
    if output_dir == project_root or project_root in output_dir.parents:
        _fail("Phase 49 raw output must be outside the repository")
    ...
    summary_path = output_dir / "phase49-v620-sllm-v1.json"
    if summary_path.exists():
        existing = _json_load(summary_path.read_bytes(), "existing Phase 49 summary")
        if existing != summary:
            _fail("existing Phase 49 sLLM summary differs; refusing overwrite")
    else:
        _atomic_write(summary_path, canonical_bytes(summary), "Phase 49 sLLM summary")
    return summary
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--gpu-uuid", default=GPU_UUID, help="canonical V620 UUID used for ROCr visibility")

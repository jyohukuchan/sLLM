#!/usr/bin/env python3
"""Aggregate the two bounded Phase 51 MI300X producer summaries.
...
ROCM_VERSION = "7.14.0"
GPU_RUNTIME_UUID = "GPU-6104e2a75685060a"
GPU_AMD_SMI_UUID = "61ff74b5-0000-1000-8004-e2a75685060a"
GPU_UUID = GPU_RUNTIME_UUID
GPU_BDF = "0000:ff:00.0"
SLLM_SCHEMA = "phase51-mi300x-sllm-v1"
LLAMA_SCHEMA = "phase51-mi300x-llama-v1"
SLLM_ROW_SCHEMA = "phase51-mi300x-sllm-row-v1"
LLAMA_ROW_SCHEMA = "phase51-mi300x-llama-row-v1"
SLLM_DIRECT_SCHEMA = "engine-performance-direct-v2"
LLAMA_WRAPPER_SCHEMA = "llama-phase51-mi300x-v1"
...
def input_ids_for(case_id: str) -> list[int]:
    ...
    _fail(f"unknown fixed Phase 51 case: {case_id}")


class Phase51Error(RuntimeError):
    """Malformed, stale, or unsafe Phase 51 evidence."""


def _fail(message: str) -> NoReturn:
    raise Phase51Error(message)
...
def _validate_summary_header(summary: Mapping[str, Any], engine: str) -> None:
    ...
    if summary.get("schema_version") != expected_schema or summary.get("state") not in {"PASS", "FAIL"}:
        _fail(f"{engine}: producer summary is not PASS/FAIL/current schema")
    if summary.get("target") != TARGET or summary.get("gpu_uuid") != GPU_RUNTIME_UUID or summary.get("amd_smi_uuid") != GPU_AMD_SMI_UUID or summary.get("gpu_bdf") != GPU_BDF or summary.get("actual_arch") != ACTUAL_ARCH or summary.get("wavefront_size") != WAVEFRONT_SIZE or summary.get("rocm_root") != ROCM_ROOT or summary.get("rocm_source_root") != ROCM_SOURCE_ROOT or summary.get("rocm_version") != ROCM_VERSION:
        _fail(f"{engine}: exact MI300X target/UUID/BDF identity is invalid")
    if engine == "llama":
    ...
    failed_rows = [row for row in rows if isinstance(row, dict) and row.get("state") == "FAIL"]
    if summary.get("state") == "PASS" and failed_rows:
        _fail(f"{engine}: PASS summary contains failed rows")
    if summary.get("state") == "FAIL":
        failures = summary.get("failures")
        if not failed_rows or isinstance(summary.get("failure_count"), bool) or not isinstance(summary.get("failure_count"), int) or summary["failure_count"] != len(failed_rows) or not isinstance(failures, list) or len(failures) != len(failed_rows):
            _fail(f"{engine}: FAIL summary failure inventory is inconsistent")
        for failure in failures:
            if not isinstance(failure, dict) or not isinstance(failure.get("kind"), str) or not isinstance(failure.get("reason"), str) or not 1 <= len(failure["kind"]) <= 32 or not 1 <= len(failure["reason"]) <= MAX_FAILURE_REASON_CHARS:
                _fail(f"{engine}: FAIL summary reason is malformed")
...
def _failure_record(report: Mapping[str, Any], label: str, expected_binary_sha: str | None, expected_model_sha: str | None, expected_lock_sha: str | None, engine: str, case_id: str, input_count: int, output_count: int) -> dict[str, Any]:
    expected_row_schema = SLLM_ROW_SCHEMA if engine == "sllm" else LLAMA_ROW_SCHEMA
    if report.get("schema_version") != expected_row_schema or report.get("state") != "FAIL":
        _fail(f"{label}: failed row schema/state is invalid")
    ...
    if report.get("target") != TARGET or report.get("gpu_uuid") != GPU_UUID or report.get("gpu_bdf") != GPU_BDF:
        _fail(f"{label}: failed row GPU identity is invalid")
    row = report.get("row")
    expected_row_id = f"phase51-mi300x-{engine}-{case_id}"
    if not isinstance(row, dict) or row.get("row_id") != expected_row_id or row.get("case_id") != case_id or row.get("input_token_count") != input_count or row.get("requested_output_tokens") != output_count:
        _fail(f"{label}: failed row matrix identity is invalid")
    input_ids = _tokens(row.get("input_token_ids"), f"{label} failed row input")
    if input_ids != input_ids_for(case_id):
        _fail(f"{label}: failed row input token sequence differs from matrix")
    failure = report.get("failure")
    if not isinstance(failure, dict) or not isinstance(failure.get("kind"), str) or not isinstance(failure.get("reason"), str) or not 1 <= len(failure["kind"]) <= 32 or not 1 <= len(failure["reason"]) <= MAX_FAILURE_REASON_CHARS:
        _fail(f"{label}: bounded failure kind/reason is absent or malformed")
    if not re.fullmatch(r"[A-Za-z0-9_-]+", failure["kind"]):
        _fail(f"{label}: failure kind contains invalid characters")
    if not isinstance(report.get("command"), list) or not all(isinstance(item, str) and item for item in report["command"]):
        _fail(f"{label}: failed row command evidence is absent")
    if not isinstance(report.get("process"), dict) or not isinstance(report.get("memory"), dict) or not isinstance(report.get("monitor"), dict):
        _fail(f"{label}: failed row process/memory/monitor evidence is absent")
    raw = report.get("raw")
    if not isinstance(raw, dict):
        _fail(f"{label}: failed row raw evidence is absent")
    for key in ("stdout", "stderr", "monitor_tsv"):
        item = raw.get(key)
        if not isinstance(item, dict) or not isinstance(item.get("path"), str) or not item.get("path") or not isinstance(item.get("sha256"), str) or re.fullmatch(r"[0-9a-f]{64}", item["sha256"]) is None:
            _fail(f"{label}: failed row raw {key} evidence is malformed")
    return {"row_id": expected_row_id, "input_ids": input_ids, "failure": {"kind": failure["kind"], "reason": failure["reason"]}, "sample_count": 0}
...
def _validate_engine_specific(result: Mapping[str, Any], report: Mapping[str, Any], engine: str, label: str, row: Mapping[str, Any]) -> None:
    if engine == "sllm":
        ...
        if isinstance(model_identity, dict) and "resolved_revision" in model_identity and model_identity.get("resolved_revision") != SLLM_MODEL_REVISION:
            _fail(f"{label}: sLLM model revision is not the fixed Phase 51 revision")
...
def _row_data(
    ...
    *,
    ...
    if isinstance(report, dict) and report.get("state") == "FAIL":
        return _failure_record(report, label, expected_binary_sha, expected_model_sha, expected_lock_sha, engine, case_id, input_count, output_count)
    if not isinstance(report, dict) or report.get("state") != "PASS":
        _fail(f"{label}: row report is neither PASS nor FAIL")
    _validate_report_identity(report, label, expected_binary_sha, expected_model_sha, expected_lock_sha)
    expected_row_schema = SLLM_ROW_SCHEMA if engine == "sllm" else LLAMA_ROW_SCHEMA
...
def aggregate_summaries(sllm: Mapping[str, Any], llama: Mapping[str, Any], *, sllm_source: str = "<memory>", llama_source: str = "<memory>") -> dict[str, Any]:
    ...
    for case_id in CASE_IDS:
        if sllm_rows[case_id].get("state") == "FAIL":
        ...
        if isinstance(model_identity, dict) and "lock_fingerprint" in model_identity:
            ...
            sllm_lock_fingerprints.append(None)
    known_lock_fingerprints = [value for value in sllm_lock_fingerprints if value is not None]
    if known_lock_fingerprints and len(set(known_lock_fingerprints)) != 1:
    ...
    for case_id, input_count, output_count in CASE_SPECS:
        ...
        left_failed = left.get("failure")
        right_failed = right.get("failure")
        if left_failed is not None or right_failed is not None:
            for metric in METRICS:
                totals[metric] = False
            rows.append({
                "case_id": case_id,
                ...
                "tokens": {
                    ...
                    "generated_sha256": {"sllm": None if left_failed is not None else _sha(left["output_tokens"]), "llama": None if right_failed is not None else _sha(right["output_tokens"])},
                    "visible_sha256": {"sllm": None if left_failed is not None else _sha(left["visible_tokens"]), "llama": None if right_failed is not None else _sha(right["visible_tokens"])},
                    "stop_sha256": {"sllm": None if left_failed is not None else _sha(left["stop"]), "llama": None if right_failed is not None else _sha(right["stop"])},
                    "generated_equal": None,
                    "visible_equal": None,
                    "stop_equal": None,
                },
                "metrics": {"sllm": None if left_failed is not None else {metric: left["stats"][metric] for metric in METRICS}, "llama": None if right_failed is not None else {metric: right["stats"][metric] for metric in METRICS}},
                "gates": {metric: None for metric in METRICS},
                "failures": {"sllm": left_failed, "llama": right_failed},
            })
            continue
    ...
    document = {
        ...
        # Cross-engine parity is a measured target, not a Phase 51 hard gate.
        # Producer PASS already proves every row's correctness, resources,
        # exact HIP path, and cleanup contract.  Keep performance disposition
        # in `gate` without turning a valid slower result into evidence FAIL.
        "state": "PASS" if sllm.get("state") == "PASS" and llama.get("state") == "PASS" else "FAIL",
        "target": TARGET,
        "gpu_uuid": GPU_UUID,
        "amd_smi_uuid": GPU_AMD_SMI_UUID,
        "gpu_bdf": GPU_BDF,
        "actual_arch": ACTUAL_ARCH,
        "wavefront_size": WAVEFRONT_SIZE,
        "rocm_root": ROCM_ROOT,
        "rocm_source_root": ROCM_SOURCE_ROOT,
        ...
        "identities": {
            "sllm": {"schema_version": SLLM_SCHEMA, "engine": "sllm", "backend": "hip", "target": TARGET, "gpu_uuid": GPU_UUID, "amd_smi_uuid": GPU_AMD_SMI_UUID, "gpu_bdf": GPU_BDF, "actual_arch": ACTUAL_ARCH, "wavefront_size": WAVEFRONT_SIZE, "rocm_root": ROCM_ROOT, "rocm_source_root": ROCM_SOURCE_ROOT, "rocm_version": ROCM_VERSION},
            "llama": {"schema_version": LLAMA_SCHEMA, "engine": "llama.cpp", "commit": LLAMA_COMMIT, "tag": LLAMA_TAG, "target": TARGET, "gpu_uuid": GPU_UUID, "amd_smi_uuid": GPU_AMD_SMI_UUID, "gpu_bdf": GPU_BDF, "actual_arch": ACTUAL_ARCH, "wavefront_size": WAVEFRONT_SIZE, "rocm_root": ROCM_ROOT, "rocm_source_root": ROCM_SOURCE_ROOT, "rocm_version": ROCM_VERSION},
        },
    ...
    for row in rows:
        ...
        if isinstance(row_failures, dict):
            for engine in ("sllm", "llama"):
                failure = row_failures.get(engine)
                if isinstance(failure, dict):
                    failures.append({"engine": engine, "case_id": row["case_id"], "row_id": row["row_ids"][engine], "kind": failure["kind"], "reason": failure["reason"]})
    if failures:
        document["failure_count"] = len(failures)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.output_dir is not None:
        output = args.output_dir / "phase51-mi300x-summary-v1.json"
    try:

// Phase 50 R9700 dedicated llama.cpp comparison wrapper.
//
...
// (tag b10453), and the only accepted GPU is the canonical gfx1201 R9700.
...
constexpr const char *kSchema = "llama-phase50-r9700-v1";
constexpr const char *kLlamaCommit = "3cb7ffb1a1f612d5e4a46244ae5a3c77ad934a70";
constexpr const char *kLlamaTag = "b10453";
constexpr const char *kTarget = "gfx1201";
constexpr const char *kGpuUuid = "GPU-a8e9ddefa2d60f55";
...
Options parse_options(int argc, char **argv) {
  ...
  if (spec == nullptr)
    throw std::runtime_error("case is outside the closed Phase 50 set");
  if (options.input.size() != spec->input_tokens ||
      options.max_new_tokens != spec->output_tokens)
    throw std::runtime_error(
        "input or output count does not match the closed Phase 50 recipe");
  ...
  if ((!standard_protocol && !extended_protocol) ||
    ...
    throw std::runtime_error("Phase 50 protocol drifted");
...
void validate_visibility() {
  ...
  if (visible == nullptr || std::string(visible) != kGpuUuid)
    throw std::runtime_error(
        "ROCR_VISIBLE_DEVICES is not the canonical R9700 UUID");
  for (const char *name :
...
void print_help(const char *program) {
  std::cout << "usage: " << program
            ...
            << "  --batch-size 1 --sequences 1 --n-batch 2048 --n-ubatch 512 "
               "--main-gpu 0\n"
            << "  --benchmark-schema-version llama-phase50-r9700-v1\n";
}
...
int main(int argc, char **argv) {
  try {
    ...
    std::cerr << "llama-phase50-r9700: " << error.what() << '\n';
    return 2;
  }
}

#!/usr/bin/env python3
"""Build and run the Phase 87 WU-2V C2 activation-shared V620 probe.

The HIP process owns the independent oracle and fixed AB/BA/AB protocol.  The
runner owns exact UUID selection, Qwen lease checks, build identity, raw output
paths, and fail-closed report validation.
"""
...
SOURCE = Path("native/hip/tests/phase87_stage2_v620_c2_probe.hip.cpp")
HELPER = Path("native/hip/tests/phase87_stage2_v620_c2_kernel.hpp")
...
SOURCE_PATHS = (
    ...
    Path("ci/tools/run_phase87_stage2_v620_c2.py"),
)
...
def validate_case(path: Path, pattern: int, benchmark: bool) -> dict:
    ...
    if identity.get("candidate") != "activation_shared_wave4_4col":
        raise RuntimeError(f"wrong C2 candidate for {path}")
    ...
    if identity.get("shared_bytes") != 12288:
        raise RuntimeError(f"wrong C2 staged activation size for {path}")
...
def main() -> int:
    ...
    binary = output / "phase87-stage2-v620-c2-gfx1030"
    ...
    report = {
        "schema": "phase87-stage2-v620-c2-execution-v1",
        "state": state,
        "target": "gfx1030",
        "uuid": UUID,
    ...
    print(f"Phase87 WU-2V C2 V620: {report['state']} patterns={len(jobs)}/{len(cases)}")

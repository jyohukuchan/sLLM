#!/usr/bin/env python3
"""Run the bounded Phase 51 MI300X direct-performance matrix.

This producer is intentionally self contained so that it can be copied to the
MI300X host together with the release binary and run without model-cache helpers.
...
TARGET = "gfx942"
# Keep logical and physical identities separate: benchmark/provider contracts
# use logical TARGET while the VM reports this feature-qualified arch.
...
# Backwards-compatible local name; reports bind the runtime UUID while the
# separately recorded amd-smi UUID remains available for VM provenance.
GPU_UUID = GPU_RUNTIME_UUID
GPU_BDF = "0000:ff:00.0"
MODEL_SIZE = "4B"
MODEL_REVISION = "851bf6e806efd8d0a36b00ddf55e13ccb7b8cd0a"
SCHEMA_VERSION = "phase51-mi300x-sllm-v1"
ROW_SCHEMA_VERSION = "phase51-mi300x-sllm-row-v1"
...
def matrix() -> list[dict[str, Any]]:
    ...
    for case_id, input_count, output_count in CASE_SPECS:
        ...
        rows.append(
            {
                    "row_id": f"phase51-mi300x-sllm-{case_id}",
                    "weight": "bf16",
...
class _SysfsMonitor:
    ...
    def start(self) -> None:
        self.thread = threading.Thread(target=self._run, name="phase51-mi300x-sllm-sysfs", daemon=True)
        self.thread.start()
...
def _execution_environment(
    ...
    # exact canonical MI300X UUID rather than relying on ordinal discovery.
    env["ROCR_VISIBLE_DEVICES"] = gpu_uuid
    env["SLLM_PHASE51_MI300X_ROW"] = row_id
...
def _failure_report(
    ...
    report: dict[str, Any] = {
        ...
        "environment": {"ROCR_VISIBLE_DEVICES": environment.get("ROCR_VISIBLE_DEVICES"), "row": environment.get("SLLM_PHASE51_MI300X_ROW")},
...
def run_row(
    ...
    report = {
        ...
        "device_index": row["device_index"],
        "command": command,
        "environment": {"ROCR_VISIBLE_DEVICES": env.get("ROCR_VISIBLE_DEVICES"), "row": env.get("SLLM_PHASE51_MI300X_ROW")},
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    if args.target != TARGET:
        _fail("Phase 51 is restricted to exact target gfx942")
    if args.gpu_uuid != GPU_UUID:
        _fail(f"Phase 51 is restricted to canonical MI300X UUID {GPU_UUID}")
    if args.gpu_bdf != GPU_BDF:
        _fail(f"Phase 51 is restricted to canonical MI300X BDF {GPU_BDF}")
    ...
    if output_dir == project_root or project_root in output_dir.parents:
        _fail("Phase 51 raw output must be outside the repository")
    ...
    summary_path = output_dir / "phase51-mi300x-sllm-v1.json"
    if summary_path.exists():
        existing = _json_load(summary_path.read_bytes(), "existing Phase 51 summary")
        if existing != summary:
            _fail("existing Phase 51 sLLM summary differs; refusing overwrite")
    else:
        _atomic_write(summary_path, canonical_bytes(summary), "Phase 51 sLLM summary")
    return summary
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--gpu-bdf", default=GPU_BDF, help="exact MI300X GPU BDF for amd-smi process observation")
    parser.add_argument("--gpu-uuid", default=GPU_UUID, help="canonical MI300X UUID used for ROCr visibility")

#!/usr/bin/env python3
"""Produce the fixed llama.cpp Phase 50 R9700 comparison rows.
...
LLAMA_TAG = "b10453"
SCHEMA_VERSION = "phase50-r9700-llama-v1"
ROW_SCHEMA_VERSION = "phase50-r9700-llama-row-v1"
WRAPPER_SCHEMA_VERSION = "llama-phase50-r9700-v1"
...
def matrix() -> list[dict[str, Any]]:
    ...
    for case_id, input_count, output_count in CASE_SPECS:
        ...
        rows.append({
            "row_id": f"phase50-r9700-llama-{case_id}",
            "case_id": case_id,
            "input_token_ids": ids,
...
class _SysfsMonitor:
    ...
    def start(self) -> None:
        self.thread = threading.Thread(target=self._run, name="phase50-r9700-llama-sysfs", daemon=True)
        self.thread.start()
...
def execution_environment(row_id: str, library_dir: Path, base: Mapping[str, str] | None = None) -> dict[str, str]:
    ...
    env["SLLM_PHASE50_R9700_LLAMA_ROW"] = row_id
    return env
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    if args.target != TARGET or args.gpu_uuid != GPU_UUID or args.gpu_bdf != GPU_BDF:
        _fail(f"Phase 50 llama producer is restricted to exact {TARGET}/{GPU_UUID}/{GPU_BDF}")
    ...
    summary_path = output_dir / "phase50-r9700-llama-v1.json"

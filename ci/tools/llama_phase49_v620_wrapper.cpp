// Phase 49 V620 dedicated llama.cpp comparison wrapper.
//
...
// (tag b10453), and the only accepted GPU is the canonical gfx1030 V620.
...
constexpr const char *kSchema = "llama-phase49-v620-v1";
constexpr const char *kLlamaCommit = "3cb7ffb1a1f612d5e4a46244ae5a3c77ad934a70";
constexpr const char *kLlamaTag = "b10453";
constexpr const char *kTarget = "gfx1030";
constexpr const char *kGpuUuid = "GPU-76a08c022586fed6";
...
Options parse_options(int argc, char **argv) {
  ...
  if (spec == nullptr)
    throw std::runtime_error("case is outside the closed Phase 49 set");
  if (options.input.size() != spec->input_tokens ||
      options.max_new_tokens != spec->output_tokens)
    throw std::runtime_error(
        "input or output count does not match the closed Phase 49 recipe");
  ...
  if ((!standard_protocol && !extended_protocol) ||
    ...
    throw std::runtime_error("Phase 49 protocol drifted");
...
void validate_visibility() {
  ...
  if (visible == nullptr || std::string(visible) != kGpuUuid)
    throw std::runtime_error(
        "ROCR_VISIBLE_DEVICES is not the canonical V620 UUID");
  for (const char *name :
...
void print_help(const char *program) {
  std::cout << "usage: " << program
            ...
            << "  --batch-size 1 --sequences 1 --n-batch 2048 --n-ubatch 512 "
               "--main-gpu 0\n"
            << "  --benchmark-schema-version llama-phase49-v620-v1\n";
}
...
int main(int argc, char **argv) {
  try {
    ...
    std::cerr << "llama-phase49-v620: " << error.what() << '\n';
    return 2;
  }
}

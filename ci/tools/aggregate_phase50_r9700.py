#!/usr/bin/env python3
"""Aggregate the two bounded Phase 50 R9700 producer summaries.
...
SLLM_SCHEMA = "phase50-r9700-sllm-v1"
LLAMA_SCHEMA = "phase50-r9700-llama-v1"
SLLM_ROW_SCHEMA = "phase50-r9700-sllm-row-v1"
LLAMA_ROW_SCHEMA = "phase50-r9700-llama-row-v1"
SLLM_DIRECT_SCHEMA = "engine-performance-direct-v2"
LLAMA_WRAPPER_SCHEMA = "llama-phase50-r9700-v1"
...
def input_ids_for(case_id: str) -> list[int]:
    ...
    _fail(f"unknown fixed Phase 50 case: {case_id}")


class Phase50Error(RuntimeError):
    """Malformed, stale, or unsafe Phase 50 evidence."""


def _fail(message: str) -> NoReturn:
    raise Phase50Error(message)
...
def _validate_summary_header(summary: Mapping[str, Any], engine: str) -> None:
    ...
    if summary.get("target") != TARGET or summary.get("gpu_uuid") != GPU_UUID or summary.get("gpu_bdf") != GPU_BDF:
        _fail(f"{engine}: exact R9700 target/UUID/BDF identity is invalid")
    if engine == "llama":
...
def _failure_record(report: Mapping[str, Any], label: str, expected_binary_sha: str | None, expected_model_sha: str | None, expected_lock_sha: str | None, engine: str, case_id: str, input_count: int, output_count: int) -> dict[str, Any]:
    ...
    row = report.get("row")
    expected_row_id = f"phase50-r9700-{engine}-{case_id}"
...
def _validate_engine_specific(result: Mapping[str, Any], report: Mapping[str, Any], engine: str, label: str, row: Mapping[str, Any]) -> None:
    if engine == "sllm":
        ...
        if isinstance(model_identity, dict) and "resolved_revision" in model_identity and model_identity.get("resolved_revision") != SLLM_MODEL_REVISION:
            _fail(f"{label}: sLLM model revision is not the fixed Phase 50 revision")
...
def aggregate_summaries(sllm: Mapping[str, Any], llama: Mapping[str, Any], *, sllm_source: str = "<memory>", llama_source: str = "<memory>") -> dict[str, Any]:
    ...
    document = {
        ...
        "identities": {
            "sllm": {"schema_version": SLLM_SCHEMA, "engine": "sllm", "backend": "hip", "target": TARGET, "gpu_uuid": GPU_UUID, "gpu_bdf": GPU_BDF},
            "llama": {"schema_version": LLAMA_SCHEMA, "engine": "llama.cpp", "commit": LLAMA_COMMIT, "tag": LLAMA_TAG, "target": TARGET, "gpu_uuid": GPU_UUID, "gpu_bdf": GPU_BDF},
        },
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.output_dir is not None:
        output = args.output_dir / "phase50-r9700-summary-v1.json"
    try:

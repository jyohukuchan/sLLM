#!/usr/bin/env python3
"""Produce the fixed llama.cpp Session D raw comparison rows.
...
LLAMA_TAG = "b10453"
SCHEMA_VERSION = "phase36-session-d-llama-v1"
ROW_SCHEMA_VERSION = "phase36-session-d-llama-row-v1"
WRAPPER_SCHEMA_VERSION = "llama-phase36-session-d-v1"
WARMUPS = 3
MEASURED = 10
...
def input_ids_for(case_id: str) -> list[int]:
    ...
    if case_id in {"32x32", "prefill-long"}:
        count = 32 if case_id == "32x32" else 1024
...
def matrix() -> list[dict[str, Any]]:
    ...
    for case_id, input_count, output_count in CASE_SPECS:
        ...
        rows.append({
            "row_id": f"phase36-d-llama-{case_id}",
            "case_id": case_id,
            "input_token_ids": ids,
...
class _SysfsMonitor:
    ...
    def start(self) -> None:
        self.thread = threading.Thread(target=self._run, name="phase36-session-d-llama-sysfs", daemon=True)
        self.thread.start()
...
def execution_environment(row_id: str, base: Mapping[str, str] | None = None) -> dict[str, str]:
    env = dict(base if base is not None else os.environ)
    for name in VISIBILITY_NAMES:
        env.pop(name, None)
    env["ROCR_VISIBLE_DEVICES"] = GPU_UUID
    env["LD_LIBRARY_PATH"] = "/home/hotaisle/phase36-llama-build-gfx942/bin:/opt/rocm/lib"
    env["SLLM_PHASE36_SESSION_D_LLAMA_ROW"] = row_id
    return env


def expected_command(binary: Path, model: Path, model_sha256: str, row: Mapping[str, Any]) -> list[str]:
    return [
        str(binary), "--model", str(model), "--model-sha256", model_sha256,
        "--row-id", str(row["row_id"]), "--case-id", str(row["case_id"]),
        "--input-token-ids", ",".join(str(value) for value in row["input_token_ids"]),
        "--max-new-tokens", str(row["requested_output_tokens"]),
        "--warmup-requests", str(WARMUPS), "--measured-requests", str(MEASURED),
        "--batch-size", "1", "--sequences", "1", "--n-batch", "10001", "--n-ubatch", "512", "--main-gpu", "0",
...
def validate_result(document: Any, row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    expected_protocol = {"batch_size": 1, "sequences": 1, "warmup_requests": WARMUPS, "measured_requests": MEASURED, "max_new_tokens": row["requested_output_tokens"], "n_ctx": row["input_token_count"] + row["requested_output_tokens"], "n_batch": 10001, "n_ubatch": 512, "n_gpu_layers": -1, "split_mode": "none", "main_gpu": 0, "offload_kqv": True, "op_offload": True, "greedy": True, "stop_token_ids": list(STOP_IDS), "bos_inserted": False}
    ...
    for group, count in (("warmups", WARMUPS), ("measured", MEASURED)):
    ...
    for index, sample in enumerate(all_samples):
        ...
        if not isinstance(generated, list) or not isinstance(visible, list) or len(generated) != row["requested_output_tokens"] or visible != generated or tokens.get("stop_token_ids_fed_back") != [] or tokens.get("bos_inserted") is not False:
            _fail(f"{row['row_id']}: sample {index} token output contract is invalid")
        stop = sample.get("stop")
        if not isinstance(stop, dict) or stop.get("kind") != "max_new_tokens" or stop.get("token_id") is not None:
...
def _resume_row(row_dir: Path, row: Mapping[str, Any], model_sha256: str) -> dict[str, Any] | None:
    ...
    if not isinstance(report, dict) or report.get("state") != "PASS" or report.get("row") != dict(row) or report.get("model", {}).get("sha256") != model_sha256:
...
def run_row(binary: Path, model: Path, model_identity: Mapping[str, Any], row: Mapping[str, Any], output_dir: Path, sysfs_root: Path, timeout_seconds: float, base_env: Mapping[str, str] | None = None) -> dict[str, Any]:
    row = {**row, "model_sha256": model_identity["sha256"]}
    row_dir = output_dir / "raw" / str(row["row_id"])
    existing = _resume_row(row_dir, row, str(model_identity["sha256"]))
    if existing is not None:
        return existing
    row_dir.mkdir(parents=True, exist_ok=False)
    baseline = read_sysfs_memory(sysfs_root)
    before_process = process_snapshot(binary)
    command = expected_command(binary, model, str(model_identity["sha256"]), row)
    monitor = _SysfsMonitor(sysfs_root)
    monitor.start()
    env = execution_environment(str(row["row_id"]), base_env)
    try:
    ...
    report = {
        ...
        "model": dict(model_identity), "target": TARGET, "gpu_uuid": GPU_UUID,
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    if args.target != TARGET or args.gpu_uuid != GPU_UUID:
        _fail("Session D llama producer is restricted to exact gfx942/GPU-1228c84fe776f2f4")
    ...
    reports = [run_row(binary, model, model_identity, row, output_dir, sysfs_root, args.timeout_seconds) for row in matrix()]
    summary = {"schema_version": SCHEMA_VERSION, "state": "PASS", "target": TARGET, "gpu_uuid": GPU_UUID, "llama": {"commit": LLAMA_COMMIT, "tag": LLAMA_TAG}, "protocol": {"warmups": WARMUPS, "measured": MEASURED, "batch_size": 1, "n_batch": 10001, "n_ubatch": 512, "weights": "BF16", "kv": "F16"}, "matrix": {"cases": [item[0] for item in CASE_SPECS], "row_count": len(reports)}, "binary": binary_identity, "model": model_identity, "rows": reports}
    summary_path = output_dir / "phase36-session-d-llama-v1.json"

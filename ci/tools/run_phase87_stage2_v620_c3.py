#!/usr/bin/env python3
"""Build and run the Phase 87 WU-2V V620 FP8 C3 probe.

The probe compares a test-only direct-wave4 pair2 candidate with the
...
SOURCE = Path("native/hip/tests/phase87_stage2_v620_c3_probe.hip.cpp")
...
SOURCE_PATHS = (
    ...
    Path("ci/tools/run_phase87_stage2_v620_c3.py"),
)
...
def validate_case(path: Path, pattern: int, benchmark: bool) -> dict:
    ...
    if identity.get("candidate") != "direct_wave4_pair2_id82_lut":
        raise RuntimeError(f"wrong C3 candidate for {path}")
...
def main() -> int:
    ...
    binary = output / "phase87-stage2-v620-c3-gfx1030"
    ...
    report = {
        "schema": "phase87-stage2-v620-c3-execution-v1",
        "state": state,
        "target": "gfx1030",
        "uuid": UUID,
    ...
    print(f"Phase87 WU-2V V620 C3: {report['state']} patterns={len(jobs)}/{len(cases)}")

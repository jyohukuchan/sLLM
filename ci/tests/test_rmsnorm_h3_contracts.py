...
class RmsNormH3ContractTests(unittest.TestCase):
    ...
    def test_global_workflow_security_and_closed_rows_fail_closed(self) -> None:
        mutations = (
            ...
            (lambda w: self._replace_run(w, 5, "src=$GITHUB_WORKSPACE,dst=/workspace,readonly", "src=$GITHUB_WORKSPACE,dst=/workspace,rw"), "writable source checkout"),
            (lambda w: self._replace_run(w, 5, "src=$RUN_ROOT,dst=/tmp", "src=/tmp,dst=/tmp"), "broad host /tmp mount"),
            (lambda w: self._replace_run(w, 5, "--network none", "--network bridge"), "enabled network"),
            (lambda w: self._replace_run(w, 5, '--mount "type=bind,src=$RUN_ROOT,dst=/tmp"', '--device /dev/kfd\n            --mount "type=bind,src=$RUN_ROOT,dst=/tmp"'), "GPU/device access"),
            (lambda w: self._replace_run(w, 4, 'test "$(git rev-parse HEAD^{tree})" = "$TREE_OID"\n', ""), "missing tree identity check"),
            (lambda w: self._replace_run(w, 7, 'mkdir -p -m 700 "$GITHUB_WORKSPACE/.local-artifacts/rmsnorm-h3-aggregate"', 'mkdir -m 700 "$GITHUB_WORKSPACE/.local-artifacts/rmsnorm-h3-aggregate"'), "missing aggregate parent creation"),
            (lambda w: self._replace_run(w, 7, "$RUN_ROOT/sllm-rmsnorm-h3-aggregate-", "/tmp/sllm-rmsnorm-h3-aggregate-"), "aggregate host/container namespace drift"),
            (lambda w: w["jobs"]["h3-rmsnorm"]["steps"][6].__setitem__("run", "set -eu\n"), "missing gfx1201 row"),
            (lambda w: w["jobs"]["h3-rmsnorm"]["steps"][8]["with"].__setitem__("path", ".local-artifacts/rmsnorm-h3-aggregate/host-bundle-gfx1030.elf\n"), "binary upload"),
        )

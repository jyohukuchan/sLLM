...
TOOL_PATH = ROOT / "ci/tools/aggregate_phase49_v620.py"
SCHEMA_PATH = ROOT / "ci/schema/phase49-v620-summary-v1.schema.json"
spec = importlib.util.spec_from_file_location("aggregate_phase49_v620", TOOL_PATH)
...
def _producer(engine, *, e2e_base=100, llama_e2e_base=None):
    ...
    for row_index, (case_id, input_count, output_count) in enumerate(aggregate_tool.CASE_SPECS):
        ...
        row_id = f"phase49-v620-{engine}-{case_id}"
        row = {
            "row_id": row_id,
            "model_size": "4B",
            "case_id": case_id,
        ...
        rows.append({"schema_version": row_schema, "state": "PASS", "target": aggregate_tool.TARGET, "gpu_uuid": aggregate_tool.GPU_UUID, "weight": "bf16" if engine == "sllm" else None, "binary": {"sha256": binary_sha}, "model": {"sha256": model_sha}, "row": row, "process": {"capture": {"process_group_gone": True}}, "memory": {"baseline": {"hbm_bytes": 10, "gtt_bytes": 20}, "settled": {"hbm_bytes": 10, "gtt_bytes": 20, "settled": True}}, "monitor": {"samples": 1, "errors": []}, "result": result})
...
class Phase49V620SummaryTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_engine_internal_token_mismatch_still_fails_closed(self):
        ...
        with self.assertRaises(aggregate_tool.Phase49Error):

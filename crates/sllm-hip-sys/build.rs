...
fn main() {
    ...
    let moe_route_api = source_dir.join("src/moe_route_api.cpp");
    ...
    let moe_expert_kernel = source_dir.join("src/moe_expert_kernel.hip.cpp");
    let moe_expert_runtime = source_dir.join("src/moe_expert_runtime.inc");
    ...
    for name in [
        "paged_kv_device_layout.hpp",
        "paged_kv_device_pool.hpp",
        "paged_kv_pool_state.hpp",
        "paged_kv_slab_plan.hpp",
        "paged_kv_image_api.hpp",
        "paged_kv_image_api.cpp",
        "paged_kv_image_runtime.inc",
    ] {
        println!(
            "cargo:rerun-if-changed={}",
    ...
    println!("cargo:rerun-if-changed={}", moe_route_api_header.display());
    println!("cargo:rerun-if-changed={}", moe_route_api.display());
    println!(
        "cargo:rerun-if-changed={}",
        moe_route_kernel_internal.display()
    );
    ...
    println!(
        "cargo:rerun-if-changed={}",
        moe_expert_kernel_internal.display()
    );
    println!("cargo:rerun-if-changed={}", moe_expert_kernel.display());
    println!("cargo:rerun-if-changed={}", moe_expert_runtime.display());
    ...
    build
        ...
        .arg("sllm_hip_stub")
        .arg("--parallel")
        .arg(native_build_parallelism(hip_configuration.as_ref()));
    if let Some(configuration) = &hip_configuration {
...
// CI and brokered semantic G1 builds keep the registered serial native build.
// Local builds follow Cargo's job count (`NUM_JOBS`), so `--jobs 1` stays serial.
fn native_build_parallelism(hip_configuration: Option<&HipConfiguration>) -> String {
    let brokered = hip_configuration.is_some_and(|c| c.compiler.client_path.is_some());
    if brokered || semantic_g1_authority_enabled() || env::var_os("GITHUB_ACTIONS").is_some() {
        return "1".into();
    }
    ...
    {
        Some(jobs) if jobs > 0 => jobs.to_string(),
        _ => "1".into(),
    }
}
        ...
        "#[path = {:?}] mod bindings;\n\
         ...
         fn main() {{\n\
            ...
            println!(\"const SLLM_STATUS_INVALID_ROTARY_DESCRIPTOR={{}}\", bindings::SLLM_STATUS_INVALID_ROTARY_DESCRIPTOR);\n\
             ...
             println!(\"const SLLM_HIP_KV_MEMORY_KIND_PAGED={{}}\", bindings::SLLM_HIP_KV_MEMORY_KIND_PAGED);\n\
             println!(\"const SLLM_HIP_KV_PAGED_CREATE_INFO_VERSION={{}}\", bindings::SLLM_HIP_KV_PAGED_CREATE_INFO_VERSION);\n\
             println!(\"const SLLM_HIP_KV_PAGED_VIEW_INFO_VERSION={{}}\", bindings::SLLM_HIP_KV_PAGED_VIEW_INFO_VERSION);\n\
             println!(\"const SLLM_HIP_KV_PAGED_TOKEN_BLOCK_SIZE={{}}\", bindings::SLLM_HIP_KV_PAGED_TOKEN_BLOCK_SIZE);\n\
             println!(\"const SLLM_HIP_KV_PAGED_LAYOUT_VERSION={{}}\", bindings::SLLM_HIP_KV_PAGED_LAYOUT_VERSION);\n\
             println!(\"const SLLM_HIP_KV_PAGED_STATE_FORK_VERSION={{}}\", bindings::SLLM_HIP_KV_PAGED_STATE_FORK_VERSION);\n\
             println!(\"const SLLM_HIP_KV_PAGED_STATE_FORK_INFO_VERSION={{}}\", bindings::SLLM_HIP_KV_PAGED_STATE_FORK_INFO_VERSION);\n\
             ...
             println!(\"const SLLM_HIP_KV_KERNEL_ID_BF16_TO_PAGED_F16_V1={{}}\", bindings::SLLM_HIP_KV_KERNEL_ID_BF16_TO_PAGED_F16_V1);\n\
             println!(\"const SLLM_HIP_KV_KERNEL_ID_BF16_TO_PAGED_MXFP8_E4_V1={{}}\", bindings::SLLM_HIP_KV_KERNEL_ID_BF16_TO_PAGED_MXFP8_E4_V1);\n\
             println!(\"const SLLM_HIP_KV_KERNEL_ID_BF16_TO_PAGED_FP8_E4_V1={{}}\", bindings::SLLM_HIP_KV_KERNEL_ID_BF16_TO_PAGED_FP8_E4_V1);\n\
             println!(\"const SLLM_HIP_KV_KERNEL_ID_BF16_TO_PAGED_FP8_STATIC_E4_V1={{}}\", bindings::SLLM_HIP_KV_KERNEL_ID_BF16_TO_PAGED_FP8_STATIC_E4_V1);\n\
             println!(\"const SLLM_HIP_KV_KERNEL_ID_BF16_TO_PAGED_NVFP4_V1={{}}\", bindings::SLLM_HIP_KV_KERNEL_ID_BF16_TO_PAGED_NVFP4_V1);\n\
             println!(\"const SLLM_HIP_KV_KERNEL_ID_BF16_TO_PAGED_MXFP8_E5_V1={{}}\", bindings::SLLM_HIP_KV_KERNEL_ID_BF16_TO_PAGED_MXFP8_E5_V1);\n\
             ...
             println!(\"const SLLM_HIP_CAUSAL_ATTENTION_KERNEL_ID_PAGED_DECODE_GQA6_V1={{}}\", bindings::SLLM_HIP_CAUSAL_ATTENTION_KERNEL_ID_PAGED_DECODE_GQA6_V1);\n\
             println!(\"const SLLM_HIP_CAUSAL_ATTENTION_KERNEL_ID_PAGED_PREFILL_GQA6_V1={{}}\", bindings::SLLM_HIP_CAUSAL_ATTENTION_KERNEL_ID_PAGED_PREFILL_GQA6_V1);\n\
             println!(\"const SLLM_HIP_CAUSAL_ATTENTION_KERNEL_ID_PAGED_DECODE_FP16_V1={{}}\", bindings::SLLM_HIP_CAUSAL_ATTENTION_KERNEL_ID_PAGED_DECODE_FP16_V1);\n\
             println!(\"const SLLM_HIP_CAUSAL_ATTENTION_KERNEL_ID_PAGED_PREFILL_FP16_V1={{}}\", bindings::SLLM_HIP_CAUSAL_ATTENTION_KERNEL_ID_PAGED_PREFILL_FP16_V1);\n\
             println!(\"const SLLM_HIP_CAUSAL_ATTENTION_KERNEL_ID_PAGED_GENERIC_FORMATS_V1={{}}\", bindings::SLLM_HIP_CAUSAL_ATTENTION_KERNEL_ID_PAGED_GENERIC_FORMATS_V1);\n\
             println!(\"const SLLM_HIP_CAUSAL_ATTENTION_KERNEL_ID_PAGED_SLIDING_STATIC_FP8_V1={{}}\", bindings::SLLM_HIP_CAUSAL_ATTENTION_KERNEL_ID_PAGED_SLIDING_STATIC_FP8_V1);\n\
            ...
            println!(\"const SLLM_HIP_ELEMENTWISE_MAX_ELEMENTS={{}}\", bindings::SLLM_HIP_ELEMENTWISE_MAX_ELEMENTS);\n\
             ...
             println!(\"const SLLM_HIP_MATMUL_KERNEL_ID_SERIAL_ROWS_BF16_FP32_V1={{}}\", bindings::SLLM_HIP_MATMUL_KERNEL_ID_SERIAL_ROWS_BF16_FP32_V1);\n\
            ...
            println!(\"const SLLM_HIP_MOE_ROUTE_VERSION={{}}\", bindings::SLLM_HIP_MOE_ROUTE_VERSION);\n\
             ...
             println!(\"const SLLM_HIP_MOE_ROUTE_DEVICE_SYMBOL_MAX={{}}\", bindings::SLLM_HIP_MOE_ROUTE_DEVICE_SYMBOL_MAX);\n\
             println!(\"const SLLM_HIP_MOE_ROUTE_WORKGROUP_SIZE={{}}\", bindings::SLLM_HIP_MOE_ROUTE_WORKGROUP_SIZE);\n\
             ...
             println!(\"const SLLM_HIP_MOE_EXPERT_HIDDEN_SIZE={{}}\", bindings::SLLM_HIP_MOE_EXPERT_HIDDEN_SIZE);\n\
             println!(\"const SLLM_HIP_MOE_EXPERT_INTERMEDIATE_SIZE={{}}\", bindings::SLLM_HIP_MOE_EXPERT_INTERMEDIATE_SIZE);\n\
             println!(\"const SLLM_HIP_MOE_EXPERT_COUNT={{}}\", bindings::SLLM_HIP_MOE_EXPERT_COUNT);\n\
            ...
            println!(\"layout sllm_elementwise_dispatch_info_t size={{}} align={{}} struct_size={{}} abi_version={{}} info_version={{}} backend={{}} dispatch_id={{}} operation={{}} dispatch_count={{}} kernel_id={{}} workgroup_size_x={{}} grid_size_x={{}} fallback_allowed={{}} fallback_used={{}} element_count={{}} kernel_symbol={{}} device_symbol={{}} gcn_arch_name={{}} reserved={{}}\", size_of::<bindings::sllm_elementwise_dispatch_info_t>(), align_of::<bindings::sllm_elementwise_dispatch_info_t>(), offset_of!(bindings::sllm_elementwise_dispatch_info_t, struct_size), offset_of!(bindings::sllm_elementwise_dispatch_info_t, abi_version), offset_of!(bindings::sllm_elementwise_dispatch_info_t, info_version), offset_of!(bindings::sllm_elementwise_dispatch_info_t, backend), offset_of!(bindings::sllm_elementwise_dispatch_info_t, dispatch_id), offset_of!(bindings::sllm_elementwise_dispatch_info_t, operation), offset_of!(bindings::sllm_elementwise_dispatch_info_t, dispatch_count), offset_of!(bindings::sllm_elementwise_dispatch_info_t, kernel_id), offset_of!(bindings::sllm_elementwise_dispatch_info_t, workgroup_size_x), offset_of!(bindings::sllm_elementwise_dispatch_info_t, grid_size_x), offset_of!(bindings::sllm_elementwise_dispatch_info_t, fallback_allowed), offset_of!(bindings::sllm_elementwise_dispatch_info_t, fallback_used), offset_of!(bindings::sllm_elementwise_dispatch_info_t, element_count), offset_of!(bindings::sllm_elementwise_dispatch_info_t, kernel_symbol), offset_of!(bindings::sllm_elementwise_dispatch_info_t, device_symbol), offset_of!(bindings::sllm_elementwise_dispatch_info_t, gcn_arch_name), offset_of!(bindings::sllm_elementwise_dispatch_info_t, reserved));\n\
             ...
             println!(\"layout sllm_kv_state_create_info_v2_t size={{}} align={{}} struct_size={{}} abi_version={{}} create_info_version={{}} reserved0={{}} session_id={{}} layer_id={{}} flags={{}} capacity_tokens={{}} head_count={{}} head_dim={{}} memory_kind={{}} layout={{}} dtype={{}} encoding={{}} block_size={{}} scale_dtype={{}} reserved={{}}\", size_of::<bindings::sllm_kv_state_create_info_v2_t>(), align_of::<bindings::sllm_kv_state_create_info_v2_t>(), offset_of!(bindings::sllm_kv_state_create_info_v2_t, struct_size), offset_of!(bindings::sllm_kv_state_create_info_v2_t, abi_version), offset_of!(bindings::sllm_kv_state_create_info_v2_t, create_info_version), offset_of!(bindings::sllm_kv_state_create_info_v2_t, reserved0), offset_of!(bindings::sllm_kv_state_create_info_v2_t, session_id), offset_of!(bindings::sllm_kv_state_create_info_v2_t, layer_id), offset_of!(bindings::sllm_kv_state_create_info_v2_t, flags), offset_of!(bindings::sllm_kv_state_create_info_v2_t, capacity_tokens), offset_of!(bindings::sllm_kv_state_create_info_v2_t, head_count), offset_of!(bindings::sllm_kv_state_create_info_v2_t, head_dim), offset_of!(bindings::sllm_kv_state_create_info_v2_t, memory_kind), offset_of!(bindings::sllm_kv_state_create_info_v2_t, layout), offset_of!(bindings::sllm_kv_state_create_info_v2_t, dtype), offset_of!(bindings::sllm_kv_state_create_info_v2_t, encoding), offset_of!(bindings::sllm_kv_state_create_info_v2_t, block_size), offset_of!(bindings::sllm_kv_state_create_info_v2_t, scale_dtype), offset_of!(bindings::sllm_kv_state_create_info_v2_t, reserved));\n\
             println!(\"layout sllm_kv_state_paged_create_info_t size={{}} align={{}} struct_size={{}} abi_version={{}} create_info_version={{}} reserved0={{}} session_id={{}} layer_id={{}} flags={{}} capacity_tokens={{}} head_count={{}} head_dim={{}} memory_kind={{}} layout={{}} dtype={{}} encoding={{}} scale_dtype={{}} quantization_block_size={{}} token_block_size={{}} physical_layout_version={{}} logical_table_capacity={{}} max_physical_blocks={{}} sliding_window_tokens={{}} static_key_scale_bits={{}} static_value_scale_bits={{}} reserved={{}}\", size_of::<bindings::sllm_kv_state_paged_create_info_t>(), align_of::<bindings::sllm_kv_state_paged_create_info_t>(), offset_of!(bindings::sllm_kv_state_paged_create_info_t, struct_size), offset_of!(bindings::sllm_kv_state_paged_create_info_t, abi_version), offset_of!(bindings::sllm_kv_state_paged_create_info_t, create_info_version), offset_of!(bindings::sllm_kv_state_paged_create_info_t, reserved0), offset_of!(bindings::sllm_kv_state_paged_create_info_t, session_id), offset_of!(bindings::sllm_kv_state_paged_create_info_t, layer_id), offset_of!(bindings::sllm_kv_state_paged_create_info_t, flags), offset_of!(bindings::sllm_kv_state_paged_create_info_t, capacity_tokens), offset_of!(bindings::sllm_kv_state_paged_create_info_t, head_count), offset_of!(bindings::sllm_kv_state_paged_create_info_t, head_dim), offset_of!(bindings::sllm_kv_state_paged_create_info_t, memory_kind), offset_of!(bindings::sllm_kv_state_paged_create_info_t, layout), offset_of!(bindings::sllm_kv_state_paged_create_info_t, dtype), offset_of!(bindings::sllm_kv_state_paged_create_info_t, encoding), offset_of!(bindings::sllm_kv_state_paged_create_info_t, scale_dtype), offset_of!(bindings::sllm_kv_state_paged_create_info_t, quantization_block_size), offset_of!(bindings::sllm_kv_state_paged_create_info_t, token_block_size), offset_of!(bindings::sllm_kv_state_paged_create_info_t, physical_layout_version), offset_of!(bindings::sllm_kv_state_paged_create_info_t, logical_table_capacity), offset_of!(bindings::sllm_kv_state_paged_create_info_t, max_physical_blocks), offset_of!(bindings::sllm_kv_state_paged_create_info_t, sliding_window_tokens), offset_of!(bindings::sllm_kv_state_paged_create_info_t, static_key_scale_bits), offset_of!(bindings::sllm_kv_state_paged_create_info_t, static_value_scale_bits), offset_of!(bindings::sllm_kv_state_paged_create_info_t, reserved));\n\
             ...
             println!(\"layout sllm_kv_paged_view_info_t size={{}} align={{}} struct_size={{}} abi_version={{}} info_version={{}} reserved0={{}} session_id={{}} layer_id={{}} dtype={{}} encoding={{}} head_count={{}} head_dim={{}} memory_kind={{}} layout={{}} token_block_size={{}} physical_layout_version={{}} reserved1={{}} capacity_tokens={{}} observed_length={{}} generation={{}} logical_table_capacity={{}} max_physical_blocks={{}} allocated_physical_blocks={{}} committed_bytes_per_plane={{}} committed_bytes_total={{}} context_identity={{}} state_identity={{}} reserved={{}}\", size_of::<bindings::sllm_kv_paged_view_info_t>(), align_of::<bindings::sllm_kv_paged_view_info_t>(), offset_of!(bindings::sllm_kv_paged_view_info_t, struct_size), offset_of!(bindings::sllm_kv_paged_view_info_t, abi_version), offset_of!(bindings::sllm_kv_paged_view_info_t, info_version), offset_of!(bindings::sllm_kv_paged_view_info_t, reserved0), offset_of!(bindings::sllm_kv_paged_view_info_t, session_id), offset_of!(bindings::sllm_kv_paged_view_info_t, layer_id), offset_of!(bindings::sllm_kv_paged_view_info_t, dtype), offset_of!(bindings::sllm_kv_paged_view_info_t, encoding), offset_of!(bindings::sllm_kv_paged_view_info_t, head_count), offset_of!(bindings::sllm_kv_paged_view_info_t, head_dim), offset_of!(bindings::sllm_kv_paged_view_info_t, memory_kind), offset_of!(bindings::sllm_kv_paged_view_info_t, layout), offset_of!(bindings::sllm_kv_paged_view_info_t, token_block_size), offset_of!(bindings::sllm_kv_paged_view_info_t, physical_layout_version), offset_of!(bindings::sllm_kv_paged_view_info_t, reserved1), offset_of!(bindings::sllm_kv_paged_view_info_t, capacity_tokens), offset_of!(bindings::sllm_kv_paged_view_info_t, observed_length), offset_of!(bindings::sllm_kv_paged_view_info_t, generation), offset_of!(bindings::sllm_kv_paged_view_info_t, logical_table_capacity), offset_of!(bindings::sllm_kv_paged_view_info_t, max_physical_blocks), offset_of!(bindings::sllm_kv_paged_view_info_t, allocated_physical_blocks), offset_of!(bindings::sllm_kv_paged_view_info_t, committed_bytes_per_plane), offset_of!(bindings::sllm_kv_paged_view_info_t, committed_bytes_total), offset_of!(bindings::sllm_kv_paged_view_info_t, context_identity), offset_of!(bindings::sllm_kv_paged_view_info_t, state_identity), offset_of!(bindings::sllm_kv_paged_view_info_t, reserved));\n\
             ...
             println!(\"layout sllm_buffer_copy_d2d_desc_t size={{}} align={{}} struct_size={{}} abi_version={{}} source_offset_bytes={{}} destination_offset_bytes={{}} size_bytes={{}} reserved={{}}\", size_of::<bindings::sllm_buffer_copy_d2d_desc_t>(), align_of::<bindings::sllm_buffer_copy_d2d_desc_t>(), offset_of!(bindings::sllm_buffer_copy_d2d_desc_t, struct_size), offset_of!(bindings::sllm_buffer_copy_d2d_desc_t, abi_version), offset_of!(bindings::sllm_buffer_copy_d2d_desc_t, source_offset_bytes), offset_of!(bindings::sllm_buffer_copy_d2d_desc_t, destination_offset_bytes), offset_of!(bindings::sllm_buffer_copy_d2d_desc_t, size_bytes), offset_of!(bindings::sllm_buffer_copy_d2d_desc_t, reserved));\n\
             ...
             println!(\"layout sllm_kv_paged_state_fork_info_t size={{}} align={{}} struct_size={{}} abi_version={{}} info_version={{}} reserved0={{}} source_state_identity={{}} child_state_identity={{}} source_owned_bytes={{}} child_owned_bytes={{}} copied_bytes={{}} shared_bytes={{}} published_length={{}} token_block_size={{}} physical_layout_version={{}} source_logical_table_capacity={{}} child_logical_table_capacity={{}} source_physical_blocks={{}} child_physical_blocks={{}} copied_physical_blocks={{}} shared_physical_blocks={{}} committed_bytes_total={{}} reserved={{}}\", size_of::<bindings::sllm_kv_paged_state_fork_info_t>(), align_of::<bindings::sllm_kv_paged_state_fork_info_t>(), offset_of!(bindings::sllm_kv_paged_state_fork_info_t, struct_size), offset_of!(bindings::sllm_kv_paged_state_fork_info_t, abi_version), offset_of!(bindings::sllm_kv_paged_state_fork_info_t, info_version), offset_of!(bindings::sllm_kv_paged_state_fork_info_t, reserved0), offset_of!(bindings::sllm_kv_paged_state_fork_info_t, source_state_identity), offset_of!(bindings::sllm_kv_paged_state_fork_info_t, child_state_identity), offset_of!(bindings::sllm_kv_paged_state_fork_info_t, source_owned_bytes), offset_of!(bindings::sllm_kv_paged_state_fork_info_t, child_owned_bytes), offset_of!(bindings::sllm_kv_paged_state_fork_info_t, copied_bytes), offset_of!(bindings::sllm_kv_paged_state_fork_info_t, shared_bytes), offset_of!(bindings::sllm_kv_paged_state_fork_info_t, published_length), offset_of!(bindings::sllm_kv_paged_state_fork_info_t, token_block_size), offset_of!(bindings::sllm_kv_paged_state_fork_info_t, physical_layout_version), offset_of!(bindings::sllm_kv_paged_state_fork_info_t, source_logical_table_capacity), offset_of!(bindings::sllm_kv_paged_state_fork_info_t, child_logical_table_capacity), offset_of!(bindings::sllm_kv_paged_state_fork_info_t, source_physical_blocks), offset_of!(bindings::sllm_kv_paged_state_fork_info_t, child_physical_blocks), offset_of!(bindings::sllm_kv_paged_state_fork_info_t, copied_physical_blocks), offset_of!(bindings::sllm_kv_paged_state_fork_info_t, shared_physical_blocks), offset_of!(bindings::sllm_kv_paged_state_fork_info_t, committed_bytes_total), offset_of!(bindings::sllm_kv_paged_state_fork_info_t, reserved));\n\
             ...
             println!(\"layout sllm_kv_paged_image_info_t size={{}} align={{}} struct_size={{}} abi_version={{}} image_version={{}} flags={{}} session_id={{}} layer_id={{}} dtype={{}} encoding={{}} head_count={{}} head_dim={{}} layout={{}} token_block_size={{}} physical_layout_version={{}} capacity_tokens={{}} published_length={{}} generation={{}} retained_start={{}} retained_length={{}} sliding_window_tokens={{}} logical_table_capacity={{}} physical_block_count={{}} plane_count={{}} ring_slot_count={{}} table_entry_width={{}} plane_block_stride={{}} plane_bytes={{}} static_key_scale_bits={{}} static_value_scale_bits={{}} reserved={{}}\", size_of::<bindings::sllm_kv_paged_image_info_t>(), align_of::<bindings::sllm_kv_paged_image_info_t>(), offset_of!(bindings::sllm_kv_paged_image_info_t, struct_size), offset_of!(bindings::sllm_kv_paged_image_info_t, abi_version), offset_of!(bindings::sllm_kv_paged_image_info_t, image_version), offset_of!(bindings::sllm_kv_paged_image_info_t, flags), offset_of!(bindings::sllm_kv_paged_image_info_t, session_id), offset_of!(bindings::sllm_kv_paged_image_info_t, layer_id), offset_of!(bindings::sllm_kv_paged_image_info_t, dtype), offset_of!(bindings::sllm_kv_paged_image_info_t, encoding), offset_of!(bindings::sllm_kv_paged_image_info_t, head_count), offset_of!(bindings::sllm_kv_paged_image_info_t, head_dim), offset_of!(bindings::sllm_kv_paged_image_info_t, layout), offset_of!(bindings::sllm_kv_paged_image_info_t, token_block_size), offset_of!(bindings::sllm_kv_paged_image_info_t, physical_layout_version), offset_of!(bindings::sllm_kv_paged_image_info_t, capacity_tokens), offset_of!(bindings::sllm_kv_paged_image_info_t, published_length), offset_of!(bindings::sllm_kv_paged_image_info_t, generation), offset_of!(bindings::sllm_kv_paged_image_info_t, retained_start), offset_of!(bindings::sllm_kv_paged_image_info_t, retained_length), offset_of!(bindings::sllm_kv_paged_image_info_t, sliding_window_tokens), offset_of!(bindings::sllm_kv_paged_image_info_t, logical_table_capacity), offset_of!(bindings::sllm_kv_paged_image_info_t, physical_block_count), offset_of!(bindings::sllm_kv_paged_image_info_t, plane_count), offset_of!(bindings::sllm_kv_paged_image_info_t, ring_slot_count), offset_of!(bindings::sllm_kv_paged_image_info_t, table_entry_width), offset_of!(bindings::sllm_kv_paged_image_info_t, plane_block_stride), offset_of!(bindings::sllm_kv_paged_image_info_t, plane_bytes), offset_of!(bindings::sllm_kv_paged_image_info_t, static_key_scale_bits), offset_of!(bindings::sllm_kv_paged_image_info_t, static_value_scale_bits), offset_of!(bindings::sllm_kv_paged_image_info_t, reserved));\n\
             println!(\"layout sllm_kv_paged_image_chunk_t size={{}} align={{}} struct_size={{}} abi_version={{}} image_version={{}} section={{}} plane={{}} byte_offset={{}} byte_length={{}} host_pointer={{}} host_capacity={{}} reserved={{}}\", size_of::<bindings::sllm_kv_paged_image_chunk_t>(), align_of::<bindings::sllm_kv_paged_image_chunk_t>(), offset_of!(bindings::sllm_kv_paged_image_chunk_t, struct_size), offset_of!(bindings::sllm_kv_paged_image_chunk_t, abi_version), offset_of!(bindings::sllm_kv_paged_image_chunk_t, image_version), offset_of!(bindings::sllm_kv_paged_image_chunk_t, section), offset_of!(bindings::sllm_kv_paged_image_chunk_t, plane), offset_of!(bindings::sllm_kv_paged_image_chunk_t, byte_offset), offset_of!(bindings::sllm_kv_paged_image_chunk_t, byte_length), offset_of!(bindings::sllm_kv_paged_image_chunk_t, host_pointer), offset_of!(bindings::sllm_kv_paged_image_chunk_t, host_capacity), offset_of!(bindings::sllm_kv_paged_image_chunk_t, reserved));\n\
         }}\n",

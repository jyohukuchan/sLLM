...
#[test]
fn gemma_sliding_batched_reader_enforces_future_and_window_lower_bounds() {
    const WIDTH: usize = 256;
    // Snapshot logical positions are 1..=513.  Query 512 may not read
    // 513 (future); query 513 may not read 1 (513 - 512).
    let mut context = RuntimeContext::create(0).unwrap();
    let mut stream = context.create_stream().unwrap();
    let q_values = vec![0.0_f32; 2 * WIDTH];
    let mut values = vec![0.0_f32; 513 * WIDTH];
    values[0] = 3.0; // logical key 1: visible to row 0 only.
    values[512 * WIDTH] = 7.0; // logical key 513: visible to row 1 only.
    let keys = vec![0.0_f32; values.len()];
    let mut q = context.alloc_buffer(q_values.len() * 4).unwrap();
    let mut key = context.alloc_buffer(keys.len() * 4).unwrap();
    let mut value = context.alloc_buffer(values.len() * 4).unwrap();
    let mut output = context.alloc_buffer(q_values.len() * 4).unwrap();
    q.copy_from_host(0, &f32s_to_le_bytes(&q_values), Some(&mut stream))
        .unwrap();
    key.copy_from_host(0, &f32s_to_le_bytes(&keys), Some(&mut stream))
        .unwrap();
    value
    ...
    let run =
        |value: &mut RuntimeBuffer, output: &mut RuntimeBuffer, mut stream: &mut RuntimeStream| {
            ...
            .unwrap();
            let mut bytes = vec![0_u8; q_values.len() * 4];
            output
    ...
    let baseline = run(&mut value, &mut output, &mut stream);

    let mut future = values.clone();
    future[512 * WIDTH] = 1000.0;
    value
        .copy_from_host(0, &f32s_to_le_bytes(&future), Some(&mut stream))
        .unwrap();
    let future_changed = run(&mut value, &mut output, &mut stream);
    assert_eq!(
        &baseline[..WIDTH],
        &future_changed[..WIDTH],
        "query 512 read future key 513"
    );
    assert_ne!(
        baseline[WIDTH], future_changed[WIDTH],
        "future fixture did not affect its legal row"
    );

    let mut lower = values;
    lower[0] = 1000.0;
    value
        .copy_from_host(0, &f32s_to_le_bytes(&lower), Some(&mut stream))
        .unwrap();
    let lower_changed = run(&mut value, &mut output, &mut stream);
    assert_eq!(
        &baseline[WIDTH..],
        &lower_changed[WIDTH..],
        "query 513 read key 1 = query - 512"
    );
    assert_ne!(
        baseline[0], lower_changed[0],
        "lower-bound fixture did not affect its legal row"
    );
}
...
fn hip_paged_decode_attn_f32_wave_softmax_long_context_matches_cpu_when_available() {
    ...
    for device_index in hip_devices {
        ...
        gate.copy_from_host(0, &gate_bytes, Some(&mut stream))
            .unwrap();
        k_cache
    ...
    for device_index in hip_devices {
        ...
        for cache_len in [1_usize, 255, 256, 257, 513, 1339] {
            for source_tile in [128_usize, 256] {
                assert_paged_decode_split_matches_expected(&mut context, cache_len, source_tile);
            }
        }
    }
}
...
fn first_hip_paged_decode_attn_split_f32_rejects_cpu_workspace_when_available() {
    ...
    .unwrap_err();
    assert!(
        error.contains("different backends") || error.contains("devices"),
        "{error}"
    );
}
...
fn first_hip_paged_kv_write_f32_writes_expected_physical_slot_when_available() {
    ...
    let physical_timestep = block_table_values[cache_position / block_size] as usize * block_size
...
fn run_linear_attn_recurrent_fixture(
    ...
    let mut gate_buffer = context
        .alloc_buffer(fixture.gate.len() * f32_bytes)
        .unwrap();
    let mut beta_buffer = context
        .alloc_buffer(fixture.beta.len() * f32_bytes)
        .unwrap();
...
fn hip_linear_attn_recurrent_production_model_shapes_match_cpu_when_enabled() {
    ...
    for &(case, key_heads, value_heads, sequence_len) in &[
        ...
        let fixture =
            linear_attn_recurrent_fixture(key_heads, value_heads, sequence_len, KEY_DIM, VALUE_DIM);
        let expected = {
...
fn first_hip_linear_attn_qkv_prepare_f32_computes_expected_values_when_available() {
    ...
    let qkv_values: Vec<f32> = (0..channels)
        ...
        .collect();
    let conv_weight_values: Vec<f32> = (0..channels).flat_map(|_| [0.25_f32, 0.5, 1.0]).collect();
...
fn first_hip_paged_chunk_invalid_table_fails_closed_when_available() {
    ...
    table
        ...
        .unwrap();
    k_cache
        .copy_from_host(0, &f32s_to_le_bytes(&cache_k), Some(&mut stream))
        .unwrap();
    v_cache
        .copy_from_host(0, &f32s_to_le_bytes(&cache_v), Some(&mut stream))
        .unwrap();
    output
        .copy_from_host(
            0,
            &f32s_to_le_bytes(&vec![42.0_f32; m * q_heads * value_dim]),
            Some(&mut stream),
        )
        .unwrap();
    stream.synchronize().unwrap();
    assert!(
        paged_kv_write_chunk_f32(
            &k,
            &v,
            &table,
            0,
    ...
    assert_eq!(
        le_bytes_to_f32s(&output_bytes),
        vec![42.0_f32; m * q_heads * value_dim]
    );
}

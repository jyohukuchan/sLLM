...
mod tests {
    ...
    #[test]
    fn typed_f16_and_fp8_paged_caches_span_full_4096_token_context_cpu() {
        ...
        // per-token/head FP8 scale slots. It is a storage/read correctness
        // test, not a substitute for long-context generated-text review.
        let shape = PagedDecodeShape {
            block_size: 16,
            cache_blocks: 256,
        ...
        for dtype in [KvCacheDtype::F16, KvCacheDtype::Fp8E4M3Fn] {
            let mut context = RuntimeContext::create(0).unwrap();
            let mut stream = context.create_stream().unwrap();
            let mut state = PagedDecodeState::new_with_kv_cache_dtypes(
                &mut context,

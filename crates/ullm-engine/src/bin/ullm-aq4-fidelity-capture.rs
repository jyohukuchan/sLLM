...
struct Args {
    ...
    sq8_artifact_dir: Option<PathBuf>,
    sq8_binding_manifest: Option<PathBuf>,
    sq8_expected_binding_manifest_sha256: Option<String>,
    sq8_expected_content_sha256: Option<String>,
    sq8_source_model_dir: Option<PathBuf>,
    sq8_row_chunk: usize,
}
...
impl<'a> CaptureObserver<'a> {
    fn new(
        ...
        let hidden_offset = hidden
            .stream_position()
            .map_err(|e| format!("hidden offset: {e}"))?;
        let logits_offset = logits
            .stream_position()
            .map_err(|e| format!("logits offset: {e}"))?;
        Ok(Self {
            hidden,
            logits,
            hidden_offset,
            ...
            expected_logits: VOCAB_SIZE,
            hidden_seen: 0,
            logits_seen: 0,
            hidden_hash: Sha256::new(),
            logits_hash: Sha256::new(),
            hidden_nonfinite: 0,
            logits_nonfinite: 0,
    ...
    fn finish_row(
        ...
        semantic: String,
        observation: String,
    ) -> Result<VectorRow, String> {
        if self.hidden_seen != self.expected_hidden
            || self.logits_seen != self.expected_logits
            || self.topk.len() != TOP_K
        {
        ...
        let greedy = self
            .topk
            .first()
            .ok_or("calibration observer top-k is empty")?
            .token_id;
        Ok(VectorRow {
            case_id: case.case_id.clone(),
            step: 0,
            semantic_input_id: semantic,
            observation,
            input_token_ids_sha256: case.context_token_ids_sha256.clone(),
            hidden: VectorRef {
                ...
                dtype: "f32",
                endianness: "little",
                sha256: format!("{:x}", self.hidden_hash.finalize()),
                nonfinite_count: self.hidden_nonfinite,
            },
            logits: VectorRef {
                ...
                dtype: "f32",
                endianness: "little",
                sha256: format!("{:x}", self.logits_hash.finalize()),
                nonfinite_count: self.logits_nonfinite,
            },
            greedy_token_id: greedy,
            topk: self.topk,
            finite: self.hidden_nonfinite == 0 && self.logits_nonfinite == 0,
        })
    }
}
...
impl Qwen35Aq4CalibrationObserver for CaptureObserver<'_> {
    fn begin(&mut self, hidden_elements: usize, logit_elements: usize) -> Result<(), String> {
        if hidden_elements != self.expected_hidden || logit_elements != self.expected_logits {
            return Err("runtime calibration shape differs".into());
        }
        Ok(())
    }
    fn observe_hidden_chunk(&mut self, start: usize, values: &[f32]) -> Result<(), String> {
        if start != self.hidden_seen || self.hidden_seen + values.len() > self.expected_hidden {
            return Err("hidden chunks are not contiguous".into());
        }
        if values.iter().any(|value| !value.is_finite()) {
            return Err("non-finite hidden value observed".into());
        }
    ...
    fn observe_logit_chunk(&mut self, start: usize, values: &[f32]) -> Result<(), String> {
        if start != self.logits_seen || self.logits_seen + values.len() > self.expected_logits {
            return Err("logit chunks are not contiguous".into());
        }
        if values.iter().any(|value| !value.is_finite()) {
            return Err("non-finite logit value observed".into());
        }
    ...
    fn finish(&mut self) -> Result<(), String> {
        if self.hidden_seen != self.expected_hidden || self.logits_seen != self.expected_logits {
            return Err("runtime calibration observer ended short".into());
        }
        Ok(())
    }
}
...
fn parse_strict_json(bytes: &[u8], label: &str) -> Result<Value, String> {
    ...
    deserializer
        .end()
        .map_err(|e| format!("{label} trailing JSON: {e}"))?;
    Ok(value)
}
...
fn aggregate_source_files(
    ...
    let entries = ordered
        .iter()
        .map(|name| source_file_identity(root, name, &format!("{label} {name}")))
        .collect::<Result<Vec<_>, _>>()?;
    let canonical =
        serde_json::to_vec(&entries).map_err(|e| format!("{label} canonical identity: {e}"))?;
...
fn validate_policy(policy: &Value) -> Result<(), String> {
    if policy.get("schema_version").and_then(Value::as_str) != Some(POLICY_SCHEMA)
        || policy.get("status").and_then(Value::as_str) != Some("formula_frozen_unbound")
        || policy.get("promotion_eligible").and_then(Value::as_bool) != Some(false)
    {
        return Err("fidelity policy schema/status differs".into());
    }
    for field in [
        "attempt2_threshold_source_forbidden",
        "observed_attempt2_values_forbidden",
        "calibration_subset_only_for_active_bf16_envelope",
        "holdout_evaluation_allowed_once",
    ] {
        if policy.get(field).and_then(Value::as_bool) != Some(true) {
            return Err(format!("fidelity policy safety flag differs: {field}"));
        }
    }
    let metrics = policy
        .get("metrics")
        .and_then(Value::as_object)
        .ok_or("fidelity policy metrics are missing")?;
    if metrics.len() != POLICY_METRICS.len() {
        return Err("fidelity policy metric set differs".into());
    }
    for (name, role, direction, aggregation, formula) in POLICY_METRICS {
        let item = metrics
            .get(*name)
            .and_then(Value::as_object)
            .ok_or_else(|| format!("fidelity policy metric missing: {name}"))?;
        if item.get("role").and_then(Value::as_str) != Some(*role)
            || item.get("direction").and_then(Value::as_str) != Some(*direction)
            || item.get("aggregation").and_then(Value::as_str) != Some(*aggregation)
            || item.get("formula").and_then(Value::as_str) != Some(*formula)
            || item.get("sample_minimum").and_then(Value::as_u64) != Some(MAX_ROWS as u64)
        {
            return Err(format!("fidelity policy metric contract differs: {name}"));
        }
    }
    if policy
        .get("quality_task")
        ...
        != Some("binary_retention_rate")
        || policy
            .get("quality_task")
            .and_then(|value| value.get("score"))
            .and_then(Value::as_str)
            != Some("bf16_top1_retained_in_aq4_top10")
    {
        return Err("fidelity policy quality task differs".into());
    }
    if policy
        .get("relative_l2_rejection")
        ...
        || policy
            ...
            .and_then(Value::as_str)
            != Some("reject any observed relative-L2 > 1 before aggregation")
    {
        return Err("fidelity policy relative-L2 rejection differs".into());
    }
    Ok(())
}

fn source_provenance(package_manifest: &Value) -> Result<SourceProvenance, String> {
    let raw_root = package_manifest
        .get("source_model_dir")
        .and_then(Value::as_str)
        .ok_or("package source_model_dir is missing")?;
    let root = PathBuf::from(raw_root);
    let root_meta =
        fs::symlink_metadata(&root).map_err(|e| format!("package source model metadata: {e}"))?;
    ...
    let dtype = config
        ...
        .and_then(Value::as_str)
        .ok_or("source config dtype is missing")?;
    let index = read_json(
    ...
    let weight_map = index
        .get("weight_map")
        .and_then(Value::as_object)
        .ok_or("source weight index has no weight_map")?;
    let mut names = vec![
    ...
    names.extend(
        weight_map
            ...
            .map(|value| {
                value
                    .as_str()
                    .ok_or("source weight index shard is invalid")
                    .map(str::to_string)
            })
            .collect::<Result<Vec<_>, _>>()?,
    );
    let (checkpoint_files, checkpoint_sha) =
        aggregate_source_files(&root, &names, "source checkpoint")?;
    let tokenizer_names = [
        "chat_template.jinja",
        "merges.txt",
        "tokenizer.json",
    ...
    .map(str::to_string)
    .collect::<Vec<_>>();
    let (tokenizer_files, tokenizer_sha) =
        aggregate_source_files(&root, &tokenizer_names, "source tokenizer")?;
    let metadata_dir = root.join(".cache/huggingface/download");
    let mut revisions = BTreeSet::new();
    for entry in
        fs::read_dir(&metadata_dir).map_err(|e| format!("source revision metadata: {e}"))?
    {
        let path = entry
            .map_err(|e| format!("source revision metadata entry: {e}"))?
            .path();
        if path.extension().and_then(|value| value.to_str()) != Some("metadata") {
            continue;
        }
        let metadata =
            fs::read_to_string(&path).map_err(|e| format!("source revision metadata read: {e}"))?;
        if let Some(first) = metadata.lines().next().filter(|line| !line.is_empty()) {
            revisions.insert(first.to_string());
        }
    }
    if revisions.len() != 1 {
        return Err("source model revision metadata is missing or ambiguous".into());
    }
    let model_revision = revisions
        .into_iter()
        .next()
        .ok_or("source model revision is missing")?;
    Ok(SourceProvenance {
        model_revision,
...
fn validate_supported_identity(
    ...
    let sq8 = implementation_id == "qwen35_aq4_sq8_linear_qkv_z_overlay_v1";
    let aq4 = implementation_id == "qwen35_aq4_rdna4_v1";
    if format_id != "AQ4_0"
        || (!aq4 && !sq8)
        || device != "gfx1201"
        || (aq4 && execution_profile != "rdna4_aq4_resident")
        || (sq8 && execution_profile != "rdna4_aq4_resident_sq8_linear_qkv_z_overlay")
    {
        return Err("served model is not a supported AQ4/SQ8 RDNA4 active identity".into());
    }
    Ok(sq8)
}
...
fn load_args() -> Result<Args, String> {
    ...
    let take = |name: &str, map: &mut BTreeMap<String, String>| {
        map.remove(name)
    ...
    let args = Args {
        ...
        device_index: map
            ...
            .parse()
            .map_err(|_| "device-index is invalid")?,
        chunk_elements: map
            .remove("--chunk-elements")
            .unwrap_or_else(|| "65536".into())
            .parse()
            .map_err(|_| "chunk-elements is invalid")?,
        ...
        expected_served_model_manifest_sha256: take(
        ...
        expected_quantized_artifact_revision: take(
            "--expected-quantized-artifact-revision",
            &mut map,
        )?,
        sq8_artifact_dir: map.remove("--sq8-artifact-dir").map(PathBuf::from),
        sq8_binding_manifest: map.remove("--sq8-binding-manifest").map(PathBuf::from),
        sq8_expected_binding_manifest_sha256: map.remove("--expected-sq8-binding-manifest-sha256"),
        sq8_expected_content_sha256: map.remove("--expected-sq8-content-sha256"),
        sq8_source_model_dir: map.remove("--sq8-source-model-dir").map(PathBuf::from),
        sq8_row_chunk: map
            .remove("--sq8-row-chunk")
            .unwrap_or_else(|| "256".into())
            .parse()
            .map_err(|_| "sq8-row-chunk is invalid")?,
    };
    for (label, value) in [
        (
            "expected split manifest SHA",
            &args.expected_split_manifest_sha256,
        ),
        ("expected policy SHA", &args.expected_policy_sha256),
        (
            "expected calibration cases SHA",
            &args.expected_calibration_cases_sha256,
        ),
        (
            "expected served manifest SHA",
            &args.expected_served_model_manifest_sha256,
        ),
        (
            "expected package manifest SHA",
            &args.expected_package_manifest_sha256,
        ),
        ("expected worker SHA", &args.expected_worker_binary_sha256),
        ("expected guard SHA", &args.expected_guard_sha256),
    ] {
        if !valid_sha(value) {
            return Err(format!("{label} is invalid"));
        }
    }
    if args.expected_device_architecture.is_empty()
        || args.expected_quantized_artifact_revision.is_empty()
    {
        return Err("expected device/revision binding is empty".into());
    }
    let sq8_values = [
        args.sq8_artifact_dir.is_some(),
        args.sq8_binding_manifest.is_some(),
        args.sq8_expected_binding_manifest_sha256.is_some(),
        args.sq8_expected_content_sha256.is_some(),
        args.sq8_source_model_dir.is_some(),
    ];
    if sq8_values.iter().any(|value| *value) && sq8_values.iter().any(|value| !*value) {
        return Err("SQ8 overlay arguments must be supplied as one complete typed set".into());
    }
    if let Some(value) = &args.sq8_expected_binding_manifest_sha256 {
        if !valid_sha(value) {
            return Err("SQ8 binding manifest SHA is invalid".into());
        }
    }
    if let Some(value) = &args.sq8_expected_content_sha256 {
        if !valid_sha(value) {
            return Err("SQ8 content SHA is invalid".into());
        }
    }
    if !map.is_empty()
        || args.chunk_elements == 0
        || args.chunk_elements > MAX_CHUNK_ELEMENTS
        || args.sq8_row_chunk == 0
        || args.sq8_row_chunk > 4096
    {
        return Err("unknown arguments or bounded option violation".into());
    }
    Ok(args)
}
...
fn load_split(
...
) -> Result<(Value, Vec<SplitRow>, String, String, String), String> {
    ...
    if manifest.get("schema_version").and_then(Value::as_str) != Some(SPLIT_SCHEMA)
        || manifest.get("status").and_then(Value::as_str) != Some("ready_for_calibration")
    {
        return Err("split manifest schema/status differs".into());
    }
    ...
    if split_sha != expected_split_sha
        ...
        || cases_sha != expected_cases_sha
    {
        return Err(
            "split/policy/calibration SHA does not match the pinned execution contract".into(),
        );
    }
    let sums = fs::read_to_string(root.join("SHA256SUMS"))
        .map_err(|e| format!("split SHA256SUMS: {e}"))?;
    let mut declared = BTreeMap::new();
    for line in sums.lines() {
        let (digest, name) = line
            .split_once("  ")
            .ok_or("split SHA256SUMS line is invalid")?;
        if !valid_sha(digest)
            || declared
                .insert(name.to_string(), digest.to_string())
                .is_some()
        {
            return Err("split SHA256SUMS contains an invalid or duplicate entry".into());
        }
    }
    let expected_names = [
    ...
    {
        return Err("split SHA256SUMS file set differs".into());
    }
    for name in expected_names {
        let expected = declared
            .get(name)
            .ok_or("split SHA256SUMS entry is missing")?;
        if sha_file(&root.join(name), name)? != *expected {
            return Err(format!("split SHA256SUMS digest differs for {name}"));
        }
    }
    if manifest.get("calibration_sha256").and_then(Value::as_str) != Some(cases_sha.as_str())
        || manifest.get("policy_sha256").and_then(Value::as_str) != Some(policy_sha.as_str())
    {
        return Err("split manifest file binding differs".into());
    }
    ...
    for (line_no, line) in BufReader::new(file).lines().enumerate() {
        ...
        let row: SplitRow = serde_json::from_value(parse_strict_json(
            line.as_bytes(),
            &format!("calibration row {line_no}"),
        )?)
        .map_err(|e| format!("calibration row {line_no}: {e}"))?;
        if !seen.insert(row.case_id.clone()) {
            return Err("calibration rows contain duplicate case_id".into());
        }
        if row.subset != "calibration"
            || row.step != 0
            || row.row_count != 1
            || row.cached_prefix_tokens != 0
            || row.generated_tokens != 0
            || row.prompt_tokens != row.context_tokens
            || !valid_sha(&row.case_sha256)
            || !valid_sha(&row.fixture_sha256)
            || !valid_sha(&row.prompt_token_ids_sha256)
            || !valid_sha(&row.context_token_ids_sha256)
        {
            return Err(format!(
                "calibration row {} violates identity/step contract",
                row.case_id
            ));
        }
        rows.push(row);
    ...
    if rows.len() != MAX_ROWS {
        return Err(format!(
            "calibration cases must contain exactly {MAX_ROWS} rows"
        ));
    }
...
fn fixture_tokens(split_root: &Path, row: &SplitRow) -> Result<Vec<usize>, String> {
    let path = {
        let raw = PathBuf::from(&row.fixture_path);
        if raw.is_absolute() {
            raw
        } else {
            split_root.join(raw)
    ...
    if sha_file(&path, "fixture")? != row.fixture_sha256 {
        return Err(format!("fixture hash differs: {}", row.case_id));
    }
    let value = read_json(&path, "fixture")?;
    if let Some(ids) = value.get("prompt_token_ids").and_then(Value::as_array) {
        return ids
            .iter()
            .map(|v| {
                v.as_u64()
                    .ok_or_else(|| "fixture token is invalid".into())
                    .map(|v| v as usize)
            })
            .collect();
    }
    let cases = value
        ...
        .ok_or("fixture has no prompt token IDs")?;
    let item = cases
        .iter()
        .find(|v| v.get("case_id").and_then(Value::as_str) == Some(row.case_id.as_str()))
        .ok_or("fixture case is missing")?;
    item.get("prompt_token_ids")
        .and_then(Value::as_array)
        .ok_or("fixture prompt token IDs are missing")?
        .iter()
        .map(|v| {
            v.as_u64()
                .ok_or_else(|| "fixture token is invalid".into())
...
fn package_tree_hash(root: &Path) -> Result<String, String> {
    ...
    while let Some(dir) = pending.pop() {
        for entry in fs::read_dir(&dir).map_err(|e| format!("package directory: {e}"))? {
            ...
            let path = entry.path();
            let meta = fs::symlink_metadata(&path).map_err(|e| format!("package metadata: {e}"))?;
            if meta.file_type().is_symlink() {
                return Err("package tree contains symlink".into());
            }
            if meta.is_dir() {
                pending.push(path);
                ...
                return Err("package tree contains non-regular entry".into());
            }
        }
    }
    ...
    for path in files {
        let relative = path
            .strip_prefix(root)
            .map_err(|_| "package path escapes root")?
            .to_string_lossy();
        ...
        for (index, pair) in file_sha.as_bytes().chunks_exact(2).enumerate() {
            raw_sha[index] = (pair[0] as char)
                .to_digit(16)
                .ok_or("package file hash is invalid")? as u8
                * 16
                + (pair[1] as char)
                    .to_digit(16)
                    .ok_or("package file hash is invalid")? as u8;
...
fn run(args: Args) -> Result<(), String> {
    if fs::symlink_metadata(&args.output).is_ok() {
        return Err("output root already exists; overwrite is forbidden".into());
    }
    let (split_manifest, split_rows, split_sha, policy_sha, cases_sha) = load_split(
        &args.split_root,
        &args.expected_split_manifest_sha256,
    ...
    if source_manifest
        ...
        != Some(SOURCE_SCHEMA)
    {
        return Err("source manifest schema differs".into());
    }
    let source_identity = source_manifest
        .get("identity")
        .cloned()
    ...
    let source_cases: SourceCases =
        serde_json::from_value(parse_strict_json(&cases_bytes, "source cases")?)
            .map_err(|e| format!("source cases JSON: {e}"))?;
    if source_cases.schema_version != "ullm.qwen35_aq4_source_calibration_cases.v1"
        || source_cases.cases.len() != MAX_ROWS
        || sha_file(&args.cases, "source cases")?
            != source_manifest
                .get("cases")
                ...
                .unwrap_or("")
    {
        return Err("source cases identity differs".into());
    }
    ...
    let model =
        load_served_model(&args.served_model_manifest).map_err(|e| format!("served model: {e}"))?;
    let sq8_identity = validate_supported_identity(
        &model.format.format_id,
        &model.format.implementation_id,
        &model.worker.identity.device,
        &model.worker.identity.execution_profile,
    )?;
    if sq8_identity != args.sq8_artifact_dir.is_some() {
        return Err("SQ8 served identity and typed overlay arguments differ".into());
    }
    let actual_guards = model
        .worker
    ...
    for name in &model.worker.required_environment {
        if env::var(name).ok().as_deref() != Some("1") {
            return Err(format!("required guard {name} is not set to 1"));
        }
    }
    ...
    if manifest_sha != model.manifest_sha256
        || manifest_sha != args.expected_served_model_manifest_sha256
        || worker_sha != model.worker.binary_sha256
        || worker_sha != args.expected_worker_binary_sha256
    {
        return Err(
            "active served identity hash differs from the pinned execution contract".into(),
        );
    }
    if model.public.revision != args.expected_quantized_artifact_revision {
        return Err(
            "quantized artifact revision differs from the pinned execution contract".into(),
        );
    }
    let package_manifest = model
        .product
        .root
        .join(&model.product.package.manifest_path);
    let package_manifest_sha = sha_file(&package_manifest, "package manifest")?;
    if package_manifest_sha != model.product.package.manifest_sha256
        || package_manifest_sha != args.expected_package_manifest_sha256
    {
        return Err("package manifest hash differs from the pinned execution contract".into());
    }
    ...
    if provenance.model_revision == model.public.revision {
        return Err("upstream and quantized artifact revisions must remain distinct".into());
    }
    let source_model_revision = source_identity
        .get("model_revision")
        .and_then(Value::as_str)
        .ok_or("source model revision is missing")?;
    if source_model_revision != provenance.model_revision
        || source_identity
            ...
            .and_then(|value| value.get("aggregate_sha256"))
            .and_then(Value::as_str)
            != provenance
                .source_checkpoint
        ...
        || source_identity
            ...
            != provenance
                ...
                .and_then(Value::as_str)
    {
        return Err("source identity does not match package-bound upstream provenance".into());
    }
    ...
    let device = ullm_runtime_sys::device_info(args.device_index)
        .map_err(|e| format!("device query: {e}"))?;
    if device.gcn_arch_name != args.expected_device_architecture
        || device.gcn_arch_name != model.worker.identity.device
    {
        return Err("runtime device differs from the pinned active identity".into());
    }
    ...
    if guard_sha != args.expected_guard_sha256 {
        return Err("required guard SHA differs from the pinned execution contract".into());
    }
    if source_identity.get("model_id").and_then(Value::as_str)
        != Some(model.public.upstream_id.as_str())
    {
        return Err("source/active model ID differs".into());
    }
    fs::create_dir_all(args.output.parent().ok_or("output parent missing")?)
    ...
    let temporary = args.output.with_file_name(format!(
        ".{}.incomplete-{}",
        args.output
            .file_name()
    ...
    if fs::symlink_metadata(&temporary).is_ok() {
        return Err("incomplete output already exists".into());
    }
    fs::create_dir(&temporary).map_err(|e| format!("output temporary root: {e}"))?;
    fs::create_dir(temporary.join("vectors"))
        .map_err(|e| format!("output vectors directory: {e}"))?;
    let mut hidden = OpenOptions::new()
        .write(true)
        .create_new(true)
        .open(temporary.join("vectors/hidden.f32le"))
        .map_err(|e| format!("hidden output: {e}"))?;
    let mut logits = OpenOptions::new()
        .write(true)
        .create_new(true)
        .open(temporary.join("vectors/logits.f32le"))
        .map_err(|e| format!("logits output: {e}"))?;
    let mut rows_file = OpenOptions::new()
        .write(true)
        .create_new(true)
        .open(temporary.join("rows.jsonl"))
        .map_err(|e| format!("rows output: {e}"))?;
    let profile = model.profile_snapshot();
    let sq8_overlay = if sq8_identity {
        Some(Qwen35Aq4Sq8OverlayLoadConfig {
            artifact_dir: args
                .sq8_artifact_dir
                .clone()
                .ok_or("SQ8 artifact directory is missing")?,
            binding_manifest: args
                .sq8_binding_manifest
                .clone()
                .ok_or("SQ8 binding manifest is missing")?,
            expected_binding_manifest_sha256: args
                .sq8_expected_binding_manifest_sha256
                .clone()
                .ok_or("SQ8 binding SHA is missing")?,
            expected_content_sha256: args
                .sq8_expected_content_sha256
                .clone()
                .ok_or("SQ8 content SHA is missing")?,
            expected_package_manifest_sha256: package_manifest_sha.clone(),
            expected_source_model_dir: args
                .sq8_source_model_dir
                .clone()
                .ok_or("SQ8 source model directory is missing")?,
            row_chunk: args.sq8_row_chunk,
        })
    } else {
        None
    };
    let model_config = Qwen35Aq4ModelLoadConfig {
        package_dir: package_root.to_path_buf(),
    ...
    let overlay_identity = runtime.sq8_overlay_identity().cloned();
    if sq8_identity && overlay_identity.is_none() {
        return Err("SQ8 served identity loaded without a validated overlay".into());
    }
    if !sq8_identity && overlay_identity.is_some() {
        return Err("plain AQ4 identity loaded an unexpected SQ8 overlay".into());
    }
    if !runtime.calibration_full_logits_top1_available() {
        return Err("active LM head has no full-logit calibration path".into());
    }
    let mut output_rows = 0usize;
    let mut finite_rows = 0usize;
    for row in &split_rows {
        let source_case = source_by_id
            .get(row.case_id.as_str())
            .ok_or_else(|| format!("source case missing: {}", row.case_id))?;
        if source_case.step_count != 1 {
            return Err("fidelity source cases must be one step".into());
        }
        let prompt = fixture_tokens(&args.split_root, row)?;
        if prompt.len() != row.prompt_tokens
            || canonical_token_hash(&prompt, false)? != row.prompt_token_ids_sha256
            || canonical_token_hash(&prompt, true)? != row.context_token_ids_sha256
            || source_case.prompt_token_ids != prompt
        {
            return Err(format!(
                "fixture/case token identity differs: {}",
                row.case_id
            ));
        }
        let requested = row.prefill_requested_m;
        if !matches!(requested, 1 | 8 | 16 | 32 | 64 | 128) {
            return Err(format!("unsupported prefill M for {}", row.case_id));
        }
        ...
        let effective_m = if row.baseline_mode == "all_m1" {
            if row.resolved_m != 1 {
                return Err(format!("all_m1 resolved M differs for {}", row.case_id));
            }
            1
        } else if row.baseline_mode == "cold_batched" {
            if row.resolved_m != requested {
                return Err(format!(
                    "cold_batched resolved M differs for {}",
                    row.case_id
                ));
            }
            requested
        ...
        while offset < prompt.len() {
            let width = effective_m.min(prompt.len() - offset);
            let label = format!("aq4-fidelity-{}-{offset}", row.case_id);
            if width == 1 {
                runtime.dispatch_token_for_phase(
                    prompt[offset],
                ...
                runtime.dispatch_prefill_chunk_for_phase(
        ...
        runtime.synchronize()?;
        let top = runtime
            .top_logits_from_last_layer(TOP_K, &format!("aq4-fidelity-top-{}", row.case_id))?;
        let epoch = runtime
            .last_generation_state_epoch()
            .ok_or("active generation epoch is missing")?;
    ...
    let files = [
        "manifest.json",
        "rows.jsonl",
        "vectors/hidden.f32le",
        "vectors/logits.f32le",
    ];
...
fn main() -> ExitCode {
    let args = match load_args() {
        ...
        Err(error) if error == "help" => {
            eprintln!("usage: ullm-aq4-fidelity-capture --served-model-manifest PATH --split-root DIR --source DIR --cases FILE --output DIR [--device-index N] [--chunk-elements N] [--sq8-artifact-dir DIR --sq8-binding-manifest PATH --expected-sq8-binding-manifest-sha256 SHA --expected-sq8-content-sha256 SHA --sq8-source-model-dir DIR --sq8-row-chunk N]");
            return ExitCode::SUCCESS;
        }
        Err(error) => {
            eprintln!("AQ4 fidelity capture arguments rejected: {error}");
    ...
    match run(args) {
        Ok(()) => ExitCode::SUCCESS,
        Err(error) => {
            eprintln!("AQ4 fidelity capture failed: {error}");
            ExitCode::from(1)
        }
    }
}
...
mod tests {
    ...
    fn observer_streams_bounded_rows_and_orders_top10() {
        ...
        observer
            .observe_hidden_chunk(0, &[1.0; HIDDEN_SIZE])
            .unwrap();
        ...
        let row = SplitRow {
            ...
            prompt_token_ids_sha256: "c".repeat(64),
            context_token_ids_sha256: "d".repeat(64),
            prompt_tokens: 1,
            cached_prefix_tokens: 0,
            context_tokens: 1,
            generated_tokens: 0,
            baseline_mode: "all_m1".into(),
            prefill_requested_m: 1,
            resolved_m: 1,
            step: 0,
            row_count: 1,
            subset: "calibration".into(),
        };
        let result = observer
            .finish_row(&row, "case".into(), "test".into())
            .unwrap();
    ...
    #[test]
    fn supported_identity_accepts_plain_aq4_and_typed_sq8_only() {
        assert_eq!(
        ...
        assert_eq!(
            validate_supported_identity(
                "AQ4_0",
                "qwen35_aq4_sq8_linear_qkv_z_overlay_v1",
                "gfx1201",
    ...
    #[test]
    fn supported_identity_rejects_wrong_sq8_profile_implementation_and_architecture() {
        assert!(validate_supported_identity(
            "SQ8_0",
            "qwen35_aq4_sq8_linear_qkv_z_overlay_v1",
            "gfx1201",
            "rdna4_aq4_resident_sq8_linear_qkv_z_overlay"
        ...
        .is_err());
        let guards = QWEN35_AQ4_REQUIRED_HIP_KERNEL_ENV
            .iter()
            .copied()
            .collect::<BTreeSet<_>>();
    ...
    fn source_provenance_rejects_ambiguous_revision_metadata() {
        ...
        let _ = fs::remove_dir_all(&root);
        for relative in [".cache/huggingface/download", ""] {
            fs::create_dir_all(root.join(relative)).unwrap();
        }
        fs::write(
            root.join("config.json"),
            br#"{"model_type":"qwen3_5","text_config":{"dtype":"bfloat16"}}"#,
        )
        .unwrap();
        fs::write(
            root.join("model.safetensors.index.json"),
            br#"{"weight_map":{"weight":"model.safetensors-00001-of-00001.safetensors"}}"#,
        )
        .unwrap();
        fs::write(
            root.join("model.safetensors-00001-of-00001.safetensors"),
            b"weights",
        )
        .unwrap();
        for name in [
        ...
        fs::write(
            root.join(".cache/huggingface/download/a.metadata"),
            b"revision-a\n",
        )
        .unwrap();
        fs::write(
            root.join(".cache/huggingface/download/b.metadata"),
            b"revision-b\n",
        )
        .unwrap();

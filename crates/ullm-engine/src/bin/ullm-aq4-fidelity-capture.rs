...
const POLICY_METRICS: &[(&str, &str, &str, &str, &str)] = &[
    (
        ...
        "wilson_lower_one_sided(successes=sum(exact 1.0 rows), n=24, confidence_level=0.95); no mean-minus-margin",
    ),
    (
        ...
        "higher",
        "mean",
        "higher: max(absolute_floor, mean-max(absolute_margin, relative_margin*abs(mean))); lower: min(absolute_ceiling, mean+max(absolute_margin, relative_margin*abs(mean)))",
    ),
    (
        "logits_cosine",
        "promotion",
    ...
    (
        ...
        "diagnostic",
        "max",
        "diagnostic_max only; no promotion bound and no absolute ceiling",
    ),
    (
...
fn load_args() -> Result<Args, String> {
    ...
    if !map.is_empty() || args.chunk_elements == 0 || args.chunk_elements > MAX_CHUNK_ELEMENTS {
...
fn run(args: Args) -> Result<(), String> {
    ...
    {
        return Err("served model is not the AQ4 RDNA4 active identity".into());
    }
...
fn main() -> ExitCode {
    let args = match load_args() {
        ...
        Err(error) if error == "help" => {
            eprintln!(
                "usage: ullm-aq4-fidelity-capture --served-model-manifest PATH --split-root DIR --source DIR --cases FILE --output DIR [--device-index N] [--chunk-elements N]"
            );

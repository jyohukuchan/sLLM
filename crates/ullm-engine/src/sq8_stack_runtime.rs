...
mod tests {
    ...
    fn serving_chunk_report_binds_measured_width_prefix_and_cache_progress() {
        for chunk_len in [8, 32, 128] {
            ...
            let overlap_prefix = prefix_position + 1;
            let overlap = Sq8ServingChunkExecutionReport {
                prefix_position: overlap_prefix,
                chunk_len,
                stack: report.stack.clone(),
                cache_lengths: [overlap_prefix + chunk_len; 40],
                kv_write_calls: chunk_len * 40,
                cached_prefix_attention_calls: 40,
                input_d2d_copy_count: 1,
            };
            overlap.validate_contract().unwrap();

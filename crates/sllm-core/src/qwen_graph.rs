...
impl ResidualRmsNormGraphNode for QwenGraphNode {
...
impl QwenGraph {
    ...
    pub(crate) fn with_qwen38_projection_pack_reuse(
        ...
        if self.fp8_sidecar_fingerprint.as_deref() != Some(plan.recipe_digest())
            || self.mtp
            || self.multimodal
        {
            return Err(QwenGraphError::InvalidPlan(
        ...
        for pack in selected {
            ...
            let activation = self
                .tensors
                .get(pack.activation_tensor_id())
                .ok_or_else(|| {
                    QwenGraphError::InvalidPlan(
                        "Qwen3.8 projection-pack activation tensor is absent".to_owned(),
                    )
                })?;
            if activation.view != *pack.activation_view() {
                return Err(QwenGraphError::InvalidPlan(
            ...
            for (node, member) in [(gate, &members[0]), (up, &members[1])] {
                let (validated_index, _) = self.qwen38_projection_node(member.consumer())?;
                if validated_index != member.node_index() {
                    return Err(QwenGraphError::InvalidPlan(format!(
                        "Qwen3.8 projection-pack member role is bound to a different node: {:?}",
                        member.consumer()
                    )));
                }
                let operation = node.operation.as_ref().ok_or_else(|| {
                ...
                let weight = self.tensors.get(member.weight_tensor_id()).ok_or_else(|| {
                    QwenGraphError::InvalidPlan(
                        "Qwen3.8 projection-pack member weight tensor is absent".to_owned(),
                    )
                })?;
                let output = self.tensors.get(member.output_tensor_id()).ok_or_else(|| {
                    QwenGraphError::InvalidPlan(
                        "Qwen3.8 projection-pack member output tensor is absent".to_owned(),
                    )
                })?;
            ...
            if let Some(pack) = packs_by_first_node.get(&cursor).copied() {
                ...
                let contract = match pack.kind() {
                    Qwen38ProjectionPackKind::Nvfp4MlpGateUp => {
                        let weight =
                            self.tensors
                                .get(members[0].weight_tensor_id())
                                .ok_or_else(|| {
                                    QwenGraphError::InvalidPlan(
                                        "Qwen3.8 NVFP4 projection-pack weight tensor is absent"
                                            .to_owned(),
                                    )
                                })?;
                        let [intermediate_size, hidden_size] = match weight.view.shape() {
                            [intermediate_size, hidden_size] => [*intermediate_size, *hidden_size],
                            _ => {
                                return Err(QwenGraphError::InvalidPlan(
                                    "Qwen3.8 NVFP4 projection-pack weight shape differs".to_owned(),
                                ));
                            }
                        };
                        let hidden_size = u32::try_from(hidden_size).map_err(|_| {
                            QwenGraphError::InvalidPlan(
                                "Qwen3.8 NVFP4 projection hidden size exceeds u32".to_owned(),
                            )
                        })?;
                        let intermediate_size = u32::try_from(intermediate_size).map_err(|_| {
                            QwenGraphError::InvalidPlan(
                                "Qwen3.8 NVFP4 projection intermediate size exceeds u32".to_owned(),
                            )
                        })?;
                        Qwen38ProjectionPackContractV1::nvfp4_mlp_gate_up_with_shape(
                            hidden_size,
                            intermediate_size,
                            nvfp4_bits.ok_or_else(|| {
                                QwenGraphError::InvalidPlan(
    ...
    /// Returns a request-local graph with every safe semantic Add -> RMSNorm
    /// pair represented by one residual/RMSNorm operation. The method only
    /// rewrites node metadata; tensor IDs, views, allocations, state
    /// descriptors, and consumers remain unchanged. Backend and target
    /// admission belongs to the execution adapter.
...
/// Build the one-layer BF16 companion graph for the exact Qwen3.8 mixed
/// artifact.  Its MTP block is placed immediately after the 64 target layers;
/// the shared BF16 embedding and untied FP8 output projection remain sourced
/// from the main artifact file.
pub fn build_qwen38_nvfp4_mtp_graph(
    lock: &ModelLock,
    plan: &WeightLoadPlan,
    artifact: &VerifiedUnslothQwen38Nvfp4,
    state_capacity: u64,
...
/// Companion graph variant with an explicit bounded row capacity. The
/// default constructor uses 1024 rows so prompt MTP state priming can submit
/// large state-only batches; decode verification still submits only width
/// three plus its pending row at runtime.
pub fn build_qwen38_nvfp4_mtp_graph_with_token_count(
    lock: &ModelLock,
    plan: &WeightLoadPlan,
    artifact: &VerifiedUnslothQwen38Nvfp4,
    state_capacity: u64,
...
pub fn build_qwen38_nvfp4_mtp_graph_with_companion(
    ...
    if artifact.repository() != crate::UNSLOTH_QWEN38_NVFP4_REPOSITORY
        || artifact.recipe_digest().is_empty()
        || plan.schema_version != "qwen38-nvfp4-mtp-plan-v1"
    {
        return Err(QwenGraphError::InvalidModel(
            "Qwen3.8 MTP artifact or companion plan identity differs".to_owned(),
        ));
    }
    let spec = validate_reviewed_model(lock)?;
    if spec.repo_id != crate::model::QWEN35_27B_REPO_ID {
        return Err(QwenGraphError::InvalidModel(
            "Qwen3.8 MTP requires the reviewed Qwen3.5-27B semantic lock".to_owned(),
        ));
    }
    let dimensions = QwenGraphDimensions::from_spec(spec)?;
    if state_capacity == 0 {
    ...
    if !dimensions.tied_embeddings {
        ...
        if output.encoding != crate::QuantizedTensorEncoding::OcpFp8E4M3FnChannelBf16Scale {
            return Err(QwenGraphError::InvalidPlan(
                "Qwen3.8 MTP output projection is not the verified FP8 channel format".to_owned(),
            ));
        }
        fp8_tensor_names.insert(output.logical_name.clone());
        quantized_weight_encodings.insert(
...
pub fn build_qwen35_gguf_mx_weight_activation_graph(
    ...
    let (bindings, known_unconsumed) = validate_plan(lock, plan, dimensions)?;
    let recipe_options = qwen_mx_recipe_options(lock, source)?;
    let retain_gdn_input_gates = recipe_options.retain_gdn_input_gates;
    let by_name: BTreeMap<_, _> = bindings
        .iter()
        .map(|binding| (binding.tensor_name.as_str(), binding))
        .collect();
        ...
        .filter(|binding| {
            is_fp8_linear_consumer(binding.consumer.role)
                && !(retain_gdn_input_gates
                    && is_qwen_gdn_input_gate_consumer(binding.consumer.role))
        })
        .map(|binding| binding.tensor_name.clone())
        .collect();
    ...
    if retain_gdn_input_gates {
        // A retained tensor must still be physically BF16.  The GGUF weight
        // plan intentionally maps I8 carriers to the logical BF16 dtype, so
        // checking the verified tensor catalog here closes that ambiguity
        // before the upload path can observe a malformed mixed artifact.
        for binding in bindings
            .iter()
            .filter(|binding| is_qwen_gdn_input_gate_consumer(binding.consumer.role))
        {
            let tensor = source.gguf().tensor(&binding.tensor_name).ok_or_else(|| {
                QwenGraphError::InvalidPlan(format!(
                    "retained Qwen3.8 GDN input gate tensor is absent: {}",
                    binding.tensor_name
                ))
            })?;
            if tensor.tensor_type != crate::GgufTensorType::Bf16 {
                return Err(QwenGraphError::InvalidPlan(format!(
                    "retained Qwen3.8 GDN input gate tensor is not BF16: {}",
                    binding.tensor_name
                )));
            }
        }
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
struct QwenMxRecipeOptions {
    retain_gdn_input_gates: bool,
    no_clipping_scale: bool,
}

fn qwen_mx_recipe_options(
    lock: &ModelLock,
    source: &VerifiedGgufWeightSource,
) -> Result<QwenMxRecipeOptions, QwenGraphError> {
    let recipe = source
        .gguf()
        .extension()
        .ok_or_else(|| QwenGraphError::InvalidModel("GGUF MX recipe is missing".to_owned()))?;
    qwen_mx_recipe_options_from_semantic_model_id(lock, &recipe.recipe.semantic_model_id)
}

fn qwen_mx_recipe_options_from_semantic_model_id(
    lock: &ModelLock,
    semantic_model_id: &str,
) -> Result<QwenMxRecipeOptions, QwenGraphError> {
    let semantic_prefix = if lock.model.repo_id == crate::QWEN38_27B_REPO_ID {
        "qwen38"
    } else {
        "qwen35"
    };
    let base = format!("{semantic_prefix}:{}", lock.fingerprint());
    // The first Qwen3.8 MX artifacts were emitted before their family-specific
    // namespace was corrected. Their lock fingerprint is authentic, and the
    // old spelling denotes only the original full-MX coverage. Diagnostic
    // tags are accepted only on the current family-specific namespace.
    if semantic_model_id == base
        || (lock.model.repo_id == crate::QWEN38_27B_REPO_ID
            && semantic_model_id == format!("qwen35:{}", lock.fingerprint()))
    {
        return Ok(QwenMxRecipeOptions {
            retain_gdn_input_gates: false,
            no_clipping_scale: false,
        });
    }
    let retained = format!(
        "{base}:mx-coverage={}",
        crate::gguf_convert::QWEN_MX_DIAGNOSTIC_GDN_INPUT_GATES
    );
    if semantic_model_id == retained {
        return Ok(QwenMxRecipeOptions {
            retain_gdn_input_gates: true,
            no_clipping_scale: false,
        });
    }
    let no_clipping = format!(
        "{base}:mx-scale={}",
        crate::gguf_convert::QWEN_MX_DIAGNOSTIC_NO_CLIPPING_SCALE
    );
    if semantic_model_id == no_clipping {
        return Ok(QwenMxRecipeOptions {
            retain_gdn_input_gates: false,
            no_clipping_scale: true,
        });
    }
    let no_clipping_retained = format!(
        "{no_clipping}:mx-coverage={}",
        crate::gguf_convert::QWEN_MX_DIAGNOSTIC_GDN_INPUT_GATES
    );
    if semantic_model_id == no_clipping_retained {
        return Ok(QwenMxRecipeOptions {
            retain_gdn_input_gates: true,
            no_clipping_scale: true,
        });
    }
    Err(QwenGraphError::InvalidModel(
        "GGUF MX recipe semantic coverage identity differs from the reviewed Qwen identity"
            .to_owned(),
    ))
}
    ...
    for entry in &plan.entries {
        match entry.classification {
            WeightClassification::Required => {
                ...
                let (expected_name, mut expected_dtype, expected_shape) = expected_weight(
                    consumer,
                    &lock.model.architecture.text_config.layer_types,
                    dimensions,
                )?;
                if lock.model.repo_id == crate::QWEN38_27B_REPO_ID
                    && matches!(
                        consumer.role,
                        WeightConsumer::GdnALog | WeightConsumer::GdnNorm
                    )
                {
                    // Qwen3.8 publishes these two GDN parameter vectors in
                    // BF16. Qwen3.5's corresponding reviewed tensors remain
                    // F32 and retain their existing graph contract.
...
fn validate_mtp_plan(
    ...
    if plan.schema_version == lock.schema_version {
        ...
        plan.schema_version.as_str(),
        "gguf-model-plan-v1" | "gguf-quantized-model-plan-v1" | "qwen38-nvfp4-mtp-plan-v1"
    ) || !plan
        .has_valid_digest()
        .map_err(|error| QwenGraphError::InvalidPlan(error.to_string()))?
    {
    ...
    let expected_embedding_role = if lock.model.architecture.text_config.tie_word_embeddings {
        WeightConsumer::EmbeddingAndTiedOutput
    } else {
        WeightConsumer::Embedding
    };
    if required.len() != expected_required
        || required
            .iter()
        ...
        || !required.iter().any(|binding| {
            binding.tensor_name == "model.language_model.embed_tokens.weight"
                && binding.consumer.role == expected_embedding_role
        })
...
impl GraphBuilder {
    fn new(config: GraphBuilderConfig) -> Result<Self, QwenGraphError> {
        ...
        let mtp_consumer_layer = bindings
            .iter()
            .filter_map(|binding| binding.consumer.layer)
            .max()
            .unwrap_or(QWEN35_MTP_CONSUMER_LAYER);
        let mtp_consumer_layer = u32::try_from(mtp_consumer_layer)
            .map_err(|_| QwenGraphError::Overflow("MTP consumer layer"))?;
        let bindings = bindings
            .into_iter()
            .map(|binding| (binding.consumer, binding))
            .collect::<BTreeMap<_, _>>();
        let expected_binding_count = if mtp {
            16 + usize::from(!dimensions.tied_embeddings)
        } else if moe {
    ...
    fn build_mtp_state(&mut self) -> Result<(), QwenGraphError> {
        let layer = self.mtp_consumer_layer;
        let descriptor = qwen_kv_state_descriptor(
            layer,
            self.state_capacity,
            usize::try_from(self.dimensions.kv_heads)
    ...
    fn build_mtp_graph(&mut self) -> Result<(), QwenGraphError> {
        let rows = usize::try_from(self.token_count)
            .map_err(|_| QwenGraphError::Overflow("MTP graph token rows"))?;
        let layer = self.mtp_consumer_layer;
        ...
        let embedding_key = WeightConsumerKey {
            layer: None,
            role: if self.dimensions.tied_embeddings {
                WeightConsumer::EmbeddingAndTiedOutput
            } else {
                WeightConsumer::Embedding
            },
        };
        let embedding_weight = self.weight_tensor(embedding_key)?;
        let embedding = self.add_tensor(
            "embedding.output",
            view(DType::Bf16, &[self.token_count, self.dimensions.hidden])?,
        );
        let embedding_node = self.add_semantic(
            "embedding",
        ...
        // Keep the fused input contiguous. Each row copy is deliberately a
        // one-row contiguous operation: the reviewed elementwise Copy ABI
        // does not accept a strided destination, while the resulting fusion
        // matmul still consumes all rows in one batch.
        let fusion_input = self.add_tensor(
            "mtp.concat.output",
            view(DType::Bf16, &[self.token_count, self.dimensions.hidden * 2])?,
        );
        let half_bytes = self
            .dimensions
            .hidden
        ...
        let embedding_norm_node = self.producers[embedding_norm].ok_or_else(|| {
            QwenGraphError::InvalidPlan("MTP embedding norm has no producer".to_owned())
        })?;
        let hidden_norm_node = self.producers[hidden_norm].ok_or_else(|| {
            QwenGraphError::InvalidPlan("MTP hidden norm has no producer".to_owned())
        })?;
        let mut fusion_copies = Vec::with_capacity(rows * 2);
        for row in 0..rows {
            let row_offset = u64::try_from(row)
                .ok()
                .and_then(|row| row.checked_mul(half_bytes * 2))
                .ok_or(QwenGraphError::Overflow("MTP concat row offset"))?;
            let embedding_offset = u64::try_from(row)
                .ok()
                .and_then(|row| row.checked_mul(half_bytes))
                .ok_or(QwenGraphError::Overflow("MTP embedding row offset"))?;
            let embedding_row = self.add_unproduced_alias(
                &format!("mtp.embedding_norm.row.{row}"),
                embedding_norm,
                TensorView::new(
                    DType::Bf16,
            ...
            let hidden_row = self.add_unproduced_alias(
                &format!("mtp.hidden_norm.row.{row}"),
                hidden_norm,
                TensorView::new(
                    DType::Bf16,
                    Encoding::Unquantized,
            ...
            let embedding_half = self.add_unproduced_alias(
                &format!("mtp.concat.embedding_half.row.{row}"),
                fusion_input,
                TensorView::new(
                    DType::Bf16,
                    Encoding::Unquantized,
            ...
            let hidden_half = self.add_unproduced_alias(
                &format!("mtp.concat.hidden_half.row.{row}"),
                fusion_input,
                TensorView::new(
                    DType::Bf16,
                    Encoding::Unquantized,
                    &[1, self.dimensions.hidden as usize],
                    &[self.dimensions.hidden as usize, 1],
                    row_offset + half_bytes,
                )?,
            )?;
            let copy_embedding = self.add_semantic(
                &format!("mtp.concat.copy_embedding.{row}"),
                SemanticOpDescriptor::new(
                    SemanticOpKind::Copy,
                    vec![self.tensors[embedding_row].view.clone()],
                    vec![self.tensors[embedding_half].view.clone()],
                )?,
                vec![embedding_row],
                vec![embedding_half],
                vec![embedding_node, embedding_norm_node],
                vec![],
            )?;
            let copy_hidden = self.add_semantic(
                &format!("mtp.concat.copy_hidden.{row}"),
                SemanticOpDescriptor::new(
                    SemanticOpKind::Copy,
                    vec![self.tensors[hidden_row].view.clone()],
                    vec![self.tensors[hidden_half].view.clone()],
                )?,
                vec![hidden_row],
                vec![hidden_half],
                vec![hidden_norm_node],
                vec![],
            )?;
            fusion_copies.extend([copy_embedding, copy_hidden]);
        }
        ...
        let output_key = WeightConsumerKey {
            layer: None,
            role: if self.dimensions.tied_embeddings {
                WeightConsumer::EmbeddingAndTiedOutput
            } else {
        ...
        let output_weight = if self.dimensions.tied_embeddings {
            embedding_weight
        } else {
            self.weight_tensor(output_key)?
        };
        let (logits_label, output_label) = if self.dimensions.tied_embeddings {
            ("tied_lm_head.logits", "tied_lm_head_matmul")
        } else {
            ("lm_head.logits", "lm_head_matmul")
        };
        ...
        let output = self.add_tensor("argmax.output", view(DType::I32, &[self.token_count])?);
        self.add_semantic(
            "argmax",
            SemanticOpDescriptor::new(
    ...
    fn layer_input_for_residual(&self, layer: u32) -> Result<usize, QwenGraphError> {
        let input_label = if self.mtp && layer == self.mtp_consumer_layer {
    ...
    fn weight_tensor(&mut self, consumer: WeightConsumerKey) -> Result<usize, QwenGraphError> {
        ...
        let view = if self.fp8_tensor_names.contains(&name) {
            ...
            let dtype = if self.model_fingerprint == crate::QWEN38_27B_FINGERPRINT
                && is_qwen38_gdn_scalar(&name)
            {
                // Qwen3.8 stores these vectors as BF16 but the native GDN
                // ABI consumes the explicitly promoted F32 resident view.
                DType::F32
            } else {
                to_dtype(binding.dtype)?
            };
            view(dtype, &shape)?
        };
...
fn is_qwen38_gdn_scalar(name: &str) -> bool {
...
mod tests {
    ...
    #[test]
    fn qwen_mx_diagnostic_semantic_identity_declares_exact_gdn_retention() {
        let lock = fixed_lock();
        let base = format!("qwen35:{}", lock.fingerprint());
        assert!(
            !qwen_mx_recipe_options_from_semantic_model_id(&lock, &base)
                .unwrap()
                .retain_gdn_input_gates
        );
        let retained = format!(
        ...
        assert!(
            qwen_mx_recipe_options_from_semantic_model_id(&lock, &retained)
                .unwrap()
                .retain_gdn_input_gates
        );
        assert!(qwen_mx_recipe_options_from_semantic_model_id(&lock, "qwen35:unexpected").is_err());
    }

    #[test]
    fn qwen_mx_diagnostic_semantic_identity_accepts_only_declared_scale_modes() {
        let lock = fixed_lock();
        let base = format!("qwen35:{}", lock.fingerprint());
        let no_clipping = format!(
            "{base}:mx-scale={}",
        ...
        assert_eq!(
            qwen_mx_recipe_options_from_semantic_model_id(&lock, &no_clipping).unwrap(),
            QwenMxRecipeOptions {
                retain_gdn_input_gates: false,
                no_clipping_scale: true,
            }
        ...
        assert_eq!(
            qwen_mx_recipe_options_from_semantic_model_id(&lock, &combined).unwrap(),
            QwenMxRecipeOptions {
                retain_gdn_input_gates: true,
                no_clipping_scale: true,
            }
        ...
        assert!(
            qwen_mx_recipe_options_from_semantic_model_id(
                &lock,
                &format!(
                    "{base}:mx-coverage={}:mx-scale={}",
                    crate::gguf_convert::QWEN_MX_DIAGNOSTIC_GDN_INPUT_GATES,
                    crate::gguf_convert::QWEN_MX_DIAGNOSTIC_NO_CLIPPING_SCALE,
                )
            )
        ...
        assert!(
            qwen_mx_recipe_options_from_semantic_model_id(
                &lock,
                &format!("{base}:mx-scale=unexpected")
            )
            .is_err()
        );
    }
    ...
    #[test]
    fn qwen38_mx_legacy_namespace_remains_default_only() {
        let lock = read_model_lock(repository_path("docs/models/locks/qwen3.8-27b-bf16.json"))
            .expect("fixed Qwen3.8 lock parses");
        let current = format!("qwen38:{}", lock.fingerprint());
        let legacy = format!("qwen35:{}", lock.fingerprint());
        assert!(
            !qwen_mx_recipe_options_from_semantic_model_id(&lock, &current)
                .unwrap()
                .retain_gdn_input_gates
        );
        assert!(
            !qwen_mx_recipe_options_from_semantic_model_id(&lock, &legacy)
                .unwrap()
                .retain_gdn_input_gates
        );
        let retained = format!(
        ...
        assert!(
            qwen_mx_recipe_options_from_semantic_model_id(
                &lock,
                &format!("{legacy}:mx-coverage=retain-gdn-in-proj-a-b"),
            )
            .is_err()
        );
        assert!(
    ...
    #[test]
    fn qwen38_projection_pack_lowering_accepts_different_fingerprint_and_partial_plan() {
        let (graph, input_scale_bits) = qwen38_projection_pack_fixture();
        let plan = graph
            .plan_qwen38_projection_pack_reuse_with("qwen38-projection-pack-fixture", |name| {
                input_scale_bits.get(name).copied()
            ...
            .unwrap()
            .unwrap();
        let mut different_model = graph.clone();
        different_model.model_fingerprint = "another-verified-model".to_owned();
        let mut partial = plan.clone();
        partial.packs.truncate(1);

        let lowered = different_model
            .with_qwen38_projection_pack_reuse(
                &partial,
                Qwen38ProjectionPackLoweringScope::Nvfp4MlpGateUpOnly,
            )
            .expect("lowering is driven by the selected verified member");
        assert_eq!(lowered.nodes().len(), graph.nodes().len() - 1);
        assert_eq!(
            lowered
                .nodes()
                .iter()
        ...
        let pack = &partial.packs()[0];
        let weight = &graph.tensor_metadata()[pack.members()[0].weight_tensor_id()].view;
        let lowered_pack = lowered
            .nodes()
            .iter()
            .find(|node| {
                node.kind() == QwenGraphNodeKind::Semantic(SemanticOpKind::Qwen38ProjectionPack2)
            })
            .expect("partial pack lowering");
        let contract = lowered_pack
            .operation()
            .and_then(|operation| operation.qwen38_projection_pack_contract())
            .expect("NVFP4 contract");
        assert_eq!(contract.hidden_size(), weight.shape()[1] as u32);
        assert_eq!(contract.intermediate_size(), weight.shape()[0] as u32);
    }

    #[test]
    fn qwen38_projection_pack_lowering_rejects_mismatched_recipe_and_duplicate_members() {
        let (graph, input_scale_bits) = qwen38_projection_pack_fixture();
        let plan = graph
            .plan_qwen38_projection_pack_reuse_with("qwen38-projection-pack-fixture", |name| {
                input_scale_bits.get(name).copied()
        ...
        let mut mismatched_recipe = plan.clone();
        mismatched_recipe.recipe_digest = "different-recipe".to_owned();
        let error = graph
            .with_qwen38_projection_pack_reuse(
                &mismatched_recipe,
                Qwen38ProjectionPackLoweringScope::Nvfp4MlpGateUpOnly,
            ...
            .unwrap_err()
            .to_string();
        assert!(error.contains("plan identity"), "{error}");

        let mut duplicate = plan.clone();
        let first = duplicate.packs()[0].clone();
        duplicate.packs.push(first);
        let error = graph
            .with_qwen38_projection_pack_reuse(
                &duplicate,
                Qwen38ProjectionPackLoweringScope::Nvfp4MlpGateUpOnly,
    ...
    fn residual_rmsnorm_fusion_is_explicit_and_preserves_intermediate_tensor() {
        ...
        let fused_nodes = fused
            ...
            .collect::<Vec<_>>();
        assert_eq!(fused_nodes.len(), QWEN35_LAYER_COUNT * 2);
        for node in fused_nodes {
            assert_eq!(node.inputs().len(), 3);
            assert_eq!(node.outputs().len(), 2);
            assert!(node.label().ends_with(".fused") || node.label() == "final_rmsnorm");
            assert_eq!(
        ...
        assert_eq!(final_norm.outputs().len(), 2);
    ...
    #[test]
    fn qwen38_residual_rmsnorm_fusion_keeps_an_incomplete_pair_decomposed() {
        ...
        let fused = graph
            .with_residual_rmsnorm_fusion(true)
            .expect("an incomplete pair leaves the original Add and RMSNorm");
        let residual_nodes = fused
            .nodes()
            .iter()
            .filter(|node| {
            ...
            .count();
        assert_eq!(residual_nodes, QWEN38_TARGET_LAYER_COUNT * 2 - 1);
    }

    #[test]
    fn qwen38_residual_rmsnorm_fusion_ignores_model_admission_metadata() {
        ...
        for candidate in [
            ...
            let fused = candidate
                .with_residual_rmsnorm_fusion(true)
                .expect("provenance and admission metadata are outside the semantic matcher");
            assert_eq!(
                fused
                    .nodes()
                    .iter()
    ...
    fn qwen38_mtp_graph_keeps_untied_fp8_output_projection() {
        ...
        let graph = build_qwen38_nvfp4_mtp_graph(
            &lock,
            &plan,
            &artifact,
            17,
        ...
        .expect("Qwen3.8 MTP graph builds");
        let binding = graph
            .weight_bindings()
            .iter()
            .find(|binding| {
        ...
        let output_id = graph
            ...
            .expect("untied output tensor")
            .id();
        assert!(matches!(
            graph.tensor_metadata()[output_id].view().encoding(),
            Encoding::Fp8Scaled {
                granularity: Fp8ScaleGranularity::OuterDimension,
                scale_dtype: DType::F32,
                resident: Fp8ResidentRepresentation::PackedBytes,
        ...
        assert!(graph.nodes().iter().any(|node| {
            node.inputs().get(1) == Some(&output_id)
                ...
                && node.weight_consumers().contains(&binding.consumer())
        }));
        assert!(
            !graph
                .nodes()

...
struct Phase85ManifestCase {
    ...
    oracle: String,
    #[serde(default)]
    activation_variants: Vec<String>,
}
...
struct CaseReport {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    expected_dispatch_count: Option<u32>,
    #[serde(skip_serializing_if = "Option::is_none")]
...
struct Report {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    phase85_activation_variant: Option<&'static str>,
    target: String,
    device_index: u32,
    block_size: usize,
    scale: &'static str,
...
fn validate_actual_dispatch_mode(
    ...
    if matches!(dispatch.kernel_id, 99 | 100) {
        let (expected_id, logical, device) = match format {
            Format::Mxfp8 => (
                99,
        ...
        let grid =
            u32::try_from(n.div_ceil(2)).map_err(|_| "M1 col2 grid exceeds u32".to_owned())?;
        if dispatch.kernel_id != expected_id
            ...
            || (format == Format::Mxfp6
                && ((target == "gfx1030" && k > 5120 && k < 10240)
                    || (target == "gfx1201" && n == 1024 && k < 5120)))
            || dispatch.kernel_symbol != logical
            || dispatch.device_symbol != device
            || dispatch.workgroup_size_x != 256
            || dispatch.grid_size_x != grid
        {
            return Err(format!(
                "M1 col2 dispatch escaped its exact scope: {dispatch:?}"
            ));
        }
    }
...
fn run_case(
    ...
    let activation_quantized = if format == Format::Mxfp8 {
        ...
        format.quantize(&activation_source, m, k)?
    };
...
fn load_phase85_manifest(
    ...
    for (index, entry) in manifest.cases.into_iter().enumerate() {
        ...
        if !entry.activation_variants.is_empty() {
            let base_variant = if format == Format::Mxfp8 { "a8" } else { "a6" };
            if !entry
                .activation_variants
                .iter()
                .any(|variant| variant == base_variant)
            {
                return Err(format!(
                    "Phase 85 case {} does not declare the required {base_variant} variant",
                    entry.case_id
                ));
            }
        }
...
mod tests {
    ...
    #[test]
    fn m1_col2_dispatch_requires_exact_identity_scope_and_tail_grid() {
        for format in [Format::Mxfp8, Format::Mxfp6] {
            let make = |n: usize, target: &str| {
                ...
                DispatchEvidence {
                    ...
                    kernel_id: id,
                    workgroup_size_x: 256,
                    grid_size_x: u32::try_from(n.div_ceil(2)).unwrap(),
                    row_count: 1,
                    normalized_size: n as u64,
                    backend: 1,
                    fallback_allowed: false,
                    fallback_used: false,
                    kernel_symbol: logical.to_owned(),
                    device_symbol: device.to_owned(),
                    target: target.to_owned(),
                }
            };
            for target in ["gfx1030", "gfx1201"] {
                for n in [1024, 1025, 17408] {
                    assert_eq!(
                        validate_actual_dispatch(format, 1, 2080, n, target, &make(n, target))
                            .is_ok(),
                        !(format == Format::Mxfp6 && target == "gfx1201" && n == 1024),
                    );
                }
            }
            for (m, k, n, target) in [
                ...
                assert!(
                    validate_actual_dispatch(format, m, k, n, target, &make(n, target)).is_err()
                );
            }
            assert_eq!(
                validate_actual_dispatch(format, 1, 6144, 5120, "gfx1030", &make(5120, "gfx1030"))
                    .is_ok(),
                format == Format::Mxfp8,
            );
            assert!(
                validate_actual_dispatch(format, 1, 6144, 5120, "gfx1201", &make(5120, "gfx1201"))
                    .is_ok()
            );
            for fault in 0..4 {
                let mut d = make(1025, "gfx1030");
                match fault {
                    0 => d.grid_size_x = 512,
                    1 => d.workgroup_size_x = 128,
                    2 => d.kernel_symbol.push_str("_extra"),
                    _ => d.device_symbol.push_str("_extra"),
                }
                assert!(validate_actual_dispatch(format, 1, 2048, 1025, "gfx1030", &d).is_err());
            }
        }
    }
    ...
    fn retired_mxfp_activation_contract_is_not_registered() {
        ...
        for format in [Format::Mxfp8, Format::Mxfp6] {
            ...
            for (k, n) in [(2016, 1023), (2016, 1024), (2080, 1025)] {
                assert!(
                    validate_actual_dispatch_mode(
                        format,
                        1,
                        k,
                        n,
            ...
            let mut wrong_dispatch_count = make(1, 2016, 1023, "gfx1030");
            wrong_dispatch_count.dispatch_count = 2;
            assert!(
                validate_actual_dispatch_mode(
                    format,
                    1,

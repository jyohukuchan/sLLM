...
- GPU command、service操作、`systemctl`実行: 0

## 次の行動

1. この変更をcommitし、新HEADをrelease sourceとして`CARGO_BUILD_JOBS=1`で`ullm-aq4-worker`をrelease rebuildする。
2. create-new candidate runtimeへbuild receiptとGateをmaterializeし、Gate SHA-256と実行後に必要なevidence一覧を固定する。
3. 独立監査でmaintenance executorがGate sequence、失敗復旧、actual request telemetry、atomic evidence publicationを満たすことを確認した後だけ、1回のactual GPU Gateを許可する。

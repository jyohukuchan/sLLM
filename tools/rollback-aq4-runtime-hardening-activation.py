#!/usr/bin/python3
"""Read-only rollback/recovery preflight for the dedicated AQ4 hardening route."""
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--recover-failed-activation", action="store_true")
...
def _require_mode(args: argparse.Namespace, observed_sha256: str) -> None:
    expected = (
        activation.RECOVERY_CONFIRMATION
        if args.recover_failed_activation
        else activation.ROLLBACK_CONFIRMATION
    )
    if args.execute:
        if args.confirm_plan_sha256 != observed_sha256 or args.confirmation != expected:
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        if args.execute:
            if args.recover_failed_activation:
                result = activation.execute_activation_recovery(
                    args.plan,
                ...
                result = activation.execute_rollback(
                    args.plan,
                    expected_plan_sha256=args.confirm_plan_sha256,
                    confirmation=args.confirmation,
                )
                schema = activation.ROLLBACK_SCHEMA
        ...
        print("AQ4 hardening rollback/recovery failed", file=sys.stderr)
        return 1
